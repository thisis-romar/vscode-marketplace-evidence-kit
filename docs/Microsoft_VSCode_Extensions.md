# Microsoft VS Code Extensions - Complete Catalog

**Comprehensive listing of all Microsoft Visual Studio Code extensions**

---

## 📊 Quick Stats

- **Total Extensions:** 329
- **Categories:** 25
- **Generated:** 2025-12-05 14:14:06
- **Source:** [Microsoft Publisher Page](https://marketplace.visualstudio.com/publishers/Microsoft)

---

## 🏆 Top 10 Most Installed Extensions
1. **[Python](https://marketplace.visualstudio.com/items?itemName=ms-python.python)** - 194.96M installs
2. **[Pylance](https://marketplace.visualstudio.com/items?itemName=ms-python.vscode-pylance)** - 162.84M installs
3. **[Python Debugger](https://marketplace.visualstudio.com/items?itemName=ms-python.debugpy)** - 100.29M installs
4. **[Jupyter](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter)** - 98.83M installs
5. **[C/C++](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools)** - 91.67M installs
6. **[Jupyter Keymap](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-keymap)** - 78.64M installs
7. **[Jupyter Notebook Renderers](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-renderers)** - 77.74M installs
8. **[IntelliCode](https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.vscodeintellicode)** - 60.28M installs
9. **[Jupyter Cell Tags](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-jupyter-cell-tags)** - 59.03M installs
10. **[Jupyter Slide Show](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-jupyter-slideshow)** - 58.96M installs

---

## 📑 Table of Contents

### 🔥 Popular Categories (Top 3)

- 🔮 **[Other Extensions](#other-extensions)** → 76 extensions (23.1%)
- ☁️ **[Azure & Cloud Services](#azure-cloud-services)** → 65 extensions (19.8%)
- 📦 **[Extension Packs](#extension-packs)** → 46 extensions (14%)

### 📚 All Categories (by Count)

| # | Icon | Category | Count | % | Jump |
|---|:---:|----------|------:|---:|------|
| 1 | 🔮 | **Other Extensions** 🔥 | 76 | 23.1% | [View →](#other-extensions) |
| 2 | ☁️ | **Azure & Cloud Services** 🔥 | 65 | 19.8% | [View →](#azure-cloud-services) |
| 3 | 📦 | **Extension Packs** 🔥 | 46 | 14% | [View →](#extension-packs) |
| 4 | 🤖 | **AI & Machine Learning** | 31 | 9.4% | [View →](#ai-machine-learning) |
| 5 | 📜 | **JavaScript & TypeScript** | 14 | 4.3% | [View →](#javascript-typescript) |
| 6 | 🐍 | **Python Development** | 13 | 4% | [View →](#python-development) |
| 7 | 🌐 | **Remote Development** | 10 | 3% | [View →](#remote-development) |
| 8 | 🐳 | **Containers & Kubernetes** | 9 | 2.7% | [View →](#containers-kubernetes) |
| 9 | ⚡ | **.NET Development** | 9 | 2.7% | [View →](#net-development) |
| 10 | 🎓 | **Education & Learning** | 8 | 2.4% | [View →](#education-learning) |
| 11 | 🎨 | **Themes & Appearance** | 8 | 2.4% | [View →](#themes-appearance) |
| 12 | 🧪 | **Testing Tools** | 7 | 2.1% | [View →](#testing-tools) |
| 13 | ☕ | **Java Development** | 6 | 1.8% | [View →](#java-development) |
| 14 | 🌍 | **Web Development** | 5 | 1.5% | [View →](#web-development) |
| 15 | 🔌 | **IoT & Hardware** | 4 | 1.2% | [View →](#iot-hardware) |
| 16 | 🔧 | **C/C++ Development** | 3 | 0.9% | [View →](#c-c-development) |
| 17 | 🐛 | **Debugging Tools** | 3 | 0.9% | [View →](#debugging-tools) |
| 18 | ⚛️ | **Quantum Computing** | 2 | 0.6% | [View →](#quantum-computing) |
| 19 | 👥 | **Collaboration Tools** | 2 | 0.6% | [View →](#collaboration-tools) |
| 20 | 📊 | **Business Central (AL)** | 2 | 0.6% | [View →](#business-central-al) |
| 21 | ✨ | **Linters & Formatters** | 2 | 0.6% | [View →](#linters-formatters) |
| 22 | ⚠️ | **Deprecated Extensions** | 1 | 0.3% | [View →](#deprecated-extensions) |
| 23 | 📈 | **Data Science & Notebooks** | 1 | 0.3% | [View →](#data-science-notebooks) |
| 24 | 🔀 | **Version Control** | 1 | 0.3% | [View →](#version-control) |
| 25 | 💻 | **PowerShell Development** | 1 | 0.3% | [View →](#powershell-development) |

**Total: 329 extensions across 25 categories**

### 🚀 Quick Navigation

- 🐍 [Python Development](#python-development) (13)
- ⚡ [.NET Development](#net-development) (9)
- 📜 [JavaScript & TypeScript](#javascript-typescript) (14)
- ☁️ [Azure & Cloud Services](#azure-cloud-services) (65)
- 🌐 [Remote Development](#remote-development) (10)
- 🤖 [AI & Machine Learning](#ai-machine-learning) (31)

[Back to Top](#microsoft-vs-code-extensions---complete-reference-guide)

---

### 📋 Complete Extension Index

*All 492 extensions organized by category - Click any extension name to jump to its marketplace page*

#### 🔮 Other Extensions

**76 extensions** (23.1%) | [View Full Category →](#other-extensions)

1. **[CMake Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cmake-tools)** - 53.24M installs
2. **[Live Preview](https://marketplace.visualstudio.com/items?itemName=ms-vscode.live-server)** - 11.05M installs
3. **[Makefile Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.makefile-tools)** - 8.65M installs
4. **[Hex Editor](https://marketplace.visualstudio.com/items?itemName=ms-vscode.hexeditor)** - 6.43M installs
5. **[Data Workspace](https://marketplace.visualstudio.com/items?itemName=ms-mssql.data-workspace-vscode)** - 4.77M installs
6. **[SQL Database Projects](https://marketplace.visualstudio.com/items?itemName=ms-mssql.sql-database-projects-vscode)** - 4.65M installs
7. **[Spring Boot Dashboard](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-spring-boot-dashboard)** - 4.32M installs
8. **[jshint](https://marketplace.visualstudio.com/items?itemName=dbaeumer.jshint)** - 2.42M installs
9. **[Notepad++ keymap](https://marketplace.visualstudio.com/items?itemName=ms-vscode.notepadplusplus-keybindings)** - 2.34M installs
10. **[Sublime Text Keymap and Settings Importer](https://marketplace.visualstudio.com/items?itemName=ms-vscode.sublime-keybindings)** - 2.34M installs
11. **[Unity](https://marketplace.visualstudio.com/items?itemName=VisualStudioToolsForUnity.vstuc)** - 1.94M installs
12. **[Serial Monitor](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-serial-monitor)** - 1.72M installs
13. **[Atom Keymap](https://marketplace.visualstudio.com/items?itemName=ms-vscode.atom-keybindings)** - 1.15M installs
14. **[Lombok Annotations Support for VS Code](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-lombok)** - 1.14M installs
15. **[VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech)** - 1.1M installs
16. **[Mono Debug](https://marketplace.visualstudio.com/items?itemName=ms-vscode.mono-debug)** - 917K installs
17. **[File Downloader](https://marketplace.visualstudio.com/items?itemName=mindaro-dev.file-downloader)** - 837K installs
18. **[Visual Studio Keymap](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vs-keybindings)** - 675K installs
19. **[Inkling](https://marketplace.visualstudio.com/items?itemName=ms-inkling.ms-inkling)** - 463K installs
20. **[Pyright](https://marketplace.visualstudio.com/items?itemName=ms-pyright.pyright)** - 459K installs
21. **[Microsoft 365 Agents Toolkit](https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.ms-teams-vscode-extension)** - 393K installs
22. **[SARIF Viewer](https://marketplace.visualstudio.com/items?itemName=MS-SarifVSCode.sarif-viewer)** - 326K installs
23. **[anycode](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode)** - 319K installs
24. **[PostgreSQL](https://marketplace.visualstudio.com/items?itemName=ms-ossdata.vscode-pgsql)** - 299K installs
25. **[Word Count](https://marketplace.visualstudio.com/items?itemName=ms-vscode.wordcount)** - 273K installs
26. **[Power Platform Tools](https://marketplace.visualstudio.com/items?itemName=microsoft-IsvExpTools.powerplatform-vscode)** - 263K installs
27. **[Brackets Keymap](https://marketplace.visualstudio.com/items?itemName=ms-vscode.brackets-keybindings)** - 207K installs
28. **[Embedded Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-embedded-tools)** - 155K installs
29. **[extension-activator](https://marketplace.visualstudio.com/items?itemName=ms-vscode.extension-activator)** - 142K installs
30. **[SandDance for VSCode](https://marketplace.visualstudio.com/items?itemName=msrvida.vscode-sanddance)** - 138K installs
31. **[Power Query / M Language](https://marketplace.visualstudio.com/items?itemName=PowerQuery.vscode-powerquery)** - 132K installs
32. **[anycode-kotlin](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-kotlin)** - 112K installs
33. **[anycode-rust](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-rust)** - 107K installs
34. **[Prompt flow for VS Code](https://marketplace.visualstudio.com/items?itemName=prompt-flow.prompt-flow)** - 87K installs
35. **[JS Debug Value Editor (Experimental)](https://marketplace.visualstudio.com/items?itemName=ms-vscode.debug-value-editor)** - 74K installs
36. **[Fabric Data Engineering VS Code](https://marketplace.visualstudio.com/items?itemName=SynapseVSCode.synapse)** - 70K installs
37. **[Resharper 9 Keybindings](https://marketplace.visualstudio.com/items?itemName=ms-vscode.resharper9-keybindings)** - 64K installs
38. **[DevSkim](https://marketplace.visualstudio.com/items?itemName=MS-CST-E.vscode-devskim)** - 63K installs
39. **[anycode-c-sharp](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-c-sharp)** - 54K installs
40. **[Semantic Kernel Tools](https://marketplace.visualstudio.com/items?itemName=ms-semantic-kernel.semantic-kernel)** - 53K installs
41. **[anycode-php](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-php)** - 51K installs
42. **[anycode-go](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-go)** - 48K installs
43. **[Terraform AzApi Provider](https://marketplace.visualstudio.com/items?itemName=azapi-vscode.azapi)** - 45K installs
44. **[Adaptive Card Previewer](https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.vscode-adaptive-cards)** - 44K installs
45. **[Power Query SDK](https://marketplace.visualstudio.com/items?itemName=PowerQuery.vscode-powerquery-sdk)** - 42K installs
46. **[SCOPE Studio for VS Code](https://marketplace.visualstudio.com/items?itemName=ms-ssdevteam.scope-vscode-ext)** - 42K installs
47. **[GN Language Server](https://marketplace.visualstudio.com/items?itemName=msedge-dev.gnls)** - 38K installs
48. **[Microsoft Fabric](https://marketplace.visualstudio.com/items?itemName=fabric.vscode-fabric)** - 37K installs
49. **[TMDL](https://marketplace.visualstudio.com/items?itemName=analysis-services.TMDL)** - 33K installs
50. **[Security IntelliSense](https://marketplace.visualstudio.com/items?itemName=azsdktm.SecurityIntelliSense)** - 28K installs
51. **[Mock Debug](https://marketplace.visualstudio.com/items?itemName=ms-vscode.mock-debug)** - 27K installs
52. **[DTDL](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.vscode-dtdl)** - 25K installs
53. **[VS Code Commander](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-commander)** - 22K installs
54. **[Microsoft MakeCode Arcade](https://marketplace.visualstudio.com/items?itemName=ms-edu.pxt-vscode-web)** - 16K installs
55. **[Extension Manifest Editor](https://marketplace.visualstudio.com/items?itemName=ms-devlabs.extension-manifest-editor)** - 15K installs
56. **[Dev Box](https://marketplace.visualstudio.com/items?itemName=DevCenter.ms-devbox)** - 11K installs
57. **[SharePoint Embedded](https://marketplace.visualstudio.com/items?itemName=SharepointEmbedded.ms-sharepoint-embedded-vscode-extension)** - 11K installs
58. **[Power BI Modeling MCP Server](https://marketplace.visualstudio.com/items?itemName=analysis-services.powerbi-modeling-mcp)** - 8K installs
59. **[POML](https://marketplace.visualstudio.com/items?itemName=poml-team.poml)** - 6K installs
60. **[Power Platform Tools [PREVIEW]](https://marketplace.visualstudio.com/items?itemName=microsoft-IsvExpTools.powerplatform-vscode-preview)** - 6K installs
61. **[Fabric User data functions](https://marketplace.visualstudio.com/items?itemName=fabric.vscode-fabric-functions)** - 5K installs
62. **[TS/JS Relative File Path Support](https://marketplace.visualstudio.com/items?itemName=ms-vscode.ts-file-path-support)** - 4K installs
63. **[Aspire](https://marketplace.visualstudio.com/items?itemName=microsoft-aspire.aspire-vscode)** - 3K installs
64. **[Browse Published Extensions](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-marketplace-extension-browser)** - 3K installs
65. **[Mezzurite](https://marketplace.visualstudio.com/items?itemName=mezzurite-devs.mezzurite)** - 3K installs
66. **[VS Code Diagnostic Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-diagnostic-tools)** - 3K installs
67. **[Microsoft Sentinel](https://marketplace.visualstudio.com/items?itemName=ms-security.ms-sentinel)** - 3K installs
68. **[Application Insights Code Optimizations](https://marketplace.visualstudio.com/items?itemName=ms-codeoptimizations.code-optimizations-preview)** - 2K installs
69. **[Atlas Design System Tools](https://marketplace.visualstudio.com/items?itemName=docsmsft.atlas-design-system-tools)** - 1K installs
70. **[PSRule](https://marketplace.visualstudio.com/items?itemName=ps-rule.vscode-ps-rule)** - 781 installs
71. **[Touch Adaptation Kit Editor](https://marketplace.visualstudio.com/items?itemName=xbox-tools.vscode-xbox-input-editor-extension)** - 713 installs
72. **[Microsoft Fabric MCP Server](https://marketplace.visualstudio.com/items?itemName=fabric.vscode-fabric-mcp-server)** - 379 installs
73. **[DebugMCP](https://marketplace.visualstudio.com/items?itemName=ozzafar.debugmcpextension)** - 246 installs
74. **[TS/JS VS Code Customized Language Service](https://marketplace.visualstudio.com/items?itemName=ms-vscode.ts-customized-language-service)** - 157 installs
75. **[Chartifact](https://marketplace.visualstudio.com/items?itemName=msrvida.chartifact)** - 114 installs
76. **[Kanagawa Language Support](https://marketplace.visualstudio.com/items?itemName=microsoft-kanagawa.kanagawalang)** - 27 installs

#### ☁️ Azure & Cloud Services

**65 extensions** (19.8%) | [View Full Category →](#azure-cloud-services)

1. **[Docker](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-docker)** - 48.67M installs
2. **[Azure Account](https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-account)** - 9.08M installs
3. **[SQL Server (mssql)](https://marketplace.visualstudio.com/items?itemName=ms-mssql.mssql)** - 8.56M installs
4. **[Container Tools](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-containers)** - 6.94M installs
5. **[Azure Resources](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureresourcegroups)** - 5.82M installs
6. **[Azure Functions](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurefunctions)** - 5.57M installs
7. **[Azure Repos](https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-repos)** - 4.7M installs
8. **[SQL Bindings](https://marketplace.visualstudio.com/items?itemName=ms-mssql.sql-bindings-vscode)** - 4.21M installs
9. **[Azure Cosmos DB](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-cosmosdb)** - 3.15M installs
10. **[Azure App Service](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureappservice)** - 2.87M installs
11. **[Azure Storage](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurestorage)** - 2.42M installs
12. **[Azure CLI Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.azurecli)** - 2.28M installs
13. **[Azure Virtual Machines](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurevirtualmachines)** - 2.01M installs
14. **[Azure Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-node-azure-pack)** - 2.01M installs
15. **[Azure Static Web Apps](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurestaticwebapps)** - 1.86M installs
16. **[Azure Developer CLI](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.azure-dev)** - 1.7M installs
17. **[Azure Container Apps](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurecontainerapps)** - 1.48M installs
18. **[Azure Resource Manager (ARM) Tools](https://marketplace.visualstudio.com/items?itemName=msazurermtools.azurerm-vscode-tools)** - 1.47M installs
19. **[Bicep](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-bicep)** - 1.38M installs
20. **[Azure Pipelines](https://marketplace.visualstudio.com/items?itemName=ms-azure-devops.azure-pipelines)** - 1.36M installs
21. **[Microsoft Terraform](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureterraform)** - 1.12M installs
22. **[GitHub Copilot for Azure](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-github-copilot)** - 868K installs
23. **[GitHub Copilot app modernization](https://marketplace.visualstudio.com/items?itemName=vscjava.migrate-java-to-azure)** - 477K installs
24. **[Azurite](https://marketplace.visualstudio.com/items?itemName=Azurite.azurite)** - 456K installs
25. **[Azure IoT Hub](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-toolkit)** - 435K installs
26. **[Azure MCP Server](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-mcp-server)** - 325K installs
27. **[Azure Data Lake Tools](https://marketplace.visualstudio.com/items?itemName=usqlextpublisher.usql-vscode-ext)** - 310K installs
28. **[Azure IoT Edge](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-edge)** - 257K installs
29. **[Azure API Management](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-apimanagement)** - 246K installs
30. **[Azure Devops Codespaces Authentication](https://marketplace.visualstudio.com/items?itemName=ms-codespaces-tools.ado-codespaces-auth)** - 184K installs
31. **[Azure Logic Apps (Consumption)](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-logicapps)** - 176K installs
32. **[Azure Application Insights](https://marketplace.visualstudio.com/items?itemName=VisualStudioOnlineApplicationInsights.application-insights)** - 139K installs
33. **[Azure PowerShell Tools](https://marketplace.visualstudio.com/items?itemName=azps-tools.azps-tools)** - 124K installs
34. **[Azure Logic Apps (Standard)](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurelogicapps)** - 115K installs
35. **[Azure Automation](https://marketplace.visualstudio.com/items?itemName=azure-automation.vscode-azureautomation)** - 88K installs
36. **[Azure Policy](https://marketplace.visualstudio.com/items?itemName=AzurePolicy.azurepolicyextension)** - 75K installs
37. **[Spark & Hive Tools](https://marketplace.visualstudio.com/items?itemName=mshdinsight.azure-hdinsight)** - 72K installs
38. **[Azure Cache](https://marketplace.visualstudio.com/items?itemName=ms-azurecache.vscode-azurecache)** - 49K installs
39. **[Azure Stream Analytics Tools](https://marketplace.visualstudio.com/items?itemName=ms-bigdatatools.vscode-asa)** - 45K installs
40. **[Azure Data Studio Debug](https://marketplace.visualstudio.com/items?itemName=ms-mssql.sqlops-debug)** - 43K installs
41. **[vscode-dev-azurecloudshell-helper](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-dev-azurecloudshell-helper)** - 40K installs
42. **[Azure Event Grid](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureeventgrid)** - 38K installs
43. **[Azure Sphere](https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-sphere-tools)** - 21K installs
44. **[Azure Serverless](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureserverlesspack)** - 21K installs
45. **[Service Fabric Reliable Services](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-service-fabric-reliable-services)** - 20K installs
46. **[Azure Sphere UI](https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-sphere-tools-ui)** - 18K installs
47. **[Azure Spring Apps](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-azurespringcloud)** - 17K installs
48. **[Microsoft Entra External ID](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.ms-entra)** - 15K installs
49. **[Azure API Center](https://marketplace.visualstudio.com/items?itemName=apidev.azure-api-center)** - 13K installs
50. **[Azure Video Analyzer](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.azure-video-analyzer)** - 12K installs
51. **[Tye](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-tye)** - 10K installs
52. **[VS Code Azure Functions Web](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-functions-web)** - 9K installs
53. **[Azure AI Speech Toolkit](https://marketplace.visualstudio.com/items?itemName=ms-azureaispeech.azure-ai-speech-toolkit)** - 9K installs
54. **[FHIR Converter](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-health-fhir-converter)** - 9K installs
55. **[vscode-azure-functions-remote-web](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-functions-remote-web)** - 8K installs
56. **[DocumentDB for VS Code](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-documentdb)** - 7K installs
57. **[Radius Bicep](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.rad-vscode-bicep)** - 7K installs
58. **[Azure Arc](https://marketplace.visualstudio.com/items?itemName=azurearc-dev.azurearc)** - 4K installs
59. **[Azure Arc-enabled machines](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurearcenabledmachines)** - 3K installs
60. **[Learn Cloud](https://marketplace.visualstudio.com/items?itemName=azurepaas-tools.vscode-learncloud)** - 3K installs
61. **[Azure Web PubSub](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurewebpubsub)** - 2K installs
62. **[Azure OSConfig](https://marketplace.visualstudio.com/items?itemName=edge-security.osconfig)** - 1K installs
63. **[VS Code for the Web - Azure](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-dev-azurecloudshell)** - 675 installs
64. **[Template MCP Server](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-template-mcp-server)** - 674 installs
65. **[Azure Terraform Helper](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.azureterraform-vscode-web-opener)** - 600 installs

#### 📦 Extension Packs

**46 extensions** (14%) | [View Full Category →](#extension-packs)

1. **[C/C++ Extension Pack](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools-extension-pack)** - 48.97M installs
2. **[Chinese (Simplified) (简体中文) Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-zh-hans)** - 46.55M installs
3. **[Extension Pack for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-pack)** - 40.12M installs
4. **[Japanese Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-ja)** - 12.97M installs
5. **[Spanish Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-es)** - 10.55M installs
6. **[Portuguese (Brazil) Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-pt-BR)** - 8.36M installs
7. **[Russian Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-ru)** - 8.06M installs
8. **[Korean Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-ko)** - 6.11M installs
9. **[French Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-fr)** - 5.81M installs
10. **[German Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-de)** - 5.06M installs
11. **[Chinese (Traditional) Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-zh-hant)** - 4.36M installs
12. **[Italian Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-it)** - 3.17M installs
13. **[Polish Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-pl)** - 2.72M installs
14. **[Czech Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-cs)** - 2.17M installs
15. **[Hungarian Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-hu)** - 2M installs
16. **[Bulgarian Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-bg)** - 1.74M installs
17. **[Turkish Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-tr)** - 1.01M installs
18. **[Learn Authoring Pack](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-authoring-pack)** - 118K installs
19. **[Chinese (Simplified, China) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-zh-cn)** - 108K installs
20. **[Brackets Extension Pack](https://marketplace.visualstudio.com/items?itemName=ms-vscode.brackets-pack)** - 101K installs
21. **[Pseudo Language Language Pack](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-qps-ploc)** - 45K installs
22. **[Spanish (Spain) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-es-es)** - 23K installs
23. **[Portuguese (Brazil) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-pt-br)** - 19K installs
24. **[French (France) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-fr-fr)** - 15K installs
25. **[Russian (Russia) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-ru-ru)** - 14K installs
26. **[Japanese (Japan) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-ja-jp)** - 11K installs
27. **[Microsoft AI Tools Extension Pack](https://marketplace.visualstudio.com/items?itemName=ms-windows-ai-studio.microsoft-ai-tools-pack)** - 10K installs
28. **[German language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-de-de)** - 10K installs
29. **[Chinese (Traditional, Taiwan) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-zh-tw)** - 8K installs
30. **[Spanish (Mexico) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-es-mx)** - 6K installs
31. **[English (United Kingdom) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-gb)** - 5K installs
32. **[Korean (South Korea) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-ko-kr)** - 4K installs
33. **[Italian (Italy) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-it-it)** - 3K installs
34. **[Turkish (Turkey) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-tr-tr)** - 3K installs
35. **[Portuguese (Portugal) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-pt-pt)** - 2K installs
36. **[English (India) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-in)** - 2K installs
37. **[Hindi (India) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-hi-in)** - 2K installs
38. **[English (Canada) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-ca)** - 900 installs
39. **[French (Canada) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-fr-ca)** - 791 installs
40. **[English (Australia) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-au)** - 738 installs
41. **[Dutch (Netherlands) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-nl-nl)** - 727 installs
42. **[Swedish (Sweden) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-sv-se)** - 571 installs
43. **[Chinese (Traditional, Hong Kong) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-zh-hk)** - 545 installs
44. **[English (Ireland) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-ie)** - 384 installs
45. **[English (New Zealand) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-nz)** - 332 installs
46. **[Danish (Denmark) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-da-dk)** - 286 installs

#### 🤖 AI & Machine Learning

**31 extensions** (9.4%) | [View Full Category →](#ai-machine-learning)

1. **[Jupyter](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter)** - 98.83M installs
2. **[Jupyter Keymap](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-keymap)** - 78.64M installs
3. **[Jupyter Notebook Renderers](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-renderers)** - 77.74M installs
4. **[IntelliCode](https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.vscodeintellicode)** - 60.28M installs
5. **[Jupyter Cell Tags](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-jupyter-cell-tags)** - 59.03M installs
6. **[Jupyter Slide Show](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-jupyter-slideshow)** - 58.96M installs
7. **[IntelliCode API Usage Examples](https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.intellicode-api-usage-examples)** - 44.6M installs
8. **[Azure Machine Learning](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai)** - 2.68M installs
9. **[Azure Machine Learning - Remote](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai-remote)** - 2.51M installs
10. **[Data Wrangler](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.datawrangler)** - 1.61M installs
11. **[Jupyter PowerToys](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-jupyter-powertoys)** - 792K installs
12. **[AI Toolkit for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=ms-windows-ai-studio.windows-ai-studio)** - 621K installs
13. **[Tensorboard](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.tensorboard)** - 550K installs
14. **[IntelliCode Completions](https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.vscodeintellicode-completions)** - 528K installs
15. **[Microsoft Foundry](https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.vscode-ai-foundry)** - 467K installs
16. **[GitHub Copilot app modernization - upgrade for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-upgrade)** - 463K installs
17. **[JupyterHub](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-hub)** - 253K installs
18. **[IntelliCode Insiders](https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.vscodeintellicode-insiders)** - 177K installs
19. **[Web Search for Copilot](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-websearchforcopilot)** - 128K installs
20. **[vscode-mermAId](https://marketplace.visualstudio.com/items?itemName=ms-vscode.copilot-mermaid-diagram)** - 89K installs
21. **[Vision for Copilot Preview](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-copilot-vision)** - 75K installs
22. **[Prompty](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.prompty)** - 46K installs
23. **[Data Analysis for Copilot](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-copilot-data-analysis)** - 31K installs
24. **[Azure Machine Learning - Inference](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai-inference)** - 15K installs
25. **[Azure Machine Learning - Remote (Web)](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai-remote-web)** - 13K installs
26. **[GenAIScript](https://marketplace.visualstudio.com/items?itemName=genaiscript.genaiscript-vscode)** - 13K installs
27. **[Copilot Studio (Preview)](https://marketplace.visualstudio.com/items?itemName=ms-CopilotStudio.vscode-copilotstudio)** - 11K installs
28. **[Vision AI DevKit](https://marketplace.visualstudio.com/items?itemName=VisionAIDevKit.vision-ai-devkit)** - 10K installs
29. **[Git DevOps Assistant (MCP Powered)](https://marketplace.visualstudio.com/items?itemName=MS-DAW-TCA.ado-productivity-copilot)** - 3K installs
30. **[AI-Powered Content Creator](https://marketplace.visualstudio.com/items?itemName=PDETs-prod.new-module-creation)** - 808 installs
31. **[AI Gallery](https://marketplace.visualstudio.com/items?itemName=ai-gallery.ai-gallery)** - 340 installs

#### 📜 JavaScript & TypeScript

**14 extensions** (4.3%) | [View Full Category →](#javascript-typescript)

1. **[[Deprecated] Debugger for Chrome](https://marketplace.visualstudio.com/items?itemName=msjsdiag.debugger-for-chrome)** - 10.84M installs
2. **[JavaScript and TypeScript Nightly](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-typescript-next)** - 6.13M installs
3. **[React Native Tools](https://marketplace.visualstudio.com/items?itemName=msjsdiag.vscode-react-native)** - 5.79M installs
4. **[JavaScript Debugger (Nightly)](https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-debug-nightly)** - 3.2M installs
5. **[Cordova Tools](https://marketplace.visualstudio.com/items?itemName=msjsdiag.cordova-tools)** - 515K installs
6. **[JavaScript Atom Grammar](https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-atom-grammar)** - 261K installs
7. **[JavaScript Debugger Companion Extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-debug-companion)** - 161K installs
8. **[JavaScript Debugger](https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-debug)** - 114K installs
9. **[Flame Chart Visualizer for JavaScript Profiles](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-js-profile-flame)** - 113K installs
10. **[Debugger for Chrome (Nightly)](https://marketplace.visualstudio.com/items?itemName=msjsdiag.debugger-for-chrome-nightly)** - 90K installs
11. **[React Native Tools (Preview)](https://marketplace.visualstudio.com/items?itemName=msjsdiag.vscode-react-native-preview)** - 69K installs
12. **[anycode-typescript](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-typescript)** - 45K installs
13. **[TypeScript (Native Preview)](https://marketplace.visualstudio.com/items?itemName=TypeScriptTeam.native-preview)** - 42K installs
14. **[Table Visualizer for JavaScript Profiles](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-js-profile-table)** - 19K installs

#### 🐍 Python Development

**13 extensions** (4%) | [View Full Category →](#python-development)

1. **[Python](https://marketplace.visualstudio.com/items?itemName=ms-python.python)** - 194.96M installs
2. **[Pylance](https://marketplace.visualstudio.com/items?itemName=ms-python.vscode-pylance)** - 162.84M installs
3. **[Python Debugger](https://marketplace.visualstudio.com/items?itemName=ms-python.debugpy)** - 100.29M installs
4. **[isort](https://marketplace.visualstudio.com/items?itemName=ms-python.isort)** - 24.92M installs
5. **[Python Environments](https://marketplace.visualstudio.com/items?itemName=ms-python.vscode-python-envs)** - 16.5M installs
6. **[Black Formatter](https://marketplace.visualstudio.com/items?itemName=ms-python.black-formatter)** - 5.97M installs
7. **[autopep8](https://marketplace.visualstudio.com/items?itemName=ms-python.autopep8)** - 5.03M installs
8. **[Pylint](https://marketplace.visualstudio.com/items?itemName=ms-python.pylint)** - 2.91M installs
9. **[Flake8](https://marketplace.visualstudio.com/items?itemName=ms-python.flake8)** - 1.64M installs
10. **[Mypy Type Checker](https://marketplace.visualstudio.com/items?itemName=ms-python.mypy-type-checker)** - 690K installs
11. **[Gather](https://marketplace.visualstudio.com/items?itemName=ms-python.gather)** - 119K installs
12. **[anycode-python](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-python)** - 65K installs
13. **[Experimental - Python for the Web](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-python-web-wasm)** - 8K installs

#### 🌐 Remote Development

**10 extensions** (3%) | [View Full Category →](#remote-development)

1. **[WSL](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-wsl)** - 37.25M installs
2. **[Dev Containers](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers)** - 35.18M installs
3. **[Remote - SSH](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh)** - 30.74M installs
4. **[Remote - SSH: Editing Configuration Files](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh-edit)** - 28.62M installs
5. **[Remote Explorer](https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-explorer)** - 22.76M installs
6. **[Remote Development](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.vscode-remote-extensionpack)** - 8.06M installs
7. **[Remote - Tunnels](https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-server)** - 7.45M installs
8. **[Remote Repositories](https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-repositories)** - 5.18M installs
9. **[WSL: Recommender](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-wsl-recommender)** - 69K installs
10. **[Fabric Data Engineering VS Code - Remote](https://marketplace.visualstudio.com/items?itemName=SynapseVSCode.vscode-synapse-remote)** - 10K installs

#### 🐳 Containers & Kubernetes

**9 extensions** (2.7%) | [View Full Category →](#containers-kubernetes)

1. **[Kubernetes](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.vscode-kubernetes-tools)** - 6.34M installs
2. **[Bridge to Kubernetes](https://marketplace.visualstudio.com/items?itemName=mindaro.mindaro)** - 488K installs
3. **[Azure Kubernetes Service](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.vscode-aks-tools)** - 402K installs
4. **[Kubernetes Kind](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.kind-vscode)** - 102K installs
5. **[Developer Tools for Azure Kubernetes Service (AKS)](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.aks-devx-tools)** - 22K installs
6. **[Open Policy Agent for Kubernetes](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.kubernetes-opa-vscode)** - 5K installs
7. **[Duffle](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.duffle-vscode)** - 4K installs
8. **[Porter](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.porter-vscode)** - 3K installs
9. **[Duffle Coat](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.duffle-coat)** - 2K installs

#### ⚡ .NET Development

**9 extensions** (2.7%) | [View Full Category →](#net-development)

1. **[C#](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csharp)** - 37.45M installs
2. **[.NET Install Tool](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscode-dotnet-runtime)** - 23.23M installs
3. **[C# Dev Kit](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csdevkit)** - 12.56M installs
4. **[IntelliCode for C# Dev Kit](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscodeintellicode-csharp)** - 4.21M installs
5. **[Polyglot Notebooks](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.dotnet-interactive-vscode)** - 1.75M installs
6. **[.NET Extension Pack](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscode-dotnet-pack)** - 1.35M installs
7. **[.NET MAUI](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.dotnet-maui)** - 347K installs
8. **[Microsoft.AspNetCore.Razor.VSCode.BlazorWasmDebuggingExtension](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.blazorwasm-companion)** - 176K installs
9. **[GitHub Copilot app modernization for .NET](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscode-dotnet-modernize)** - 6K installs

#### 🎓 Education & Learning

**8 extensions** (2.4%) | [View Full Category →](#education-learning)

1. **[learn-markdown](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-markdown)** - 471K installs
2. **[learn-yaml](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-yaml)** - 466K installs
3. **[learn-images](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-images)** - 295K installs
4. **[learn-preview](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-preview)** - 247K installs
5. **[learn-article-templates](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-article-templates)** - 125K installs
6. **[learn-scaffolding](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-scaffolding)** - 89K installs
7. **[Learn Linter](https://marketplace.visualstudio.com/items?itemName=docsmsft.learnlinter)** - 3K installs
8. **[Microsoft Learn Authoring Assistant](https://marketplace.visualstudio.com/items?itemName=docsmsft.learn-authoring-assistant)** - 580 installs

#### 🎨 Themes & Appearance

**8 extensions** (2.4%) | [View Full Category →](#themes-appearance)

1. **[C/C++ Themes](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools-themes)** - 50.46M installs
2. **[Markdown Theme Kit](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-MarkdownKit)** - 565K installs
3. **[Material Theme Kit](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-MaterialKit)** - 415K installs
4. **[Tomorrow and Tomorrow Night Theme Kit](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-TomorrowKit)** - 207K installs
5. **[1337 Theme](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-1337)** - 124K installs
6. **[Predawn Theme Kit](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-PredawnKit)** - 107K installs
7. **[3024 Theme Kit](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-3024Kit)** - 26K installs
8. **[Theme Tester](https://marketplace.visualstudio.com/items?itemName=ms-vscode.theme-tester)** - 3K installs

#### 🧪 Testing Tools

**7 extensions** (2.1%) | [View Full Category →](#testing-tools)

1. **[Test Runner for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-test)** - 40.58M installs
2. **[Test Adapter Converter](https://marketplace.visualstudio.com/items?itemName=ms-vscode.test-adapter-converter)** - 4.65M installs
3. **[Playwright Test for VSCode](https://marketplace.visualstudio.com/items?itemName=ms-playwright.playwright)** - 1.98M installs
4. **[Azure Load Testing](https://marketplace.visualstudio.com/items?itemName=ms-azure-load-testing.microsoft-testing)** - 337K installs
5. **[Extension Test Runner](https://marketplace.visualstudio.com/items?itemName=ms-vscode.extension-test-runner)** - 198K installs
6. **[vscode-smoketest-check](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-smoketest-check)** - 174K installs
7. **[Web Template Studio (Preview)](https://marketplace.visualstudio.com/items?itemName=WASTeamAccount.WebTemplateStudio-dev-nightly)** - 134K installs

#### ☕ Java Development

**6 extensions** (1.8%) | [View Full Category →](#java-development)

1. **[Debugger for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-debug)** - 45.46M installs
2. **[Maven for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-maven)** - 41.9M installs
3. **[Project Manager for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-dependency)** - 40.33M installs
4. **[Gradle for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-gradle)** - 21.23M installs
5. **[Spring Initializr Java Support](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-spring-initializr)** - 5.68M installs
6. **[anycode-java](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-java)** - 54K installs

#### 🌍 Web Development

**5 extensions** (1.5%) | [View Full Category →](#web-development)

1. **[Microsoft Edge Tools for VS Code](https://marketplace.visualstudio.com/items?itemName=ms-edgedevtools.vscode-edge-devtools)** - 5.51M installs
2. **[WebAssembly DWARF Debugging](https://marketplace.visualstudio.com/items?itemName=ms-vscode.wasm-dwarf-debugging)** - 19K installs
3. **[WASI based WebAssembly Execution Engine](https://marketplace.visualstudio.com/items?itemName=ms-vscode.wasm-wasi-core)** - 13K installs
4. **[Experimental - A shell for VS Code Web](https://marketplace.visualstudio.com/items?itemName=ms-vscode.webshell)** - 4K installs
5. **[Custom Web Editors (Experimental)](https://marketplace.visualstudio.com/items?itemName=ms-vscode.web-editors)** - 3K installs

#### 🔌 IoT & Hardware

**4 extensions** (1.2%) | [View Full Category →](#iot-hardware)

1. **[ROS](https://marketplace.visualstudio.com/items?itemName=ms-iot.vscode-ros)** - 1.32M installs
2. **[IoT Device Cube](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.vscode-iot-device-cube)** - 122K installs
3. **[Microsoft Kiota](https://marketplace.visualstudio.com/items?itemName=ms-graph.kiota)** - 24K installs
4. **[Microsoft Open Enclave](https://marketplace.visualstudio.com/items?itemName=ms-iot.msiot-vscode-openenclave)** - 19K installs

#### 🔧 C/C++ Development

**3 extensions** (0.9%) | [View Full Category →](#c-c-development)

1. **[C/C++](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools)** - 91.67M installs
2. **[anycode-cpp](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-cpp)** - 71K installs
3. **[anycode-cpp](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-cpp)** - 71K installs

#### 🐛 Debugging Tools

**3 extensions** (0.9%) | [View Full Category →](#debugging-tools)

1. **[[Deprecated] Node Debug](https://marketplace.visualstudio.com/items?itemName=ms-vscode.node-debug2)** - 374K installs
2. **[[Deprecated] Node Debug (legacy)](https://marketplace.visualstudio.com/items?itemName=ms-vscode.node-debug)** - 231K installs
3. **[Microsoft Office Add-ins Development Kit](https://marketplace.visualstudio.com/items?itemName=msoffice.microsoft-office-add-in-debugger)** - 166K installs

#### ⚛️ Quantum Computing

**2 extensions** (0.6%) | [View Full Category →](#quantum-computing)

1. **[Azure Quantum Development Kit (QDK)](https://marketplace.visualstudio.com/items?itemName=quantum.qsharp-lang-vscode)** - 25K installs
2. **[[DEV BUILD] Azure Quantum Development Kit (QDK)](https://marketplace.visualstudio.com/items?itemName=quantum.qsharp-lang-vscode-dev)** - 4K installs

#### 👥 Collaboration Tools

**2 extensions** (0.6%) | [View Full Category →](#collaboration-tools)

1. **[Live Share](https://marketplace.visualstudio.com/items?itemName=MS-vsliveshare.vsliveshare)** - 22.23M installs
2. **[[Deprecated] Live Share Audio](https://marketplace.visualstudio.com/items?itemName=MS-vsliveshare.vsliveshare-audio)** - 5.45M installs

#### 📊 Business Central (AL)

**2 extensions** (0.6%) | [View Full Category →](#business-central-al)

1. **[AL Language extension for Microsoft Dynamics 365 Business Central](https://marketplace.visualstudio.com/items?itemName=ms-dynamics-smb.al)** - 470K installs
2. **[Performance Toolkit](https://marketplace.visualstudio.com/items?itemName=ms-dynamics-smb.bcpt)** - 7K installs

#### ✨ Linters & Formatters

**2 extensions** (0.6%) | [View Full Category →](#linters-formatters)

1. **[ESLint](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint)** - 46.85M installs
2. **[TSLint](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-typescript-tslint-plugin)** - 4.47M installs

#### ⚠️ Deprecated Extensions

**1 extensions** (0.3%) | [View Full Category →](#deprecated-extensions)

1. **[[DEPRECATED] LaTeX Language Support](https://marketplace.visualstudio.com/items?itemName=ms-vscode.latex)** - 42K installs

#### 📈 Data Science & Notebooks

**1 extensions** (0.3%) | [View Full Category →](#data-science-notebooks)

1. **[Markdown Notebook](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-markdown-notebook)** - 46K installs

#### 🔀 Version Control

**1 extensions** (0.3%) | [View Full Category →](#version-control)

1. **[GitHub Issue Notebooks](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-github-issue-notebooks)** - 255K installs

#### 💻 PowerShell Development

**1 extensions** (0.3%) | [View Full Category →](#powershell-development)

1. **[PowerShell](https://marketplace.visualstudio.com/items?itemName=ms-vscode.PowerShell)** - 17.05M installs

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Other Extensions

**Category Stats:** 76 extensions | Total Installs: 114.53M | Avg Rating: 4.4⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [CMake Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cmake-tools)
*Extension #1 of 329*

Extended CMake support in Visual Studio Code

**Details:**

- **Extension ID:** `ms-vscode.cmake-tools`
- **Version:** 1.22.20
- **Installs:** 53.24M
- **Rating:** ⭐ 4.5/5 (75 ratings)
- **Published:** 2016-04-16
- **Last Updated:** 2025-12-05
- **VS Code Engine:** ^1.88.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.cmake-tools

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-cmake-tools.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-cmake-tools/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-cmake-tools)

**Marketplace Categories:**

Programming Languages, Debuggers

**Tags:**

__ext_cmake, build, c++, cmake, CMakeCache, cmake-cache, debuggers, json, keybindings, native, syntaxes, tools

**Works With:**

All Platforms

---

### 2. [Live Preview](https://marketplace.visualstudio.com/items?itemName=ms-vscode.live-server)
*Extension #2 of 329*

Hosts a local server in your workspace for you to preview your webpages on.

**Details:**

- **Extension ID:** `ms-vscode.live-server`
- **Version:** 0.5.2025091501
- **Installs:** 11.05M
- **Rating:** ⭐ 4.4/5 (76 ratings)
- **Published:** 2021-06-21
- **Last Updated:** 2025-09-15
- **VS Code Engine:** ^1.80.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.live-server

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-livepreview.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-livepreview/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-livepreview#readme)

**Marketplace Categories:**

Other

**Tags:**

browser, html, live, livepreview, preview, refresh, reload

**Works With:**

All Platforms

---

### 3. [Makefile Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.makefile-tools)
*Extension #3 of 329*

Provide makefile support in VS Code: C/C++ IntelliSense, build, debug/run.

**Details:**

- **Extension ID:** `ms-vscode.makefile-tools`
- **Version:** 0.13.12
- **Installs:** 8.65M
- **Rating:** ⭐ 2.6/5 (39 ratings)
- **Published:** 2021-02-17
- **Last Updated:** 2025-09-01
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.makefile-tools

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-makefile-tools.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-makefile-tools/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-makefile-tools)

**Marketplace Categories:**

Programming Languages, Debuggers

**Tags:**

C, C++, IntelliSense, Makefile, Microsoft

**Works With:**

All Platforms

---

### 4. [Hex Editor](https://marketplace.visualstudio.com/items?itemName=ms-vscode.hexeditor)
*Extension #4 of 329*

Allows viewing and editing files in a hex editor

**Details:**

- **Extension ID:** `ms-vscode.hexeditor`
- **Version:** 1.11.1
- **Installs:** 6.43M
- **Rating:** ⭐ 3.5/5 (60 ratings)
- **Published:** 2020-06-08
- **Last Updated:** 2024-11-03
- **VS Code Engine:** ^1.89.0-insider
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.hexeditor

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-hexeditor.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-hexeditor/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-hexeditor#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, keybindings

**Works With:**

All Platforms

---

### 5. [Data Workspace](https://marketplace.visualstudio.com/items?itemName=ms-mssql.data-workspace-vscode)
*Extension #5 of 329*

Additional common functionality for database projects

**Details:**

- **Extension ID:** `ms-mssql.data-workspace-vscode`
- **Version:** 0.6.3
- **Installs:** 4.77M
- **Rating:** ⭐ 4/5 (4 ratings)
- **Published:** 2021-10-27
- **Last Updated:** 2025-06-18
- **VS Code Engine:** >=1.48.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-mssql.data-workspace-vscode

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/azuredatastudio.git)
- 🐛 [Issues](https://github.com/Microsoft/azuredatastudio/issues)
- 🏠 [Homepage](https://github.com/Microsoft/azuredatastudio#readme)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 6. [SQL Database Projects](https://marketplace.visualstudio.com/items?itemName=ms-mssql.sql-database-projects-vscode)
*Extension #6 of 329*

Enables users to develop and publish database schemas for MSSQL Databases

**Details:**

- **Extension ID:** `ms-mssql.sql-database-projects-vscode`
- **Version:** 1.5.5
- **Installs:** 4.65M
- **Rating:** ⭐ 4.2/5 (5 ratings)
- **Published:** 2021-10-27
- **Last Updated:** 2025-11-18
- **VS Code Engine:** ^1.98.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-mssql.sql-database-projects-vscode

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/azuredatastudio.git)
- 🐛 [Issues](https://github.com/Microsoft/azuredatastudio/issues)
- 🏠 [Homepage](https://github.com/Microsoft/azuredatastudio#readme)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 7. [Spring Boot Dashboard](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-spring-boot-dashboard)
*Extension #7 of 329*

Spring Boot Dashboard for VS Code

**Details:**

- **Extension ID:** `vscjava.vscode-spring-boot-dashboard`
- **Version:** 0.14.2025041702
- **Installs:** 4.32M
- **Rating:** ⭐ 4.2/5 (9 ratings)
- **Published:** 2018-09-14
- **Last Updated:** 2025-04-17
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-spring-boot-dashboard

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-spring-boot-dashboard.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-spring-boot-dashboard/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-spring-boot-dashboard)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 8. [jshint](https://marketplace.visualstudio.com/items?itemName=dbaeumer.jshint)
*Extension #8 of 329*

Integrates JSHint into VS Code. JSHint is a linter for JavaScript

**Details:**

- **Extension ID:** `dbaeumer.jshint`
- **Version:** 0.11.0
- **Installs:** 2.42M
- **Rating:** ⭐ 3.7/5 (41 ratings)
- **Published:** 2015-11-06
- **Last Updated:** 2020-11-10
- **VS Code Engine:** ^1.26.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=dbaeumer.jshint

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-jshint.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-jshint/issues)

**Marketplace Categories:**

Linters

**Tags:**

html, javascript, javascriptreact, json

**Works With:**

All Platforms

---

### 9. [Notepad++ keymap](https://marketplace.visualstudio.com/items?itemName=ms-vscode.notepadplusplus-keybindings)
*Extension #9 of 329*

Popular Notepad++ keybindings for Visual Studio Code

**Details:**

- **Extension ID:** `ms-vscode.notepadplusplus-keybindings`
- **Version:** 1.0.7
- **Installs:** 2.34M
- **Rating:** ⭐ 4.7/5 (12 ratings)
- **Published:** 2017-12-04
- **Last Updated:** 2021-03-26
- **VS Code Engine:** ^1.55.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.notepadplusplus-keybindings

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-notepadplusplus-keybindings.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-notepadplusplus-keybindings/issues)

**Marketplace Categories:**

Keymaps

**Tags:**

__web_extension, keybindings, keymap

**Works With:**

All Platforms

---

### 10. [Sublime Text Keymap and Settings Importer](https://marketplace.visualstudio.com/items?itemName=ms-vscode.sublime-keybindings)
*Extension #10 of 329*

Import Sublime Text settings and keybindings into VS Code.

**Details:**

- **Extension ID:** `ms-vscode.sublime-keybindings`
- **Version:** 4.1.10
- **Installs:** 2.34M
- **Rating:** ⭐ 5/5 (37 ratings)
- **Published:** 2016-03-11
- **Last Updated:** 2024-01-19
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.sublime-keybindings

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-sublime-keybindings.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-sublime-keybindings/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-sublime-keybindings#readme)

**Marketplace Categories:**

Keymaps

**Tags:**

__web_extension, Importer, keybindings, keymap, Settings, Sublime Text

**Works With:**

All Platforms

---

### 11. [Unity](https://marketplace.visualstudio.com/items?itemName=VisualStudioToolsForUnity.vstuc)
*Extension #11 of 329*

Integrates Visual Studio Code with Unity

**Details:**

- **Extension ID:** `VisualStudioToolsForUnity.vstuc`
- **Version:** 1.2.0
- **Installs:** 1.94M
- **Rating:** ⭐ 3.8/5 (76 ratings)
- **Published:** 2023-08-03
- **Last Updated:** 2025-11-21
- **VS Code Engine:** ^1.76.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=VisualStudioToolsForUnity.vstuc

**Resources:**

- 🐛 [Issues](https://github.com/microsoft/vscode-dotnettools/issues)

**Marketplace Categories:**

Programming Languages, Linters, Debuggers

**Tags:**

__ext_asmdef, __ext_cginc, __ext_compute, __ext_hlsl, __ext_raytrace, __ext_shader, __ext_uss, __ext_uxml, C#, debugger, debuggers, dotnet, gaming, json, scripting, unity, unity3d, UnityShader, UnityUSS, UnityUXML

**Works With:**

All Platforms

---

### 12. [Serial Monitor](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-serial-monitor)
*Extension #12 of 329*

Send and receive text from serial ports.

**Details:**

- **Extension ID:** `ms-vscode.vscode-serial-monitor`
- **Version:** 0.13.251128001
- **Installs:** 1.72M
- **Rating:** ⭐ 3.6/5 (37 ratings)
- **Published:** 2022-06-06
- **Last Updated:** 2025-11-28
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-serial-monitor

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-serial-monitor.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-serial-monitor/issues)
- 💬 [Support](https://developercommunity.visualstudio.com/cpp)
- 🏠 [Homepage](https://github.com/microsoft/vscode-serial-monitor)

**Marketplace Categories:**

Visualization, Testing

**Tags:**

embedded, monitor, port, serial

**Works With:**

All Platforms

---

### 13. [Atom Keymap](https://marketplace.visualstudio.com/items?itemName=ms-vscode.atom-keybindings)
*Extension #13 of 329*

Popular Atom keybindings for Visual Studio Code

**Details:**

- **Extension ID:** `ms-vscode.atom-keybindings`
- **Version:** 3.3.0
- **Installs:** 1.15M
- **Rating:** ⭐ 4.8/5 (22 ratings)
- **Published:** 2016-09-23
- **Last Updated:** 2023-01-20
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.atom-keybindings

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-atom-keybindings.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-atom-keybindings/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-atom-keybindings#readme)

**Marketplace Categories:**

Keymaps

**Tags:**

__web_extension, keybindings, keymap

**Works With:**

All Platforms

---

### 14. [Lombok Annotations Support for VS Code](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-lombok)
*Extension #14 of 329*

Refactor code with Lombok annotations, or remove Lombok annotations with actual methods.

**Details:**

- **Extension ID:** `vscjava.vscode-lombok`
- **Version:** 1.1.2024071804
- **Installs:** 1.14M
- **Rating:** ⭐ 4.3/5 (19 ratings)
- **Published:** 2018-03-11
- **Last Updated:** 2024-07-18
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-lombok

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-lombok.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-lombok/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-lombok#readme)

**Marketplace Categories:**

Other

**Tags:**

__ext_jav, __ext_java, java, lombok, vs-code

**Works With:**

All Platforms

---

### 15. [VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech)
*Extension #15 of 329*

A VS Code extension to bring speech-to-text and other voice capabilities to VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech`
- **Version:** 0.16.0
- **Installs:** 1.1M
- **Rating:** ⭐ 3.4/5 (23 ratings)
- **Published:** 2023-10-31
- **Last Updated:** 2025-06-13
- **VS Code Engine:** ^1.90.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, transcription, voice

**Works With:**

macOS Apple Silicon, Linux ARM64, Windows x64, Windows ARM64, macOS Intel, Linux ARM, Linux x64

---

### 16. [Mono Debug](https://marketplace.visualstudio.com/items?itemName=ms-vscode.mono-debug)
*Extension #16 of 329*

Visual Studio Code debugger extension for Mono

**Details:**

- **Extension ID:** `ms-vscode.mono-debug`
- **Version:** 0.16.3
- **Installs:** 917K
- **Rating:** ⭐ 3.5/5 (11 ratings)
- **Published:** 2016-02-19
- **Last Updated:** 2022-08-09
- **VS Code Engine:** ^1.32.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.mono-debug

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-mono-debug.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-mono-debug/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-mono-debug#readme)

**Marketplace Categories:**

Debuggers

**Tags:**

debuggers

**Works With:**

All Platforms

---

### 17. [File Downloader](https://marketplace.visualstudio.com/items?itemName=mindaro-dev.file-downloader)
*Extension #17 of 329*

Exposes an API that allows other extensions to download files.

**Details:**

- **Extension ID:** `mindaro-dev.file-downloader`
- **Version:** 1.0.13
- **Installs:** 837K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2020-07-13
- **Last Updated:** 2023-10-16
- **VS Code Engine:** ^1.66.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=mindaro-dev.file-downloader

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-file-downloader.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-file-downloader/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-file-downloader#readme)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 18. [Visual Studio Keymap](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vs-keybindings)
*Extension #18 of 329*

Popular Visual Studio keybindings for VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vs-keybindings`
- **Version:** 0.2.1
- **Installs:** 675K
- **Rating:** ⭐ 4.3/5 (35 ratings)
- **Published:** 2016-11-15
- **Last Updated:** 2021-04-06
- **VS Code Engine:** ^1.22.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vs-keybindings

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-vs-keybindings.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-vs-keybindings/issues)

**Marketplace Categories:**

Keymaps

**Tags:**

__web_extension, keybindings, keymap

**Works With:**

All Platforms

---

### 19. [Inkling](https://marketplace.visualstudio.com/items?itemName=ms-inkling.ms-inkling)
*Extension #19 of 329*

VS Code language support for the inkling language

**Details:**

- **Extension ID:** `ms-inkling.ms-inkling`
- **Version:** 1.0.40
- **Installs:** 463K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2019-03-07
- **Last Updated:** 2022-06-21
- **VS Code Engine:** ^1.52.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-inkling.ms-inkling

**Resources:**

- 🏠 [Homepage](https://www.bons.ai)

**Marketplace Categories:**

Programming Languages

**Tags:**

__ext_ink, ai, inkling, reinforcement

**Works With:**

All Platforms

---

### 20. [Pyright](https://marketplace.visualstudio.com/items?itemName=ms-pyright.pyright)
*Extension #20 of 329*

VS Code static type checking for Python

**Details:**

- **Extension ID:** `ms-pyright.pyright`
- **Version:** 1.1.407
- **Installs:** 459K
- **Rating:** ⭐ 4.6/5 (20 ratings)
- **Published:** 2019-03-21
- **Last Updated:** 2025-10-22
- **VS Code Engine:** ^1.101.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-pyright.pyright

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/pyright.git)
- 🐛 [Issues](https://github.com/Microsoft/pyright/issues)
- 🏠 [Homepage](https://github.com/Microsoft/pyright#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__ext_py, __ext_pyi, json, python

**Works With:**

All Platforms

---

### 21. [Microsoft 365 Agents Toolkit](https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.ms-teams-vscode-extension)
*Extension #21 of 329*

Create, debug, and deploy agents with Microsoft 365 Agents Toolkit

**Details:**

- **Extension ID:** `TeamsDevApp.ms-teams-vscode-extension`
- **Version:** 6.5.2025120408
- **Installs:** 393K
- **Rating:** ⭐ 3.5/5 (39 ratings)
- **Published:** 2020-05-13
- **Last Updated:** 2025-12-05
- **VS Code Engine:** ^1.104.2
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.ms-teams-vscode-extension

**Resources:**

- 📦 [Repository](https://github.com/OfficeDev/TeamsFx.git)
- 🐛 [Issues](https://github.com/OfficeDev/TeamsFx/issues)
- 🏠 [Homepage](https://github.com/OfficeDev/TeamsFx#readme)

**Marketplace Categories:**

Snippets, Extension Packs, Azure

**Tags:**

Azure, Microsoft 365, Microsoft Agents, teamsfx-toolkit-output

**Works With:**

All Platforms

---

### 22. [SARIF Viewer](https://marketplace.visualstudio.com/items?itemName=MS-SarifVSCode.sarif-viewer)
*Extension #22 of 329*

Adds support for viewing SARIF logs

**Details:**

- **Extension ID:** `MS-SarifVSCode.sarif-viewer`
- **Version:** 3.4.5
- **Installs:** 326K
- **Rating:** ⭐ 3.8/5 (6 ratings)
- **Published:** 2018-04-25
- **Last Updated:** 2025-05-06
- **VS Code Engine:** ^1.72.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-SarifVSCode.sarif-viewer

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/sarif-vscode-extension.git)
- 🐛 [Issues](https://github.com/Microsoft/sarif-vscode-extension/issues)
- 🏠 [Homepage](https://github.com/Microsoft/sarif-vscode-extension#readme)

**Marketplace Categories:**

Other

**Tags:**

__ext_sarif, json, sarif

**Works With:**

All Platforms

---

### 23. [anycode](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode)
*Extension #23 of 329*

**Details:**

- **Extension ID:** `ms-vscode.anycode`
- **Version:** 0.0.75
- **Installs:** 319K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2021-07-09
- **Last Updated:** 2025-08-18
- **VS Code Engine:** ^1.80.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-anycode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-anycode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-anycode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 24. [PostgreSQL](https://marketplace.visualstudio.com/items?itemName=ms-ossdata.vscode-pgsql)
*Extension #24 of 329*

Develop PostgreSQL applications everywhere.

**Details:**

- **Extension ID:** `ms-ossdata.vscode-pgsql`
- **Version:** 1.12.1
- **Installs:** 299K
- **Rating:** ⭐ 4.5/5 (32 ratings)
- **Published:** 2025-05-14
- **Last Updated:** 2025-12-02
- **VS Code Engine:** ^1.95.2
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-ossdata.vscode-pgsql

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-pgsql.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-pgsql/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-pgsql/blob/main/README.md)

**Marketplace Categories:**

Programming Languages, Azure

**Tags:**

__ext_sql, Azure PostgreSQL, chat-participant, keybindings, language-model-tools, PGSQL, Postgres, PostgreSQL, snippet, SQL, tools

**Works With:**

Linux ARM64, Linux x64, macOS Intel, macOS Apple Silicon, Windows x64

---

### 25. [Word Count](https://marketplace.visualstudio.com/items?itemName=ms-vscode.wordcount)
*Extension #25 of 329*

Markdown Word Count Example - a status bar contribution that reports out the number of works in a Markdown document as you interact with it.

**Details:**

- **Extension ID:** `ms-vscode.wordcount`
- **Version:** 0.1.0
- **Installs:** 273K
- **Rating:** ⭐ 2.8/5 (6 ratings)
- **Published:** 2015-11-17
- **Last Updated:** 2015-11-18
- **VS Code Engine:** 0.10.x
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.wordcount

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-wordcount.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-wordcount/issues)

**Marketplace Categories:**

Other

**Tags:**

markdown

**Works With:**

All Platforms

---

### 26. [Power Platform Tools](https://marketplace.visualstudio.com/items?itemName=microsoft-IsvExpTools.powerplatform-vscode)
*Extension #26 of 329*

Tooling to create Power Platform solutions & packages, manage Power Platform environments and edit Power Apps Portals

**Details:**

- **Extension ID:** `microsoft-IsvExpTools.powerplatform-vscode`
- **Version:** 2.0.110
- **Installs:** 263K
- **Rating:** ⭐ 3.8/5 (4 ratings)
- **Published:** 2021-05-25
- **Last Updated:** 2025-12-04
- **VS Code Engine:** ^1.91.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=microsoft-IsvExpTools.powerplatform-vscode

**Resources:**

- 📦 [Repository](https://github.com/microsoft/powerplatform-vscode.git)
- 🐛 [Issues](https://github.com/microsoft/powerplatform-vscode/issues)
- 🏠 [Homepage](https://github.com/microsoft/powerplatform-vscode/blob/main/README.md)

**Marketplace Categories:**

Snippets, AI, Chat

**Tags:**

__web_extension, chat, copilot, debuggers, icon-theme, json, Power Apps, Power Pages, Power Platform, snippet, theme

**Works With:**

All Platforms

---

### 27. [Brackets Keymap](https://marketplace.visualstudio.com/items?itemName=ms-vscode.brackets-keybindings)
*Extension #27 of 329*

Popular Brackets keybindings for VS Code.

**Details:**

- **Extension ID:** `ms-vscode.brackets-keybindings`
- **Version:** 0.1.1
- **Installs:** 207K
- **Rating:** ⭐ 5/5 (4 ratings)
- **Published:** 2020-12-03
- **Last Updated:** 2021-02-26
- **VS Code Engine:** ^1.6.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.brackets-keybindings

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-brackets-keybindings.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-brackets-keybindings/issues)

**Marketplace Categories:**

Keymaps

**Tags:**

__web_extension, brackets, keybindings, keymap

**Works With:**

All Platforms

---

### 28. [Embedded Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-embedded-tools)
*Extension #28 of 329*

View registers and RTOS data when debugging embedded devices.

**Details:**

- **Extension ID:** `ms-vscode.vscode-embedded-tools`
- **Version:** 0.8.251128001
- **Installs:** 155K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2022-03-09
- **Last Updated:** 2025-11-28
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-embedded-tools

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-embedded-tools.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-embedded-tools/issues)
- 💬 [Support](https://developercommunity.visualstudio.com/cpp)
- 🏠 [Homepage](https://github.com/microsoft/vscode-embedded-tools#readme)

**Marketplace Categories:**

Debuggers

**Tags:**

debuggers, embedded, peripherals, registers, RTOS, SVD

**Works With:**

Linux x64, Linux ARM64, Windows ARM64, Linux ARM, Alpine Linux ARM64, Windows x64, macOS Intel, macOS Apple Silicon, Alpine Linux x64, Windows x86

---

### 29. [extension-activator](https://marketplace.visualstudio.com/items?itemName=ms-vscode.extension-activator)
*Extension #29 of 329*

Eagerly activates all other installed extensions.

**Details:**

- **Extension ID:** `ms-vscode.extension-activator`
- **Version:** 0.0.2
- **Installs:** 142K
- **Rating:** ⭐ 1/5 (1 ratings)
- **Published:** 2022-03-10
- **Last Updated:** 2023-02-17
- **VS Code Engine:** ^1.65.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.extension-activator

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-extension-activator.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-extension-activator/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-extension-activator#readme)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 30. [SandDance for VSCode](https://marketplace.visualstudio.com/items?itemName=msrvida.vscode-sanddance)
*Extension #30 of 329*

Visually explore, understand, and present your data.

**Details:**

- **Extension ID:** `msrvida.vscode-sanddance`
- **Version:** 4.2.0
- **Installs:** 138K
- **Rating:** ⭐ 4.7/5 (13 ratings)
- **Published:** 2019-04-18
- **Last Updated:** 2025-06-28
- **VS Code Engine:** ^1.53.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=msrvida.vscode-sanddance

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/Sanddance.git)
- 🐛 [Issues](https://github.com/Microsoft/Sanddance/issues)
- 🏠 [Homepage](https://github.com/Microsoft/Sanddance#readme)

**Marketplace Categories:**

Data Science, Visualization

**Tags:**

__ext_csv, __ext_tsv, csv, data exploration, data visualization, dataviz, tsv

**Works With:**

All Platforms

---

### 31. [Power Query / M Language](https://marketplace.visualstudio.com/items?itemName=PowerQuery.vscode-powerquery)
*Extension #31 of 329*

Language service for the Power Query / M formula language

**Details:**

- **Extension ID:** `PowerQuery.vscode-powerquery`
- **Version:** 0.1.62
- **Installs:** 132K
- **Rating:** ⭐ 4.8/5 (8 ratings)
- **Published:** 2020-06-23
- **Last Updated:** 2025-08-11
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=PowerQuery.vscode-powerquery

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-powerquery.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-powerquery/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-powerquery#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__ext_m, __ext_mout, __ext_pq, __ext_pqm, __ext_pqout, M, powerquery, PowerQueryFormulaLanguage, PowerQueryM, pq

**Works With:**

All Platforms

---

### 32. [anycode-kotlin](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-kotlin)
*Extension #32 of 329*

Kotlin for Anycode

**Details:**

- **Extension ID:** `ms-vscode.anycode-kotlin`
- **Version:** 0.0.8
- **Installs:** 112K
- **Published:** 2022-07-13
- **Last Updated:** 2025-03-28
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-kotlin

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-anycode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-anycode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-anycode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__ext_kt, __web_extension, kotlin

**Works With:**

All Platforms

---

### 33. [anycode-rust](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-rust)
*Extension #33 of 329*

Rust for Anycode

**Details:**

- **Extension ID:** `ms-vscode.anycode-rust`
- **Version:** 0.0.8
- **Installs:** 107K
- **Published:** 2022-03-15
- **Last Updated:** 2025-03-28
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-rust

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-anycode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-anycode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-anycode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension, rust

**Works With:**

All Platforms

---

### 34. [Prompt flow for VS Code](https://marketplace.visualstudio.com/items?itemName=prompt-flow.prompt-flow)
*Extension #34 of 329*

**Details:**

- **Extension ID:** `prompt-flow.prompt-flow`
- **Version:** 1.21.138799792
- **Installs:** 87K
- **Rating:** ⭐ 4.5/5 (4 ratings)
- **Published:** 2023-08-31
- **Last Updated:** 2024-08-26
- **VS Code Engine:** ^1.82.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=prompt-flow.prompt-flow

**Resources:**

- 🐛 [Issues](https://github.com/microsoft/promptflow/issues)

**Marketplace Categories:**

Machine Learning

**Tags:**

__ext_prompty, keybindings, llm, prompt, prompt-flow, prompty

**Works With:**

All Platforms

---

### 35. [JS Debug Value Editor (Experimental)](https://marketplace.visualstudio.com/items?itemName=ms-vscode.debug-value-editor)
*Extension #35 of 329*

This extension allows to edit and view values of a debugee with a full text editor. Currently only supports javascript debug adaptors.

**Details:**

- **Extension ID:** `ms-vscode.debug-value-editor`
- **Version:** 0.2.2
- **Installs:** 74K
- **Published:** 2024-05-03
- **Last Updated:** 2025-07-11
- **VS Code Engine:** ^1.101.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.debug-value-editor

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-debug-value-editor.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-debug-value-editor/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-debug-value-editor#readme)

**Marketplace Categories:**

Debuggers

**Tags:**

debug, debugger, editor, javascript, language-model-tools, tools, typescript

**Works With:**

All Platforms

---

### 36. [Fabric Data Engineering VS Code](https://marketplace.visualstudio.com/items?itemName=SynapseVSCode.synapse)
*Extension #36 of 329*

Microsoft Fabric VS Code experience for Data engineering and Data science of Microsoft Fabric (Previously Synapse VS Code)

**Details:**

- **Extension ID:** `SynapseVSCode.synapse`
- **Version:** 1.17.0
- **Installs:** 70K
- **Rating:** ⭐ 1.8/5 (17 ratings)
- **Published:** 2024-03-21
- **Last Updated:** 2025-12-02
- **VS Code Engine:** ^1.85.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=SynapseVSCode.synapse

**Resources:**

- 📦 [Repository](msdata@vs-ssh.visualstudio.com:v3/msdata/A365/TridentVSCodeExtension)

**Marketplace Categories:**

Data Science, Machine Learning

**Tags:**

Azure, Data Engineering and Data Science, Fabric, jupyter, Machine Learning, python, Spark, Synapse

**Works With:**

All Platforms

---

### 37. [Resharper 9 Keybindings](https://marketplace.visualstudio.com/items?itemName=ms-vscode.resharper9-keybindings)
*Extension #37 of 329*

Port of Resharper 9 Keybindings

**Details:**

- **Extension ID:** `ms-vscode.resharper9-keybindings`
- **Version:** 1.1.0
- **Installs:** 64K
- **Rating:** ⭐ 5/5 (4 ratings)
- **Published:** 2016-03-11
- **Last Updated:** 2016-09-23
- **VS Code Engine:** 0.10.x
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.resharper9-keybindings

**Resources:**

- 📦 [Repository](https://github.com/waderyan/vscode-resharper-keybindings)
- 🐛 [Issues](https://github.com/waderyan/vscode-resharper-keybindings/issues)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, keybindings, keymap

**Works With:**

All Platforms

---

### 38. [DevSkim](https://marketplace.visualstudio.com/items?itemName=MS-CST-E.vscode-devskim)
*Extension #38 of 329*

DevSkim Security Analyzer Plugin for IDEs. Find security mistakes as code is authored, and fix them with a mouse click.

**Details:**

- **Extension ID:** `MS-CST-E.vscode-devskim`
- **Version:** 1.0.68
- **Installs:** 63K
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2017-03-13
- **Last Updated:** 2025-11-19
- **VS Code Engine:** ^1.63.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CST-E.vscode-devskim

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/devskim.git)
- 🐛 [Issues](https://github.com/Microsoft/DevSkim/issues)
- 🏠 [Homepage](https://github.com/Microsoft/DevSkim#readme)

**Marketplace Categories:**

Linters

**Tags:**

bat, c, clojure, coffeescript, cpp, csharp, fsharp, go, groovy, jade, java, javascript, javascriptreact, json, lua, objective-c, perl, perl6, php, plaintext, powershell, python, r, ruby, rust, Security, shellscript, sql, swift, typescript, typescriptreact, vb, xml, yaml

**Works With:**

All Platforms

---

### 39. [anycode-c-sharp](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-c-sharp)
*Extension #39 of 329*

C# for Anycode

**Details:**

- **Extension ID:** `ms-vscode.anycode-c-sharp`
- **Version:** 0.0.8
- **Installs:** 54K
- **Published:** 2022-03-15
- **Last Updated:** 2025-03-28
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-c-sharp

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-anycode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-anycode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-anycode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension, c#

**Works With:**

All Platforms

---

### 40. [Semantic Kernel Tools](https://marketplace.visualstudio.com/items?itemName=ms-semantic-kernel.semantic-kernel)
*Extension #40 of 329*

AI Tools for Semantic Kernel

**Details:**

- **Extension ID:** `ms-semantic-kernel.semantic-kernel`
- **Version:** 0.13.0
- **Installs:** 53K
- **Rating:** ⭐ 5/5 (4 ratings)
- **Published:** 2023-04-11
- **Last Updated:** 2025-05-19
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-semantic-kernel.semantic-kernel

**Resources:**

- 📦 [Repository](https://github.com/microsoft/semantic-kernel.git)
- 🐛 [Issues](https://github.com/microsoft/semantic-kernel/issues)
- 🏠 [Homepage](https://github.com/microsoft/semantic-kernel)

**Marketplace Categories:**

Programming Languages, Data Science, Machine Learning

**Tags:**

debuggers, Semantic Kernel, semantic-kernel.skprompt, SK Prompt, skprompt

**Works With:**

All Platforms

---

### 41. [anycode-php](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-php)
*Extension #41 of 329*

PHP for Anycode

**Details:**

- **Extension ID:** `ms-vscode.anycode-php`
- **Version:** 0.0.8
- **Installs:** 51K
- **Published:** 2022-03-15
- **Last Updated:** 2025-03-28
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-php

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-anycode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-anycode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-anycode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension, php

**Works With:**

All Platforms

---

### 42. [anycode-go](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-go)
*Extension #42 of 329*

Go for Anycode

**Details:**

- **Extension ID:** `ms-vscode.anycode-go`
- **Version:** 0.0.8
- **Installs:** 48K
- **Published:** 2022-03-15
- **Last Updated:** 2025-03-28
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-go

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-anycode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-anycode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-anycode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 43. [Terraform AzApi Provider](https://marketplace.visualstudio.com/items?itemName=azapi-vscode.azapi)
*Extension #43 of 329*

Terraform AzApi Provider language support for Visual Studio Code

**Details:**

- **Extension ID:** `azapi-vscode.azapi`
- **Version:** 2.5.0
- **Installs:** 45K
- **Rating:** ⭐ 5/5 (4 ratings)
- **Published:** 2022-04-11
- **Last Updated:** 2025-07-01
- **VS Code Engine:** ^1.82.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=azapi-vscode.azapi

**Resources:**

- 📦 [Repository](https://github.com/Azure/azapi-vscode.git)
- 🐛 [Issues](https://github.com/Azure/azapi-vscode/issues)
- 🏠 [Homepage](https://github.com/Azure/azapi-vscode#readme)

**Marketplace Categories:**

Programming Languages, Linters, Formatters

**Tags:**

__ext_tf, azurerm, devops, hcl, snippet, terraform

**Works With:**

Linux ARM64, Linux x64, Linux ARM, Windows x64, Windows ARM64, macOS Intel, macOS Apple Silicon, Windows x86

---

### 44. [Adaptive Card Previewer](https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.vscode-adaptive-cards)
*Extension #44 of 329*

A VS Code extension to preview adaptive cards for Microsoft 365 app

**Details:**

- **Extension ID:** `TeamsDevApp.vscode-adaptive-cards`
- **Version:** 0.1.3
- **Installs:** 44K
- **Published:** 2023-11-01
- **Last Updated:** 2024-07-17
- **VS Code Engine:** ^1.80.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.vscode-adaptive-cards

**Resources:**

- 📦 [Repository](https://github.com/OfficeDev/acpreviewer.git)
- 🐛 [Issues](https://github.com/OfficeDev/acpreviewer/issues)
- 🏠 [Homepage](https://github.com/OfficeDev/acpreviewer#readme)

**Marketplace Categories:**

Snippets, Extension Packs, Visualization

**Tags:**

Adaptive Card, adaptive-card-previewer-output, json, keybindings, Microsoft 365

**Works With:**

All Platforms

---

### 45. [Power Query SDK](https://marketplace.visualstudio.com/items?itemName=PowerQuery.vscode-powerquery-sdk)
*Extension #45 of 329*

Power Query Connector SDK

**Details:**

- **Extension ID:** `PowerQuery.vscode-powerquery-sdk`
- **Version:** 0.6.3
- **Installs:** 42K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2022-10-10
- **Last Updated:** 2025-08-11
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=PowerQuery.vscode-powerquery-sdk

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-powerquery-sdk.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-powerquery-sdk/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-powerquery-sdk#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

debuggers, json, powerquery

**Works With:**

Windows x64, Windows ARM64

---

### 46. [SCOPE Studio for VS Code](https://marketplace.visualstudio.com/items?itemName=ms-ssdevteam.scope-vscode-ext)
*Extension #46 of 329*

Author, monitor and run your Cosmos jobs both locally and on cluster

**Details:**

- **Extension ID:** `ms-ssdevteam.scope-vscode-ext`
- **Version:** 1.5.0
- **Installs:** 42K
- **Rating:** ⭐ 5/5 (7 ratings)
- **Published:** 2023-06-02
- **Last Updated:** 2025-11-24
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-ssdevteam.scope-vscode-ext

**Resources:**

- 📦 [Repository](https://github.com/microsoft/scopestudio-vscode.git)
- 🐛 [Issues](https://github.com/microsoft/scopestudio-vscode/issues)
- 🏠 [Homepage](https://github.com/microsoft/scopestudio-vscode#readme)

**Marketplace Categories:**

Programming Languages, Snippets, Debuggers, Formatters, Azure, AI, Chat

**Tags:**

__ext_module, __ext_script, __ext_view, Azure, Azure Data Lake, Azure Data Lake Analytics, Azure Data Lake Storage, Cosmos, debuggers, language-model-tools, SCOPE, snippet, tools

**Works With:**

All Platforms

---

### 47. [GN Language Server](https://marketplace.visualstudio.com/items?itemName=msedge-dev.gnls)
*Extension #47 of 329*

Code IntelliSense for GN build system.

**Details:**

- **Extension ID:** `msedge-dev.gnls`
- **Version:** 0.1.4
- **Installs:** 38K
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2021-07-21
- **Last Updated:** 2023-04-16
- **VS Code Engine:** ^1.77.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=msedge-dev.gnls

**Resources:**

- 📦 [Repository](https://github.com/microsoft/gnls.git)
- 🐛 [Issues](https://github.com/microsoft/gnls/issues)
- 🏠 [Homepage](https://github.com/microsoft/gnls)

**Marketplace Categories:**

Programming Languages

**Tags:**

__ext_gn, __ext_gni, chromium, gn

**Works With:**

All Platforms

---

### 48. [Microsoft Fabric](https://marketplace.visualstudio.com/items?itemName=fabric.vscode-fabric)
*Extension #48 of 329*

View, manage, and develop your Microsoft Fabric items directly in VS Code.

**Details:**

- **Extension ID:** `fabric.vscode-fabric`
- **Version:** 0.35.6
- **Installs:** 37K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2024-10-02
- **Last Updated:** 2025-11-25
- **VS Code Engine:** ^1.77.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=fabric.vscode-fabric

**Resources:**

- 📦 [Repository](https://github.com/microsoft/microsoft-fabric-vscode-extension.git)
- 🐛 [Issues](https://aka.ms/fabric/vscode/issues)
- 🏠 [Homepage](https://github.com/microsoft/microsoft-fabric-vscode-extension#readme)

**Marketplace Categories:**

Data Science

**Tags:**

Data Engineering, Data Science, Fabric, Functions, Python

**Works With:**

All Platforms

---

### 49. [TMDL](https://marketplace.visualstudio.com/items?itemName=analysis-services.TMDL)
*Extension #49 of 329*

Language support for the Tabular Model Definition Language (TMDL).

**Details:**

- **Extension ID:** `analysis-services.TMDL`
- **Version:** 1.6.1
- **Installs:** 33K
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2023-10-10
- **Last Updated:** 2025-11-21
- **VS Code Engine:** ^1.84.2
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=analysis-services.TMDL

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-tmdl.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-tmdl/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-tmdl#readme)

**Marketplace Categories:**

Other

**Tags:**

__ext_tmd, __ext_tmdl, __web_extension, Tabular Model Definition Language, tmdl

**Works With:**

All Platforms

---

### 50. [Security IntelliSense](https://marketplace.visualstudio.com/items?itemName=azsdktm.SecurityIntelliSense)
*Extension #50 of 329*

Provides quick and inline security suggestion and fixes for C# and XML source code

**Details:**

- **Extension ID:** `azsdktm.SecurityIntelliSense`
- **Version:** 0.8.0
- **Installs:** 28K
- **Published:** 2019-09-10
- **Last Updated:** 2019-09-16
- **VS Code Engine:** ^1.12.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=azsdktm.SecurityIntelliSense

**Marketplace Categories:**

Other

**Tags:**

.NET, .NET Core, Autocomplete, Autocompletion, Azure, C#, CSharp, IntelliSense, Security, xml

**Works With:**

All Platforms

---

### 51. [Mock Debug](https://marketplace.visualstudio.com/items?itemName=ms-vscode.mock-debug)
*Extension #51 of 329*

Starter extension for developing debug adapters for VS Code.

**Details:**

- **Extension ID:** `ms-vscode.mock-debug`
- **Version:** 0.52.0
- **Installs:** 27K
- **Published:** 2015-11-16
- **Last Updated:** 2023-01-20
- **VS Code Engine:** ^1.66.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.mock-debug

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-mock-debug.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-mock-debug/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-mock-debug#readme)

**Marketplace Categories:**

Debuggers

**Tags:**

__web_extension, debuggers, multi-root ready

**Works With:**

All Platforms

---

### 52. [DTDL](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.vscode-dtdl)
*Extension #52 of 329*

This extension provides syntax highlighting to read and edit JSON documents using the Digital Twins Definition Language

**Details:**

- **Extension ID:** `vsciot-vscode.vscode-dtdl`
- **Version:** 1.4.1
- **Installs:** 25K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2020-06-07
- **Last Updated:** 2023-07-13
- **VS Code Engine:** ^1.63.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.vscode-dtdl

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-dtdl.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-dtdl/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-dtdl/blob/main/README.md)

**Marketplace Categories:**

Snippets, Azure

**Tags:**

colorized-channel, json, snippet

**Works With:**

All Platforms

---

### 53. [VS Code Commander](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-commander)
*Extension #53 of 329*

Enables users to configure VS Code using chat. The extension allows to search for settings and commands and reason about them. The settings values can easily be updated and commands can be run.

**Details:**

- **Extension ID:** `ms-vscode.vscode-commander`
- **Version:** 0.2.0
- **Installs:** 22K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2024-10-15
- **Last Updated:** 2024-10-30
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-commander

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-commander.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-commander/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-commander#readme)

**Marketplace Categories:**

AI, Chat

**Tags:**

chat-participant, github-copilot

**Works With:**

All Platforms

---

### 54. [Microsoft MakeCode Arcade](https://marketplace.visualstudio.com/items?itemName=ms-edu.pxt-vscode-web)
*Extension #54 of 329*

Make games and learn code. Create retro arcade style video games, art, and music inside VS Code.

**Details:**

- **Extension ID:** `ms-edu.pxt-vscode-web`
- **Version:** 1.0.8
- **Installs:** 16K
- **Rating:** ⭐ 5/5 (5 ratings)
- **Published:** 2023-01-05
- **Last Updated:** 2025-03-07
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-edu.pxt-vscode-web

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-makecode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-makecode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-makecode#readme)

**Marketplace Categories:**

Education

**Tags:**

__ext_blocks, __ext_gjres, __ext_jres, __web_extension, arcade, education, html, json, learn, makecode, student

**Works With:**

All Platforms

---

### 55. [Extension Manifest Editor](https://marketplace.visualstudio.com/items?itemName=ms-devlabs.extension-manifest-editor)
*Extension #55 of 329*

Preview how your extension appears on the Marketplace.

**Details:**

- **Extension ID:** `ms-devlabs.extension-manifest-editor`
- **Version:** 0.1.5
- **Installs:** 15K
- **Rating:** ⭐ 2.9/5 (13 ratings)
- **Published:** 2016-08-08
- **Last Updated:** 2017-09-13
- **VS Code Engine:** ^1.0.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-devlabs.extension-manifest-editor

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/extension-manifest-editor)
- 🐛 [Issues](https://github.com/Microsoft/extension-manifest-editor/issues)

**Marketplace Categories:**

Other

**Tags:**

keybindings

**Works With:**

All Platforms

---

### 56. [Dev Box](https://marketplace.visualstudio.com/items?itemName=DevCenter.ms-devbox)
*Extension #56 of 329*

An extension for connecting to your Dev Box and creating customization files.

**Details:**

- **Extension ID:** `DevCenter.ms-devbox`
- **Version:** 2.2.0
- **Installs:** 11K
- **Rating:** ⭐ 4.2/5 (5 ratings)
- **Published:** 2024-02-20
- **Last Updated:** 2025-11-26
- **VS Code Engine:** ^1.101.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=DevCenter.ms-devbox

**Marketplace Categories:**

Linters, AI, Chat

**Tags:**

chat-participant, dev box, dev box mcp, devbox, devbox mcp, keybindings, language-model-tools, mcp, microsoft, microsoft dev box, tools, yaml

**Works With:**

All Platforms

---

### 57. [SharePoint Embedded](https://marketplace.visualstudio.com/items?itemName=SharepointEmbedded.ms-sharepoint-embedded-vscode-extension)
*Extension #57 of 329*

Build SharePoint Embedded apps

**Details:**

- **Extension ID:** `SharepointEmbedded.ms-sharepoint-embedded-vscode-extension`
- **Version:** 1.0.1
- **Installs:** 11K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2024-01-12
- **Last Updated:** 2025-10-28
- **VS Code Engine:** ^1.81.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=SharepointEmbedded.ms-sharepoint-embedded-vscode-extension

**Resources:**

- 📦 [Repository](https://github.com/microsoft/SharePoint-Embedded-VS-Code-Extension.git)
- 🐛 [Issues](https://github.com/microsoft/SharePoint-Embedded-VS-Code-Extension/issues)
- 🏠 [Homepage](https://github.com/microsoft/SharePoint-Embedded-VS-Code-Extension#readme)

**Marketplace Categories:**

Snippets, Extension Packs, Azure

**Works With:**

All Platforms

---

### 58. [Power BI Modeling MCP Server](https://marketplace.visualstudio.com/items?itemName=analysis-services.powerbi-modeling-mcp)
*Extension #58 of 329*

The Power BI Modeling MCP Server, brings Power BI semantic modeling capabilities to your agents.

**Details:**

- **Extension ID:** `analysis-services.powerbi-modeling-mcp`
- **Version:** 0.1.9
- **Installs:** 8K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2025-11-18
- **Last Updated:** 2025-11-21
- **VS Code Engine:** ^1.80.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=analysis-services.powerbi-modeling-mcp

**Resources:**

- 📦 [Repository](https://github.com/microsoft/powerbi-modeling-mcp.git)
- 🐛 [Issues](https://github.com/microsoft/powerbi-modeling-mcp/issues)
- 🏠 [Homepage](https://github.com/microsoft/powerbi-modeling-mcp#readme)

**Marketplace Categories:**

AI, Chat

**Works With:**

Windows x64, Windows ARM64

---

### 59. [POML](https://marketplace.visualstudio.com/items?itemName=poml-team.poml)
*Extension #59 of 329*

Prompt Orchestration Markup Language

**Details:**

- **Extension ID:** `poml-team.poml`
- **Version:** 0.0.8
- **Installs:** 6K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2025-06-10
- **Last Updated:** 2025-08-25
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=poml-team.poml

**Resources:**

- 📦 [Repository](https://github.com/microsoft/poml.git)
- 🐛 [Issues](https://github.com/microsoft/poml/issues)
- 🏠 [Homepage](https://github.com/microsoft/poml#readme)

**Marketplace Categories:**

Other

**Tags:**

__ext_poml, chat-participant, poml

**Works With:**

Windows x64, Linux x64, macOS Apple Silicon, macOS Intel, Linux ARM64

---

### 60. [Power Platform Tools [PREVIEW]](https://marketplace.visualstudio.com/items?itemName=microsoft-IsvExpTools.powerplatform-vscode-preview)
*Extension #60 of 329*

Tooling to create Power Platform solutions & packages, manage Power Platform environments and edit Power Apps Portals

**Details:**

- **Extension ID:** `microsoft-IsvExpTools.powerplatform-vscode-preview`
- **Version:** 2.0.42
- **Installs:** 6K
- **Published:** 2022-11-04
- **Last Updated:** 2024-04-16
- **VS Code Engine:** ^1.73.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=microsoft-IsvExpTools.powerplatform-vscode-preview

**Resources:**

- 📦 [Repository](https://github.com/microsoft/powerplatform-vscode.git)
- 🐛 [Issues](https://github.com/microsoft/powerplatform-vscode/issues)
- 🏠 [Homepage](https://github.com/microsoft/powerplatform-vscode/blob/main/README.md)

**Marketplace Categories:**

Snippets

**Tags:**

__web_extension, debuggers, icon-theme, Power Platform, snippet, theme

**Works With:**

All Platforms

---

### 61. [Fabric User data functions](https://marketplace.visualstudio.com/items?itemName=fabric.vscode-fabric-functions)
*Extension #61 of 329*

View, manage, and develop your Microsoft Fabric items directly in VS Code.

**Details:**

- **Extension ID:** `fabric.vscode-fabric-functions`
- **Version:** 0.31.41
- **Installs:** 5K
- **Rating:** ⭐ 5/5 (4 ratings)
- **Published:** 2025-01-23
- **Last Updated:** 2025-12-02
- **VS Code Engine:** ^1.77.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=fabric.vscode-fabric-functions

**Resources:**

- 📦 [Repository](https://github.com/microsoft/microsoft-fabric-vscode-extension.git)
- 🐛 [Issues](https://aka.ms/fabric/vscode/issues)
- 🏠 [Homepage](https://github.com/microsoft/microsoft-fabric-vscode-extension#readme)

**Marketplace Categories:**

Data Science

**Tags:**

Data Engineering, Data Science, Fabric, Functions, Python

**Works With:**

All Platforms

---

### 62. [TS/JS Relative File Path Support](https://marketplace.visualstudio.com/items?itemName=ms-vscode.ts-file-path-support)
*Extension #62 of 329*

Provides support for relative file paths in TypeScript and JavaScript. This includes auto-complete, go to definition, rename and inlining/extraction.

**Details:**

- **Extension ID:** `ms-vscode.ts-file-path-support`
- **Version:** 1.0.0
- **Installs:** 4K
- **Published:** 2024-07-01
- **Last Updated:** 2024-11-04
- **VS Code Engine:** ^1.90.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.ts-file-path-support

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-ts-file-path-support.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-ts-file-path-support/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-ts-file-path-support#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

file, javascript, javascriptreact, path, relative, typescript, typescriptreact

**Works With:**

All Platforms

---

### 63. [Aspire](https://marketplace.visualstudio.com/items?itemName=microsoft-aspire.aspire-vscode)
*Extension #63 of 329*

Official Aspire extension for Visual Studio Code

**Details:**

- **Extension ID:** `microsoft-aspire.aspire-vscode`
- **Version:** 1.0.2
- **Installs:** 3K
- **Published:** 2025-09-25
- **Last Updated:** 2025-11-12
- **VS Code Engine:** ^1.98.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=microsoft-aspire.aspire-vscode

**Resources:**

- 📦 [Repository](https://github.com/dotnet/aspire.git)
- 🐛 [Issues](https://github.com/dotnet/aspire/issues)
- 🏠 [Homepage](https://github.com/dotnet/aspire#readme)

**Marketplace Categories:**

Programming Languages, Debuggers

**Tags:**

apphost, debuggers, dotnet, orchestrator

**Works With:**

All Platforms

---

### 64. [Browse Published Extensions](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-marketplace-extension-browser)
*Extension #64 of 329*

Browse the content of extensions published in the VS Code Marketplace

**Details:**

- **Extension ID:** `ms-vscode.vscode-marketplace-extension-browser`
- **Version:** 0.0.7
- **Installs:** 3K
- **Published:** 2021-07-15
- **Last Updated:** 2022-12-23
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-marketplace-extension-browser

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-marketplace-extension-browser.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-marketplace-extension-browser/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-marketplace-extension-browser#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 65. [Mezzurite](https://marketplace.visualstudio.com/items?itemName=mezzurite-devs.mezzurite)
*Extension #65 of 329*

Instrument your application with Mezzurite SDK quickly and easily with this extension.

**Details:**

- **Extension ID:** `mezzurite-devs.mezzurite`
- **Version:** 2.0.1
- **Installs:** 3K
- **Published:** 2018-11-28
- **Last Updated:** 2019-01-15
- **VS Code Engine:** ^1.28.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=mezzurite-devs.mezzurite

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-mezzurite)
- 🐛 [Issues](https://github.com/Microsoft/vscode-mezzurite/issues)
- 💬 [Support](https://github.com/Microsoft/vscode-mezzurite)

**Marketplace Categories:**

Snippets, Linters

**Tags:**

Angular, Microsoft, React, Single Page Application, SPA

**Works With:**

All Platforms

---

### 66. [VS Code Diagnostic Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-diagnostic-tools)
*Extension #66 of 329*

A collection of tools designed to aid debugging and developing VS Code

**Details:**

- **Extension ID:** `ms-vscode.vscode-diagnostic-tools`
- **Version:** 1.2.0
- **Installs:** 3K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2023-08-31
- **Last Updated:** 2024-07-16
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-diagnostic-tools

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-diagnostic-tools.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-diagnostic-tools/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-diagnostic-tools#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 67. [Microsoft Sentinel](https://marketplace.visualstudio.com/items?itemName=ms-security.ms-sentinel)
*Extension #67 of 329*

Build solutions for Microsoft Security products

**Details:**

- **Extension ID:** `ms-security.ms-sentinel`
- **Version:** 1.1.1
- **Installs:** 3K
- **Published:** 2025-07-18
- **Last Updated:** 2025-12-01
- **VS Code Engine:** ^1.96.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-security.ms-sentinel

**Resources:**

- 📦 [Repository](msdata@vs-ssh.visualstudio.com:v3/msdata/Babylon/vscode-security)

**Marketplace Categories:**

Data Science, Notebooks

**Tags:**

job, jupyter, lake, microsoft, notebook, pyspark, python, security, sentinel, spark, table

**Works With:**

All Platforms

---

### 68. [Application Insights Code Optimizations](https://marketplace.visualstudio.com/items?itemName=ms-codeoptimizations.code-optimizations-preview)
*Extension #68 of 329*

Code Optimizations helps improve code level performance for .NET apps. GitHub Copilot required.

**Details:**

- **Extension ID:** `ms-codeoptimizations.code-optimizations-preview`
- **Version:** 1.1.1
- **Installs:** 2K
- **Published:** 2024-10-28
- **Last Updated:** 2025-05-02
- **VS Code Engine:** ^1.93.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-codeoptimizations.code-optimizations-preview

**Resources:**

- 📦 [Repository](https://devdiv.visualstudio.com/OnlineServices/_git/PerfChatAgent)

**Marketplace Categories:**

Azure, Testing

**Tags:**

Application Insights, chat-participant, github-copilot, Optimization, Performance, Profiler, Trace analysis

**Works With:**

All Platforms

---

### 69. [Atlas Design System Tools](https://marketplace.visualstudio.com/items?itemName=docsmsft.atlas-design-system-tools)
*Extension #69 of 329*

VSCode tooling for using the Atlas Design System.

**Details:**

- **Extension ID:** `docsmsft.atlas-design-system-tools`
- **Version:** 0.0.4
- **Installs:** 1K
- **Published:** 2022-09-21
- **Last Updated:** 2022-09-22
- **VS Code Engine:** ^1.71.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=docsmsft.atlas-design-system-tools

**Resources:**

- 📦 [Repository](https://github.com/microsoft/atlas-design.git)
- 🐛 [Issues](https://github.com/microsoft/atlas-design/issues)
- 🏠 [Homepage](https://github.com/microsoft/atlas-design#readme)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 70. [PSRule](https://marketplace.visualstudio.com/items?itemName=ps-rule.vscode-ps-rule)
*Extension #70 of 329*

Validate infrastructure as code (IaC) and DevOps repositories using PSRule.

**Details:**

- **Extension ID:** `ps-rule.vscode-ps-rule`
- **Version:** 2025.3.32
- **Installs:** 781
- **Published:** 2024-11-27
- **Last Updated:** 2025-03-18
- **VS Code Engine:** ^1.98.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ps-rule.vscode-ps-rule

**Resources:**

- 📦 [Repository](https://github.com/microsoft/PSRule.git)
- 🐛 [Issues](https://github.com/microsoft/PSRule/issues)
- 🏠 [Homepage](https://aka.ms/ps-rule)

**Marketplace Categories:**

Programming Languages, Snippets

**Tags:**

json, PowerShell, snippet, yaml

**Works With:**

All Platforms

---

### 71. [Touch Adaptation Kit Editor](https://marketplace.visualstudio.com/items?itemName=xbox-tools.vscode-xbox-input-editor-extension)
*Extension #71 of 329*

Create and edit Touch Adaptation Kit bundles and layouts.

**Details:**

- **Extension ID:** `xbox-tools.vscode-xbox-input-editor-extension`
- **Version:** 0.200.0
- **Installs:** 713
- **Published:** 2024-03-26
- **Last Updated:** 2025-03-07
- **VS Code Engine:** ^1.79.1
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=xbox-tools.vscode-xbox-input-editor-extension

**Resources:**

- 🐛 [Issues](https://github.com/microsoft/xbox-game-streaming-tools/issues)

**Marketplace Categories:**

Linters, Visualization

**Tags:**

Game Development, Gaming, Input, TAK, TAK Editor, Touch, Touch Adaptation Kit, Xbox

**Works With:**

All Platforms

---

### 72. [Microsoft Fabric MCP Server](https://marketplace.visualstudio.com/items?itemName=fabric.vscode-fabric-mcp-server)
*Extension #72 of 329*

Provides Model Context Protocol (MCP) integration and tooling for Fabric in Visual Studio Code.

**Details:**

- **Extension ID:** `fabric.vscode-fabric-mcp-server`
- **Version:** 0.0.3
- **Installs:** 379
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2025-11-21
- **Last Updated:** 2025-12-04
- **VS Code Engine:** ^1.101.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=fabric.vscode-fabric-mcp-server

**Resources:**

- 📦 [Repository](https://github.com/microsoft/mcp.git)
- 🐛 [Issues](https://github.com/microsoft/mcp/issues)
- 🏠 [Homepage](https://github.com/microsoft/mcp#readme)

**Marketplace Categories:**

AI, Chat

**Tags:**

agent, ai, chat, copilot, fabric, language-model-tools, mcp, model context protocol

**Works With:**

Linux ARM64, Windows x64, Windows ARM64, macOS Intel, macOS Apple Silicon, Linux x64

---

### 73. [DebugMCP](https://marketplace.visualstudio.com/items?itemName=ozzafar.debugmcpextension)
*Extension #73 of 329*

A VSCode extension that provides comprehensive multi-language debugging capabilities and automatically exposes itself as an MCP (Model Context Protocol) server for seamless integration with AI assistants.

**Details:**

- **Extension ID:** `ozzafar.debugmcpextension`
- **Version:** 1.0.2
- **Installs:** 246
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2025-11-05
- **Last Updated:** 2025-11-23
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ozzafar.debugmcpextension

**Resources:**

- 📦 [Repository](https://github.com/microsoft/DebugMCP.git)
- 🐛 [Issues](https://github.com/microsoft/DebugMCP/issues)
- 🏠 [Homepage](https://github.com/microsoft/DebugMCP)

**Marketplace Categories:**

Debuggers, AI

**Tags:**

ai, debug, debugging, mcp, microsoft, model context protocol, vscode-extension

**Works With:**

All Platforms

---

### 74. [TS/JS VS Code Customized Language Service](https://marketplace.visualstudio.com/items?itemName=ms-vscode.ts-customized-language-service)
*Extension #74 of 329*

Customized features/improvements/fixes.

**Details:**

- **Extension ID:** `ms-vscode.ts-customized-language-service`
- **Version:** 1.1.2025110301
- **Installs:** 157
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2025-09-09
- **Last Updated:** 2025-11-03
- **VS Code Engine:** ^1.90.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.ts-customized-language-service

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-ts-customized-language-service.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-ts-customized-language-service/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-ts-customized-language-service#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

javascript, javascriptreact, typescript, typescriptreact

**Works With:**

All Platforms

---

### 75. [Chartifact](https://marketplace.visualstudio.com/items?itemName=msrvida.chartifact)
*Extension #75 of 329*

Declarative, interactive data documents

**Details:**

- **Extension ID:** `msrvida.chartifact`
- **Version:** 1.0.9
- **Installs:** 114
- **Published:** 2025-08-31
- **Last Updated:** 2025-11-21
- **VS Code Engine:** ^1.101.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=msrvida.chartifact

**Resources:**

- 📦 [Repository](https://github.com/microsoft/chartifact.git)
- 🐛 [Issues](https://github.com/microsoft/chartifact/issues)
- 🏠 [Homepage](https://github.com/microsoft/chartifact#readme)

**Marketplace Categories:**

Visualization

**Tags:**

__web_extension, data visualization, dataviz

**Works With:**

All Platforms

---

### 76. [Kanagawa Language Support](https://marketplace.visualstudio.com/items?itemName=microsoft-kanagawa.kanagawalang)
*Extension #76 of 329*

Syntax highlighting for the Kanagawa hardware description language

**Details:**

- **Extension ID:** `microsoft-kanagawa.kanagawalang`
- **Version:** 0.1.8
- **Installs:** 27
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2025-10-27
- **Last Updated:** 2025-11-18
- **VS Code Engine:** ^1.70.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=microsoft-kanagawa.kanagawalang

**Resources:**

- 📦 [Repository](https://github.com/microsoft/kanagawa.git)
- 🐛 [Issues](https://github.com/microsoft/kanagawa/issues)
- 🏠 [Homepage](https://github.com/microsoft/kanagawa#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__ext_k, __web_extension, kanagawa

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Azure & Cloud Services

**Category Stats:** 65 extensions | Total Installs: 123.45M | Avg Rating: 3.9⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [Docker](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-docker)
*Extension #77 of 329*

Makes it easy to create, manage, and debug containerized applications.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-docker`
- **Version:** 2.0.0
- **Installs:** 48.67M
- **Rating:** ⭐ 4.5/5 (106 ratings)
- **Published:** 2015-11-13
- **Last Updated:** 2025-05-28
- **VS Code Engine:** ^1.92.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-docker

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-docker.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-docker/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-docker/blob/main/README.md)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, compose, container, containers, docker

**Works With:**

All Platforms

---

### 2. [Azure Account](https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-account)
*Extension #78 of 329*

A common Sign In and Subscription management extension for VS Code.

**Details:**

- **Extension ID:** `ms-vscode.azure-account`
- **Version:** 0.13.0
- **Installs:** 9.08M
- **Rating:** ⭐ 2.1/5 (56 ratings)
- **Published:** 2017-09-06
- **Last Updated:** 2025-01-22
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-account

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-azure-account.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-azure-account/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azure-account/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

azure

**Works With:**

All Platforms

---

### 3. [SQL Server (mssql)](https://marketplace.visualstudio.com/items?itemName=ms-mssql.mssql)
*Extension #79 of 329*

Design and optimize schemas for SQL Server, Azure SQL, and SQL Database in Fabric using a modern, lightweight extension built for developers

**Details:**

- **Extension ID:** `ms-mssql.mssql`
- **Version:** 1.37.1
- **Installs:** 8.56M
- **Rating:** ⭐ 3.1/5 (171 ratings)
- **Published:** 2016-11-16
- **Last Updated:** 2025-12-02
- **VS Code Engine:** ^1.98.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-mssql.mssql

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-mssql.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-mssql/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-mssql/blob/master/README.md)

**Marketplace Categories:**

Programming Languages, Azure

**Tags:**

__ext_sql, __ext_sqlplan, Azure SQL Database, chat-participant, keybindings, MSSQL, multi-root ready, snippet, SQL, SQL database in Fabric, SQL Server, SQL Server Execution Plan, sqlplan

**Works With:**

All Platforms

---

### 4. [Container Tools](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-containers)
*Extension #80 of 329*

Makes it easy to create, manage, and debug containerized applications.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-containers`
- **Version:** 2.3.0
- **Installs:** 6.94M
- **Rating:** ⭐ 3.6/5 (17 ratings)
- **Published:** 2025-04-21
- **Last Updated:** 2025-11-10
- **VS Code Engine:** ^1.105.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-containers

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-containers.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-containers/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-containers/blob/main/README.md)

**Marketplace Categories:**

Programming Languages, Linters, Azure

**Tags:**

compose, container, containers, debuggers, docker, dockercompose, dockerfile, ignore, multi-root ready, podman

**Works With:**

All Platforms

---

### 5. [Azure Resources](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureresourcegroups)
*Extension #81 of 329*

An extension for viewing and managing Azure resources.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azureresourcegroups`
- **Version:** 0.11.7
- **Installs:** 5.82M
- **Rating:** ⭐ 2.5/5 (6 ratings)
- **Published:** 2020-03-17
- **Last Updated:** 2025-11-18
- **VS Code Engine:** ^1.105.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureresourcegroups

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-azureresourcegroups.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-azureresourcegroups/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azureresourcegroups/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

__web_extension, Azure, keybindings, multi-root ready, Resource Groups

**Works With:**

Web

---

### 6. [Azure Functions](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurefunctions)
*Extension #82 of 329*

An Azure Functions extension for Visual Studio Code.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azurefunctions`
- **Version:** 1.20.2
- **Installs:** 5.57M
- **Rating:** ⭐ 3.2/5 (34 ratings)
- **Published:** 2017-10-19
- **Last Updated:** 2025-11-25
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurefunctions

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-azurefunctions.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-azurefunctions/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azurefunctions/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

Azure, Functions, json, multi-root ready, serverless

**Works With:**

All Platforms

---

### 7. [Azure Repos](https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-repos)
*Extension #83 of 329*

Remotely browse and edit any Azure Repos

**Details:**

- **Extension ID:** `ms-vscode.azure-repos`
- **Version:** 0.41.2025081801
- **Installs:** 4.7M
- **Rating:** ⭐ 3.2/5 (4 ratings)
- **Published:** 2022-01-31
- **Last Updated:** 2025-08-18
- **VS Code Engine:** ^1.78.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-repos

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-remote-repositories-github.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-remote-repositories-github/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-remote-repositories-github#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 8. [SQL Bindings](https://marketplace.visualstudio.com/items?itemName=ms-mssql.sql-bindings-vscode)
*Extension #84 of 329*

Enables users to develop and publish Azure Functions with Azure SQL bindings

**Details:**

- **Extension ID:** `ms-mssql.sql-bindings-vscode`
- **Version:** 0.4.1
- **Installs:** 4.21M
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2022-04-20
- **Last Updated:** 2025-04-30
- **VS Code Engine:** ^1.30.1
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-mssql.sql-bindings-vscode

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/azuredatastudio.git)
- 🐛 [Issues](https://github.com/Microsoft/azuredatastudio/issues)
- 🏠 [Homepage](https://github.com/Microsoft/azuredatastudio#readme)

**Marketplace Categories:**

Other

**Tags:**

sql

**Works With:**

All Platforms

---

### 9. [Azure Cosmos DB](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-cosmosdb)
*Extension #85 of 329*

Connect Azure CosmosDB databases in Azure, inspect and edit your data and run powerful queries with the visual query editor.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-cosmosdb`
- **Version:** 0.31.0
- **Installs:** 3.15M
- **Rating:** ⭐ 3.2/5 (41 ratings)
- **Published:** 2017-09-19
- **Last Updated:** 2025-11-17
- **VS Code Engine:** ^1.94.2
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-cosmosdb

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-cosmosdb.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-cosmosdb/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-cosmosdb/blob/main/README.md)

**Marketplace Categories:**

Azure, Data Science

**Tags:**

__ext_nosql, Cosmos DB, CosmosNoSQL, Database, keybindings, nosql

**Works With:**

All Platforms

---

### 10. [Azure App Service](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureappservice)
*Extension #86 of 329*

An Azure App Service management extension for Visual Studio Code.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azureappservice`
- **Version:** 0.26.4
- **Installs:** 2.87M
- **Rating:** ⭐ 3.4/5 (31 ratings)
- **Published:** 2017-09-19
- **Last Updated:** 2025-09-25
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureappservice

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-azureappservice.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-azureappservice/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azureappservice/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

App Service, Azure, multi-root ready

**Works With:**

All Platforms

---

### 11. [Azure Storage](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurestorage)
*Extension #87 of 329*

Manage your Azure Storage accounts including Blob Containers, File Shares, Tables and Queues

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azurestorage`
- **Version:** 0.17.1
- **Installs:** 2.42M
- **Rating:** ⭐ 3/5 (18 ratings)
- **Published:** 2017-12-12
- **Last Updated:** 2025-09-26
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurestorage

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-azurestorage.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-azurestorage/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-azurestorage/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

__web_extension, Azure, Blob Containers, File Share, Static website, Storage Account

**Works With:**

All Platforms

---

### 12. [Azure CLI Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.azurecli)
*Extension #88 of 329*

Tools for developing and running commands of the Azure CLI.

**Details:**

- **Extension ID:** `ms-vscode.azurecli`
- **Version:** 0.6.0
- **Installs:** 2.28M
- **Rating:** ⭐ 3.2/5 (19 ratings)
- **Published:** 2017-05-09
- **Last Updated:** 2024-01-19
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.azurecli

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-azurecli.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-azurecli/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azurecli#readme)

**Marketplace Categories:**

Programming Languages, Snippets, Azure

**Tags:**

__ext_azcli, azcli, Azure CLI Scrapbook, keybindings

**Works With:**

All Platforms

---

### 13. [Azure Virtual Machines](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurevirtualmachines)
*Extension #89 of 329*

An Azure Virtual Machines extension for Visual Studio Code.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azurevirtualmachines`
- **Version:** 0.6.10
- **Installs:** 2.01M
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2020-03-05
- **Last Updated:** 2025-09-25
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurevirtualmachines

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-azurevirtualmachines.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-azurevirtualmachines/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azurevirtualmachines/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

Azure, multi-root ready, Remote, SSH, Virtual Machines, VMs

**Works With:**

All Platforms

---

### 14. [Azure Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-node-azure-pack)
*Extension #90 of 329*

Build, deploy, and manage Azure applications with support from Copilot all without leaving VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-node-azure-pack`
- **Version:** 1.8.0
- **Installs:** 2.01M
- **Rating:** ⭐ 2.9/5 (19 ratings)
- **Published:** 2017-11-02
- **Last Updated:** 2025-11-04
- **VS Code Engine:** ^1.92.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-node-azure-pack

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-node-azure-pack.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-node-azure-pack/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-node-azure-pack)

**Marketplace Categories:**

Extension Packs, Azure

**Tags:**

__web_extension, azure, cloud, hosting

**Works With:**

All Platforms

---

### 15. [Azure Static Web Apps](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurestaticwebapps)
*Extension #91 of 329*

An Azure Static Web Apps extension for Visual Studio Code.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azurestaticwebapps`
- **Version:** 0.13.2
- **Installs:** 1.86M
- **Rating:** ⭐ 3/5 (10 ratings)
- **Published:** 2020-05-18
- **Last Updated:** 2025-09-25
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurestaticwebapps

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-azurestaticwebapps.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-azurestaticwebapps/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azurestaticwebapps/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

__web_extension, Azure, debuggers, json, multi-root ready

**Works With:**

All Platforms

---

### 16. [Azure Developer CLI](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.azure-dev)
*Extension #92 of 329*

Makes it easy to run, provision, and deploy Azure applications using the Azure Developer CLI

**Details:**

- **Extension ID:** `ms-azuretools.azure-dev`
- **Version:** 0.10.0
- **Installs:** 1.7M
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2022-07-11
- **Last Updated:** 2025-09-22
- **VS Code Engine:** ^1.90.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.azure-dev

**Resources:**

- 📦 [Repository](https://github.com/azure/azure-dev.git)
- 🐛 [Issues](https://github.com/Azure/azure-dev/issues)
- 🏠 [Homepage](https://github.com/azure/azure-dev#readme)

**Marketplace Categories:**

Azure

**Tags:**

azd, azure

**Works With:**

All Platforms

---

### 17. [Azure Container Apps](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurecontainerapps)
*Extension #93 of 329*

An Azure Container Apps extension for Visual Studio Code.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azurecontainerapps`
- **Version:** 0.10.0
- **Installs:** 1.48M
- **Published:** 2022-04-12
- **Last Updated:** 2025-11-18
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurecontainerapps

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-azurecontainerapps.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-azurecontainerapps/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-azurecontainerapps/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

__web_extension, Azure, json, multi-root ready

**Works With:**

All Platforms

---

### 18. [Azure Resource Manager (ARM) Tools](https://marketplace.visualstudio.com/items?itemName=msazurermtools.azurerm-vscode-tools)
*Extension #94 of 329*

Language server, editing tools and snippets for Azure Resource Manager (ARM) template files.

**Details:**

- **Extension ID:** `msazurermtools.azurerm-vscode-tools`
- **Version:** 0.15.15
- **Installs:** 1.47M
- **Rating:** ⭐ 4.2/5 (26 ratings)
- **Published:** 2016-02-29
- **Last Updated:** 2025-04-10
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=msazurermtools.azurerm-vscode-tools

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-azurearmtools.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-azurearmtools/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azurearmtools/blob/main/README.md)

**Marketplace Categories:**

Programming Languages, Snippets, Linters, Formatters, Azure

**Tags:**

ARM, arm-template, Azure, Azure Resource Manager Template, Azure Template, json, jsonc, Resource Manager, Snippets

**Works With:**

All Platforms

---

### 19. [Bicep](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-bicep)
*Extension #95 of 329*

Bicep language support for Visual Studio Code

**Details:**

- **Extension ID:** `ms-azuretools.vscode-bicep`
- **Version:** 0.39.26
- **Installs:** 1.38M
- **Rating:** ⭐ 5/5 (27 ratings)
- **Published:** 2020-11-11
- **Last Updated:** 2025-11-17
- **VS Code Engine:** ^1.101.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-bicep

**Resources:**

- 📦 [Repository](https://github.com/Azure/bicep.git)
- 🐛 [Issues](https://github.com/Azure/bicep/issues)
- 🏠 [Homepage](https://github.com/Azure/bicep/blob/main/README.md)

**Marketplace Categories:**

Programming Languages, Azure

**Tags:**

__ext_bicep, __ext_bicepparam, ARM Template, Azure, Azure Resource Manager, Bicep, Bicep Parameters, Bicep Params, bicep-params, json, jsonc, keybindings, snippet

**Works With:**

All Platforms

---

### 20. [Azure Pipelines](https://marketplace.visualstudio.com/items?itemName=ms-azure-devops.azure-pipelines)
*Extension #96 of 329*

Syntax highlighting, IntelliSense, and more for Azure Pipelines YAML

**Details:**

- **Extension ID:** `ms-azure-devops.azure-pipelines`
- **Version:** 1.261.1
- **Installs:** 1.36M
- **Rating:** ⭐ 2.4/5 (43 ratings)
- **Published:** 2018-11-07
- **Last Updated:** 2025-10-09
- **VS Code Engine:** ^1.82.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azure-devops.azure-pipelines

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/azure-pipelines-vscode.git)
- 🐛 [Issues](https://github.com/Microsoft/azure-pipelines-vscode/issues/)
- 🏠 [Homepage](https://github.com/Microsoft/azure-pipelines-vscode/blob/main/README.md)

**Marketplace Categories:**

Programming Languages, Azure

**Tags:**

Azure Pipelines, AzurePipelines, azure-pipelines, CI/CD, continuous integration, YAML

**Works With:**

All Platforms

---

### 21. [Microsoft Terraform](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureterraform)
*Extension #97 of 329*

VS Code extension for developing with Terraform on Azure

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azureterraform`
- **Version:** 0.8.0
- **Installs:** 1.12M
- **Rating:** ⭐ 3.9/5 (11 ratings)
- **Published:** 2018-03-15
- **Last Updated:** 2025-12-05
- **VS Code Engine:** ^1.82.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureterraform

**Resources:**

- 📦 [Repository](https://github.com/Azure/vscode-azureterraform.git)
- 🐛 [Issues](https://github.com/Azure/vscode-azureterraform/issues)
- 🏠 [Homepage](https://github.com/Azure/vscode-azureterraform#readme)

**Marketplace Categories:**

Azure

**Tags:**

__ext_tf, azure, cloudshell, devops, terraform

**Works With:**

Linux x64, Linux ARM, Windows x64, macOS Apple Silicon, Linux ARM64, Windows ARM64, macOS Intel

---

### 22. [GitHub Copilot for Azure](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-github-copilot)
*Extension #98 of 329*

GitHub Copilot for Azure is the @azure extension. It's designed to help streamline the process of developing for Azure. You can ask @azure questions about Azure services or get help with tasks related to Azure and developing for Azure, all from within Visual Studio Code.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azure-github-copilot`
- **Version:** 1.0.137
- **Installs:** 868K
- **Rating:** ⭐ 3.7/5 (12 ratings)
- **Published:** 2024-10-28
- **Last Updated:** 2025-10-23
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-github-copilot

**Resources:**

- 📦 [Repository](https://github.com/microsoft/GitHub-Copilot-for-Azure.git)
- 🐛 [Issues](https://github.com/microsoft/GitHub-Copilot-for-Azure/issues)
- 🏠 [Homepage](https://github.com/microsoft/GitHub-Copilot-for-Azure)

**Marketplace Categories:**

Azure, AI, Chat

**Tags:**

agent, Azure, chat, chat-participant, copilot, github-copilot, language-model-tools, tools

**Works With:**

Linux ARM64, Linux x64, macOS Intel, macOS Apple Silicon, Windows ARM64, Windows x64

---

### 23. [GitHub Copilot app modernization](https://marketplace.visualstudio.com/items?itemName=vscjava.migrate-java-to-azure)
*Extension #99 of 329*

Upgrade and migrate your applications to Azure

**Details:**

- **Extension ID:** `vscjava.migrate-java-to-azure`
- **Version:** 1.8.2025120402
- **Installs:** 477K
- **Rating:** ⭐ 1.9/5 (24 ratings)
- **Published:** 2025-05-09
- **Last Updated:** 2025-12-04
- **VS Code Engine:** ^1.106.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.migrate-java-to-azure

**Resources:**

- 📦 [Repository](https://aka.ms/ghcp-appmod/feedback)

**Marketplace Categories:**

AI, Chat

**Tags:**

csharp, java, typescript

**Works With:**

Linux ARM64, macOS Apple Silicon, macOS Intel, Windows x64, Windows ARM64, Linux x64

---

### 24. [Azurite](https://marketplace.visualstudio.com/items?itemName=Azurite.azurite)
*Extension #100 of 329*

An open source Azure Storage API compatible server

**Details:**

- **Extension ID:** `Azurite.azurite`
- **Version:** 3.35.0
- **Installs:** 456K
- **Rating:** ⭐ 5/5 (6 ratings)
- **Published:** 2019-06-26
- **Last Updated:** 2025-07-31
- **VS Code Engine:** ^1.39.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=Azurite.azurite

**Resources:**

- 📦 [Repository](https://github.com/azure/azurite.git)
- 🐛 [Issues](https://github.com/azure/azurite/issues)
- 🏠 [Homepage](https://github.com/azure/azurite#readme)

**Marketplace Categories:**

Other

**Tags:**

Azure, Azurite, Blob, Emulator, Microsoft, Queue, Storage

**Works With:**

All Platforms

---

### 25. [Azure IoT Hub](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-toolkit)
*Extension #101 of 329*

This extension is now a part of Azure IoT Tools extension pack. We highly recommend installing Azure IoT Tools to get full capabilities for Azure IoT development. Interact with Azure IoT Hub, IoT Device Management, IoT Edge Management, IoT Hub Device Simulation, IoT Hub Code Generation and IoT Hub D

**Details:**

- **Extension ID:** `vsciot-vscode.azure-iot-toolkit`
- **Version:** 2.17.1
- **Installs:** 435K
- **Rating:** ⭐ 4.6/5 (8 ratings)
- **Published:** 2016-10-28
- **Last Updated:** 2022-09-20
- **VS Code Engine:** ^1.65.1
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-toolkit

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-azure-iot-toolkit.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-azure-iot-toolkit/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azure-iot-toolkit/blob/master/README.md)

**Marketplace Categories:**

Programming Languages, Snippets, Azure

**Tags:**

azure, dps, iot, IoT Edge, snippet, toolkit

**Works With:**

All Platforms

---

### 26. [Azure MCP Server](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-mcp-server)
*Extension #102 of 329*

Provides Model Context Protocol (MCP) integration and tooling for Azure in Visual Studio Code.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azure-mcp-server`
- **Version:** 2.0.7
- **Installs:** 325K
- **Rating:** ⭐ 5/5 (6 ratings)
- **Published:** 2025-07-24
- **Last Updated:** 2025-11-26
- **VS Code Engine:** ^1.101.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-mcp-server

**Resources:**

- 📦 [Repository](https://github.com/microsoft/mcp.git)
- 🐛 [Issues](https://github.com/microsoft/mcp/issues)
- 🏠 [Homepage](https://github.com/microsoft/mcp#readme)

**Marketplace Categories:**

Extension Packs, Azure, AI, Chat

**Tags:**

agent, ai, azure, chat, copilot, language-model-tools, mcp, model context protocol

**Works With:**

Windows ARM64, Windows x64, macOS Intel, macOS Apple Silicon, Linux x64, Linux ARM64

---

### 27. [Azure Data Lake Tools](https://marketplace.visualstudio.com/items?itemName=usqlextpublisher.usql-vscode-ext)
*Extension #103 of 329*

ADL - USQL Language Support

**Details:**

- **Extension ID:** `usqlextpublisher.usql-vscode-ext`
- **Version:** 0.2.16
- **Installs:** 310K
- **Rating:** ⭐ 2.3/5 (3 ratings)
- **Published:** 2017-03-01
- **Last Updated:** 2023-10-12
- **VS Code Engine:** ^1.23.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=usqlextpublisher.usql-vscode-ext

**Resources:**

- 📦 [Repository](https://github.com/microsoft/AzureDatalakeToolsForVSCode.git)
- 🐛 [Issues](https://github.com/microsoft/AzureDatalakeToolsForVSCode/issues)
- 💬 [Support](mailto:hdivstool@microsoft.com)
- 🏠 [Homepage](https://github.com/microsoft/AzureDatalakeToolsForVSCode#readme)

**Marketplace Categories:**

Programming Languages, Azure

**Tags:**

__ext_abr, __ext_dll, __ext_jobInfo, __ext_usql, Azure, Azure Data Lake, Azure Data Lake Analytics, Azure Data Lake Store, dll, keybindings, snippet, usql, U-SQL

**Works With:**

All Platforms

---

### 28. [Azure IoT Edge](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-edge)
*Extension #104 of 329*

This extension is now in maintenance mode. See announcement https://github.com/microsoft/vscode-azure-iot-edge/issues/639#issue-1359146049 and latest documentation here: https://learn.microsoft.com/en-us/azure/iot-edge/how-to-vs-code-develop-module.

**Details:**

- **Extension ID:** `vsciot-vscode.azure-iot-edge`
- **Version:** 1.25.11
- **Installs:** 257K
- **Rating:** ⭐ 4.2/5 (8 ratings)
- **Published:** 2017-11-13
- **Last Updated:** 2023-01-09
- **VS Code Engine:** ^1.40.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-edge

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-azure-iot-edge.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-azure-iot-edge/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azure-iot-edge/blob/master/README.md)

**Marketplace Categories:**

Programming Languages, Debuggers, Azure

**Tags:**

azure, cloud, debug, debuggers, iot, IoT Edge, json, multi-root ready

**Works With:**

All Platforms

---

### 29. [Azure API Management](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-apimanagement)
*Extension #105 of 329*

An Azure API Management extension for Visual Studio Code.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-apimanagement`
- **Version:** 1.3.0
- **Installs:** 246K
- **Rating:** ⭐ 4.2/5 (13 ratings)
- **Published:** 2019-06-27
- **Last Updated:** 2025-07-31
- **VS Code Engine:** ^1.50.1
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-apimanagement

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-apimanagement.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-apimanagement/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-apimanagement#readme)

**Marketplace Categories:**

Azure

**Tags:**

API Management, APIM, apim-policy, Azure, debuggers, json, language-model-tools, OpenAPI, policy, snippet, swagger, tools, xml

**Works With:**

All Platforms

---

### 30. [Azure Devops Codespaces Authentication](https://marketplace.visualstudio.com/items?itemName=ms-codespaces-tools.ado-codespaces-auth)
*Extension #106 of 329*

Seamless authentication to Azure Devops in GitHub Codespaces

**Details:**

- **Extension ID:** `ms-codespaces-tools.ado-codespaces-auth`
- **Version:** 1.2.1
- **Installs:** 184K
- **Published:** 2023-02-19
- **Last Updated:** 2024-02-13
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-codespaces-tools.ado-codespaces-auth

**Resources:**

- 📦 [Repository](https://github.com/microsoft/ado-codespaces-auth.git)
- 🐛 [Issues](https://github.com/microsoft/ado-codespaces-auth/issues)
- 🏠 [Homepage](https://github.com/microsoft/ado-codespaces-auth#readme)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 31. [Azure Logic Apps (Consumption)](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-logicapps)
*Extension #107 of 329*

Visual Studio Code extension for Azure Logic Apps (Consumption)

**Details:**

- **Extension ID:** `ms-azuretools.vscode-logicapps`
- **Version:** 1.2.9
- **Installs:** 176K
- **Rating:** ⭐ 2.8/5 (4 ratings)
- **Published:** 2018-09-21
- **Last Updated:** 2023-07-17
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-logicapps

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-azurelogicapps.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-azurelogicapps/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-azurelogicapps/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

Azure, Integration Accounts, Logic Apps, snippet

**Works With:**

All Platforms

---

### 32. [Azure Application Insights](https://marketplace.visualstudio.com/items?itemName=VisualStudioOnlineApplicationInsights.application-insights)
*Extension #108 of 329*

Brings information from your production services right into your code editor to help you find and fix issues faster.

**Details:**

- **Extension ID:** `VisualStudioOnlineApplicationInsights.application-insights`
- **Version:** 0.4.2
- **Installs:** 139K
- **Rating:** ⭐ 1/5 (3 ratings)
- **Published:** 2016-12-20
- **Last Updated:** 2018-03-02
- **VS Code Engine:** ^1.5.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=VisualStudioOnlineApplicationInsights.application-insights

**Marketplace Categories:**

Other

**Tags:**

Azure, CodeLens, Express, Node, Telemetry

**Works With:**

All Platforms

---

### 33. [Azure PowerShell Tools](https://marketplace.visualstudio.com/items?itemName=azps-tools.azps-tools)
*Extension #109 of 329*

(preview) Automate the migration of Azure PowerShell scripts and modules from AzureRM to the Az PowerShell module.

**Details:**

- **Extension ID:** `azps-tools.azps-tools`
- **Version:** 0.3.0
- **Installs:** 124K
- **Rating:** ⭐ 2/5 (3 ratings)
- **Published:** 2021-08-31
- **Last Updated:** 2023-09-28
- **VS Code Engine:** ^1.47.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=azps-tools.azps-tools

**Resources:**

- 📦 [Repository](https://github.com/Azure/azure-powershell-migration.git)
- 🐛 [Issues](https://github.com/Azure/azure-powershell-migration/issues)
- 🏠 [Homepage](https://github.com/Azure/azure-powershell-migration#readme)

**Marketplace Categories:**

Azure

**Tags:**

powershell

**Works With:**

All Platforms

---

### 34. [Azure Logic Apps (Standard)](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurelogicapps)
*Extension #110 of 329*

Visual Studio Code extension for Azure Logic Apps (Standard)

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azurelogicapps`
- **Version:** 5.185.10
- **Installs:** 115K
- **Rating:** ⭐ 3.6/5 (5 ratings)
- **Published:** 2020-09-21
- **Last Updated:** 2025-10-20
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurelogicapps

**Resources:**

- 📦 [Repository](https://github.com/Azure/LogicAppsUX.git)
- 🐛 [Issues](https://github.com/Azure/LogicAppsUX/issues)
- 🏠 [Homepage](https://azure.microsoft.com/services/logic-apps/)

**Marketplace Categories:**

Azure

**Tags:**

Azure, debuggers, Logic App, multi-root ready, serverless

**Works With:**

All Platforms

---

### 35. [Azure Automation](https://marketplace.visualstudio.com/items?itemName=azure-automation.vscode-azureautomation)
*Extension #111 of 329*

An Azure Automation extension for Visual Studio Code

**Details:**

- **Extension ID:** `azure-automation.vscode-azureautomation`
- **Version:** 1.2.5
- **Installs:** 88K
- **Rating:** ⭐ 3.9/5 (10 ratings)
- **Published:** 2022-10-04
- **Last Updated:** 2024-07-08
- **VS Code Engine:** ^1.71.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=azure-automation.vscode-azureautomation

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-azureAutomation.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-azureAutomation/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-azureAutomation#readme)

**Marketplace Categories:**

Azure

**Tags:**

automation, azure, azure automation

**Works With:**

All Platforms

---

### 36. [Azure Policy](https://marketplace.visualstudio.com/items?itemName=AzurePolicy.azurepolicyextension)
*Extension #112 of 329*

Azure resource, policy, and alias viewer

**Details:**

- **Extension ID:** `AzurePolicy.azurepolicyextension`
- **Version:** 0.2.5
- **Installs:** 75K
- **Rating:** ⭐ 2.6/5 (18 ratings)
- **Published:** 2019-10-29
- **Last Updated:** 2025-10-13
- **VS Code Engine:** ^1.101.2
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=AzurePolicy.azurepolicyextension

**Resources:**

- 📦 [Repository](https://msazure.visualstudio.com/One/_git/mgmt-Governance-PolicyVSCodeExtension)

**Marketplace Categories:**

Other

**Tags:**

__ext_resource, json, resource

**Works With:**

All Platforms

---

### 37. [Spark & Hive Tools](https://marketplace.visualstudio.com/items?itemName=mshdinsight.azure-hdinsight)
*Extension #113 of 329*

Spark & Hive Tools - PySpark Interactive Query, PySpark Batch, Hive Interactive Query, Hive Batch

**Details:**

- **Extension ID:** `mshdinsight.azure-hdinsight`
- **Version:** 1.1.19
- **Installs:** 72K
- **Rating:** ⭐ 3/5 (2 ratings)
- **Published:** 2022-12-14
- **Last Updated:** 2023-01-19
- **VS Code Engine:** ^1.23.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=mshdinsight.azure-hdinsight

**Resources:**

- 📦 [Repository](https://github.com/microsoft/Spark-Hive-Tools.git)
- 🐛 [Issues](https://github.com/microsoft/Spark-Hive-Tools/issues)
- 🏠 [Homepage](https://github.com/microsoft/Spark-Hive-Tools#readme)

**Marketplace Categories:**

Programming Languages, Azure

**Tags:**

__ext_hive, __ext_hql, Azure, Azure HDInsight, Big Data, Hive, hql, jupyter, keybindings, LLAP, Notebook, Python, Spark, SQL Server

**Works With:**

All Platforms

---

### 38. [Azure Cache](https://marketplace.visualstudio.com/items?itemName=ms-azurecache.vscode-azurecache)
*Extension #114 of 329*

Manage and browse through data in your Azure Caches.

**Details:**

- **Extension ID:** `ms-azurecache.vscode-azurecache`
- **Version:** 0.1.0
- **Installs:** 49K
- **Rating:** ⭐ 4/5 (4 ratings)
- **Published:** 2020-07-15
- **Last Updated:** 2020-07-22
- **VS Code Engine:** ^1.47.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azurecache.vscode-azurecache

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-azurecache)
- 🐛 [Issues](https://github.com/microsoft/vscode-azurecache/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-azurecache/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

Azure, Azure Cache, Azure Cache for Redis, Redis

**Works With:**

All Platforms

---

### 39. [Azure Stream Analytics Tools](https://marketplace.visualstudio.com/items?itemName=ms-bigdatatools.vscode-asa)
*Extension #115 of 329*

Author, local test and submit Azure Stream Analytics jobs.

**Details:**

- **Extension ID:** `ms-bigdatatools.vscode-asa`
- **Version:** 1.5.5
- **Installs:** 45K
- **Rating:** ⭐ 5/5 (4 ratings)
- **Published:** 2019-05-05
- **Last Updated:** 2024-02-01
- **VS Code Engine:** ^1.25.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-bigdatatools.vscode-asa

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-asa/issues)
- 🐛 [Issues](https://github.com/Microsoft/vscode-asa/issues)

**Marketplace Categories:**

Programming Languages, Azure

**Tags:**

__ext_asaql, __ext_log, asa, asaql, azure, Azure Stream, Azure Stream Analytics, Azure Streaming, debuggers, Log, Stream Analytics, Streaming

**Works With:**

All Platforms

---

### 40. [Azure Data Studio Debug](https://marketplace.visualstudio.com/items?itemName=ms-mssql.sqlops-debug)
*Extension #116 of 329*

Azure Data Studio extension debugging support

**Details:**

- **Extension ID:** `ms-mssql.sqlops-debug`
- **Version:** 1.4.0
- **Installs:** 43K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2018-03-27
- **Last Updated:** 2021-08-26
- **VS Code Engine:** ^1.45.1
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-mssql.sqlops-debug

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/azuredatastudio-debug.git)
- 🐛 [Issues](https://github.com/Microsoft/azuredatastudio-debug/issues)

**Marketplace Categories:**

Debuggers

**Tags:**

debuggers

**Works With:**

All Platforms

---

### 41. [vscode-dev-azurecloudshell-helper](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-dev-azurecloudshell-helper)
*Extension #117 of 329*

**Details:**

- **Extension ID:** `ms-azuretools.vscode-dev-azurecloudshell-helper`
- **Version:** 0.0.4
- **Installs:** 40K
- **Published:** 2025-03-21
- **Last Updated:** 2025-04-22
- **VS Code Engine:** ^1.98.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-dev-azurecloudshell-helper

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-dev-azure.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-dev-azure/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-dev-azure#readme)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 42. [Azure Event Grid](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureeventgrid)
*Extension #118 of 329*

An Azure Event Grid extension for Visual Studio Code.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azureeventgrid`
- **Version:** 0.1.1
- **Installs:** 38K
- **Published:** 2018-06-01
- **Last Updated:** 2018-12-17
- **VS Code Engine:** ^1.23.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureeventgrid

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-azureeventgrid)
- 🐛 [Issues](https://github.com/Microsoft/vscode-azureeventgrid/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azureeventgrid/blob/master/README.md)

**Marketplace Categories:**

Azure

**Tags:**

Azure, EventGrid, json, multi-root ready

**Works With:**

All Platforms

---

### 43. [Azure Sphere](https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-sphere-tools)
*Extension #119 of 329*

VS Code Extension for Azure Sphere

**Details:**

- **Extension ID:** `ms-vscode.azure-sphere-tools`
- **Version:** 25.2.499922
- **Installs:** 21K
- **Published:** 2019-12-05
- **Last Updated:** 2025-05-01
- **VS Code Engine:** ^1.49.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-sphere-tools

**Resources:**

- 📦 [Repository](https://github.com/MicrosoftDocs/azure-sphere-issues.git)
- 🐛 [Issues](https://github.com/MicrosoftDocs/azure-sphere-issues/issues)
- 🏠 [Homepage](https://github.com/MicrosoftDocs/azure-sphere-issues#readme)

**Marketplace Categories:**

Other

**Tags:**

debuggers, json

**Works With:**

All Platforms

---

### 44. [Azure Serverless](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureserverlesspack)
*Extension #120 of 329*

Extension pack for serverless development on Azure.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azureserverlesspack`
- **Version:** 0.1.1
- **Installs:** 21K
- **Published:** 2020-02-10
- **Last Updated:** 2022-10-31
- **VS Code Engine:** ^1.42.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azureserverlesspack

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-azureserverlesspack.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-azureserverlesspack/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-azureserverlesspack)

**Marketplace Categories:**

Extension Packs, Azure

**Tags:**

__web_extension, azure, node

**Works With:**

All Platforms

---

### 45. [Service Fabric Reliable Services](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-service-fabric-reliable-services)
*Extension #121 of 329*

Service Fabric Reliable Services extension for VSCode

**Details:**

- **Extension ID:** `ms-azuretools.vscode-service-fabric-reliable-services`
- **Version:** 0.1.6
- **Installs:** 20K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2018-06-14
- **Last Updated:** 2019-06-19
- **VS Code Engine:** ^1.16.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-service-fabric-reliable-services

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-service-fabric-reliable-services)
- 🐛 [Issues](https://github.com/Microsoft/vscode-service-fabric-reliable-services/issues)

**Marketplace Categories:**

Azure

**Tags:**

Azure, ServiceFabric

**Works With:**

All Platforms

---

### 46. [Azure Sphere UI](https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-sphere-tools-ui)
*Extension #122 of 329*

VS Code Extension for Azure Sphere (UI component)

**Details:**

- **Extension ID:** `ms-vscode.azure-sphere-tools-ui`
- **Version:** 25.2.499922
- **Installs:** 18K
- **Published:** 2020-11-09
- **Last Updated:** 2025-05-01
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.azure-sphere-tools-ui

**Resources:**

- 📦 [Repository](https://github.com/MicrosoftDocs/azure-sphere-issues.git)
- 🐛 [Issues](https://github.com/MicrosoftDocs/azure-sphere-issues/issues)
- 🏠 [Homepage](https://github.com/MicrosoftDocs/azure-sphere-issues#readme)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 47. [Azure Spring Apps](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-azurespringcloud)
*Extension #123 of 329*

An Azure Spring Apps extension for Visual Studio Code.

**Details:**

- **Extension ID:** `vscjava.vscode-azurespringcloud`
- **Version:** 0.10.0
- **Installs:** 17K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2021-01-08
- **Last Updated:** 2024-07-03
- **VS Code Engine:** ^1.76.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-azurespringcloud

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-azurespringcloud.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-azurespringcloud/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-azurespringcloud/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

Azure, cloud, Java, keybindings, microservice, spring

**Works With:**

All Platforms

---

### 48. [Microsoft Entra External ID](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.ms-entra)
*Extension #124 of 329*

Kickstart identity integration for customer-facing apps, without leaving your development environment.

**Details:**

- **Extension ID:** `ms-azuretools.ms-entra`
- **Version:** 1.0.8
- **Installs:** 15K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2024-02-29
- **Last Updated:** 2025-02-07
- **VS Code Engine:** ^1.80.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.ms-entra

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-ms-entra.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-ms-entra/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-ms-entra/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

Authentication, Authorization, AzureAD, B2C, CIAM, Customer-id, CustomerIdentity, External-id, IAM, Identity, IdentityManagement, json, MicrosoftEntra, MicrosoftIdentity, MSAL, OAuth, Security, Tenant

**Works With:**

All Platforms

---

### 49. [Azure API Center](https://marketplace.visualstudio.com/items?itemName=apidev.azure-api-center)
*Extension #125 of 329*

Build, discover, and consume APIs.

**Details:**

- **Extension ID:** `apidev.azure-api-center`
- **Version:** 1.3.0
- **Installs:** 13K
- **Published:** 2024-01-22
- **Last Updated:** 2025-08-04
- **VS Code Engine:** ^1.99.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=apidev.azure-api-center

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-azureapicenter.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-azureapicenter/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-azureapicenter#readme)

**Marketplace Categories:**

Azure, AI

**Tags:**

AsyncAPI, GraphQL, gRPC, language-model-tools, OpenAPI, RAML, tools

**Works With:**

All Platforms

---

### 50. [Azure Video Analyzer](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.azure-video-analyzer)
*Extension #126 of 329*

Azure Video Analyzer extension for Visual Studio Code provides support by making it easy to edit and manage media topologies and live pipelines.

**Details:**

- **Extension ID:** `ms-azuretools.azure-video-analyzer`
- **Version:** 0.2.4
- **Installs:** 12K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2021-05-20
- **Last Updated:** 2022-01-28
- **VS Code Engine:** ^1.38.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.azure-video-analyzer

**Resources:**

- 📦 [Repository](https://github.com/Azure/video-analyzer-vscode-extension)
- 🐛 [Issues](https://github.com/Azure/video-analyzer-vscode-extension/issues)
- 🏠 [Homepage](https://github.com/Azure/video-analyzer-vscode-extension)

**Marketplace Categories:**

Extension Packs, Azure

**Tags:**

AVA, azure video analyzer, azureVideo, video analyzer, videoAnalyzer

**Works With:**

All Platforms

---

### 51. [Tye](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-tye)
*Extension #127 of 329*

Makes it easy to debug and interact with applications using Tye.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-tye`
- **Version:** 0.5.1
- **Installs:** 10K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2021-06-01
- **Last Updated:** 2022-08-01
- **VS Code Engine:** ^1.67.2
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-tye

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-tye.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-tye/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-tye/)

**Marketplace Categories:**

Other

**Tags:**

debuggers

**Works With:**

All Platforms

---

### 52. [VS Code Azure Functions Web](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-functions-web)
*Extension #128 of 329*

A wrapper extension for the Azure Functions extension that provides a webview-based experience for creating, running, and deploying Azure Functions.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azure-functions-web`
- **Version:** 0.0.39
- **Installs:** 9K
- **Published:** 2024-05-21
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-functions-web

**Resources:**

- 📦 [Repository](https://github.com/Azure/azure-functions-vscode-web/issues)
- 🐛 [Issues](https://github.com/Azure/azure-functions-vscode-web/issues)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 53. [Azure AI Speech Toolkit](https://marketplace.visualstudio.com/items?itemName=ms-azureaispeech.azure-ai-speech-toolkit)
*Extension #129 of 329*

Build Azure Speech empowered apps with just a few clicks

**Details:**

- **Extension ID:** `ms-azureaispeech.azure-ai-speech-toolkit`
- **Version:** 0.0.21
- **Installs:** 9K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2024-10-17
- **Last Updated:** 2025-08-28
- **VS Code Engine:** ^1.92.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azureaispeech.azure-ai-speech-toolkit

**Resources:**

- 📦 [Repository](https://github.com/microsoft/AzureAISpeechToolKit.git)
- 🐛 [Issues](https://github.com/microsoft/AzureAISpeechToolKit/issues)
- 🏠 [Homepage](https://github.com/microsoft/AzureAISpeechToolKit#readme)

**Marketplace Categories:**

Azure, AI

**Works With:**

All Platforms

---

### 54. [FHIR Converter](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-health-fhir-converter)
*Extension #130 of 329*

Enables the conversion of legacy formatted health data to FHIR, expanding the use cases for health data and enabling interoperability.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-health-fhir-converter`
- **Version:** 1.4.2
- **Installs:** 9K
- **Rating:** ⭐ 5/5 (4 ratings)
- **Published:** 2020-12-07
- **Last Updated:** 2022-04-08
- **VS Code Engine:** ^1.44.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-health-fhir-converter

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-azurehealthcareapis-tools.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-azurehealthcareapis-tools/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-azurehealthcareapis-tools/blob/main/FHIR-Converter/README.md)

**Marketplace Categories:**

Azure

**Tags:**

Azure, Azure API for FHIR, C-CDA, Converter, FHIR, FHIR Server for Azure, Healthcare, HL7v2, keybindings, Liquid

**Works With:**

All Platforms

---

### 55. [vscode-azure-functions-remote-web](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-functions-remote-web)
*Extension #131 of 329*

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azure-functions-remote-web`
- **Version:** 0.2.54
- **Installs:** 8K
- **Published:** 2022-09-12
- **Last Updated:** 2024-10-03
- **VS Code Engine:** ^1.90.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azure-functions-remote-web

**Marketplace Categories:**

Other

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 56. [DocumentDB for VS Code](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-documentdb)
*Extension #132 of 329*

DocumentDB and MongoDB GUI: Connect to databases, run queries, and manage your data. Supports working with databases across both cloud-based and local environments.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-documentdb`
- **Version:** 0.6.1
- **Installs:** 7K
- **Rating:** ⭐ 5/5 (8 ratings)
- **Published:** 2025-05-09
- **Last Updated:** 2025-11-20
- **VS Code Engine:** ^1.90.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-documentdb

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-documentdb.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-documentdb/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-documentdb/blob/main/README.md)

**Marketplace Categories:**

Programming Languages, Snippets, Azure, Data Science, Machine Learning, Visualization, AI, Chat

**Tags:**

__ext_vscode-documentdb-scrapbook, Database, Databases, DocumentDB, DocumentDBScrapbook, FerretDB, JSON, keybindings, MongoDB, NoSQL, vscode-documentdb-scrapbook-language

**Works With:**

All Platforms

---

### 57. [Radius Bicep](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.rad-vscode-bicep)
*Extension #133 of 329*

Radius custom Bicep language support for Visual Studio Code

**Details:**

- **Extension ID:** `ms-azuretools.rad-vscode-bicep`
- **Version:** 0.36.0
- **Installs:** 7K
- **Published:** 2023-10-17
- **Last Updated:** 2024-07-22
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.rad-vscode-bicep

**Resources:**

- 📦 [Repository](https://github.com/radius-project/bicep.git)
- 🐛 [Issues](https://github.com/radius-project/bicep/issues)
- 🏠 [Homepage](https://github.com/radius-project/bicep/blob/main/README.md)

**Marketplace Categories:**

Programming Languages, Azure

**Tags:**

__ext_bicep, __ext_bicepparam, ARM Template, Azure, Azure Resource Manager, Bicep, Bicep Parameters, Bicep Params, bicep-params, json, jsonc, keybindings, Radius, snippet

**Works With:**

All Platforms

---

### 58. [Azure Arc](https://marketplace.visualstudio.com/items?itemName=azurearc-dev.azurearc)
*Extension #134 of 329*

Azure Arc

**Details:**

- **Extension ID:** `azurearc-dev.azurearc`
- **Version:** 0.1.0
- **Installs:** 4K
- **Published:** 2023-08-30
- **Last Updated:** 2023-10-10
- **VS Code Engine:** ^1.78.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=azurearc-dev.azurearc

**Resources:**

- 📦 [Repository](https://github.com/azure/azurearc-dev.git)
- 🐛 [Issues](https://github.com/azure/azurearc-dev/issues)
- 🏠 [Homepage](https://github.com/azure/azurearc-dev#readme)

**Marketplace Categories:**

Azure

**Works With:**

All Platforms

---

### 59. [Azure Arc-enabled machines](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurearcenabledmachines)
*Extension #135 of 329*

An Azure Arc-enabled machines extension for Visual Studio Code.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azurearcenabledmachines`
- **Version:** 0.0.3
- **Installs:** 3K
- **Published:** 2024-02-07
- **Last Updated:** 2024-05-30
- **VS Code Engine:** ^1.89.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurearcenabledmachines

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-azurearcenabledmachines.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-azurearcenabledmachines/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-azurearcenabledmachines/blob/main/README.md)

**Marketplace Categories:**

Azure

**Tags:**

Azure

**Works With:**

All Platforms

---

### 60. [Learn Cloud](https://marketplace.visualstudio.com/items?itemName=azurepaas-tools.vscode-learncloud)
*Extension #136 of 329*

Guiding first-time cloud users to deploy to Azure PaaS

**Details:**

- **Extension ID:** `azurepaas-tools.vscode-learncloud`
- **Version:** 0.8.0
- **Installs:** 3K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2024-05-08
- **Last Updated:** 2024-09-20
- **VS Code Engine:** ^1.83.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=azurepaas-tools.vscode-learncloud

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-learncloud.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-learncloud/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-learncloud#readme)

**Marketplace Categories:**

Azure, Education

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 61. [Azure Web PubSub](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurewebpubsub)
*Extension #137 of 329*

An Azure Web PubSub extension for Visual Studio Code.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-azurewebpubsub`
- **Version:** 0.1.1
- **Installs:** 2K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2024-04-26
- **Last Updated:** 2024-06-03
- **VS Code Engine:** ^1.76.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-azurewebpubsub

**Resources:**

- 📦 [Repository](https://github.com/Azure/azure-webpubsub.git)
- 🐛 [Issues](https://github.com/Azure/azure-webpubsub/issues)
- 🏠 [Homepage](https://github.com/Azure/azure-webpubsub/blob/main/tools/vscode-azurewebpubsub/README.md)

**Marketplace Categories:**

Azure

**Tags:**

Azure, cloud, microservice, Web PubSub

**Works With:**

All Platforms

---

### 62. [Azure OSConfig](https://marketplace.visualstudio.com/items?itemName=edge-security.osconfig)
*Extension #138 of 329*

Rich authoring for Azure Device OS Configuration Desired Configuration (DC) documents.

**Details:**

- **Extension ID:** `edge-security.osconfig`
- **Version:** 1.0.0
- **Installs:** 1K
- **Published:** 2023-03-08
- **Last Updated:** 2023-03-08
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=edge-security.osconfig

**Resources:**

- 📦 [Repository](https://github.com/Azure/vscode-osconfig.git)
- 🐛 [Issues](https://github.com/Azure/vscode-osconfig/issues)
- 🏠 [Homepage](https://github.com/Azure/vscode-osconfig#readme)

**Marketplace Categories:**

Programming Languages, Snippets, Linters

**Tags:**

azure, desired configuration, edge security, json, multi-root ready, osconfig

**Works With:**

All Platforms

---

### 63. [VS Code for the Web - Azure](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-dev-azurecloudshell)
*Extension #139 of 329*

**Details:**

- **Extension ID:** `ms-azuretools.vscode-dev-azurecloudshell`
- **Version:** 0.5.3
- **Installs:** 675
- **Published:** 2024-11-25
- **Last Updated:** 2025-11-14
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-dev-azurecloudshell

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-dev-azure.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-dev-azure/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-dev-azure#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 64. [Template MCP Server](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-template-mcp-server)
*Extension #140 of 329*

Template project for validating the microsoft/mcp engineering system.

**Details:**

- **Extension ID:** `ms-azuretools.vscode-template-mcp-server`
- **Version:** 0.0.5629128
- **Installs:** 674
- **Published:** 2025-10-07
- **Last Updated:** 2025-12-02
- **VS Code Engine:** ^1.101.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-template-mcp-server

**Resources:**

- 📦 [Repository](https://github.com/microsoft/mcp.git)
- 🐛 [Issues](https://github.com/microsoft/mcp/issues)
- 🏠 [Homepage](https://github.com/microsoft/mcp#readme)

**Marketplace Categories:**

Extension Packs, AI, Chat

**Tags:**

agent, ai, chat, copilot, language-model-tools, mcp, model context protocol

**Works With:**

Linux ARM64, Linux x64, macOS Intel, macOS Apple Silicon, Windows ARM64, Windows x64

---

### 65. [Azure Terraform Helper](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.azureterraform-vscode-web-opener)
*Extension #141 of 329*

**Details:**

- **Extension ID:** `ms-azuretools.azureterraform-vscode-web-opener`
- **Version:** 0.0.9
- **Installs:** 600
- **Published:** 2025-08-20
- **Last Updated:** 2025-10-31
- **VS Code Engine:** ^1.102.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azuretools.azureterraform-vscode-web-opener

**Marketplace Categories:**

Other

**Tags:**

__web_extension

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Extension Packs

**Category Stats:** 46 extensions | Total Installs: 210.24M | Avg Rating: 4.5⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [C/C++ Extension Pack](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools-extension-pack)
*Extension #142 of 329*

Popular extensions for C++ development in Visual Studio Code.

**Details:**

- **Extension ID:** `ms-vscode.cpptools-extension-pack`
- **Version:** 1.3.1
- **Installs:** 48.97M
- **Rating:** ⭐ 4.6/5 (44 ratings)
- **Published:** 2020-09-08
- **Last Updated:** 2025-02-24
- **VS Code Engine:** ^1.48.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools-extension-pack

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-cpptools.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-cpptools/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-cpptools#readme)

**Marketplace Categories:**

Programming Languages, Extension Packs

**Tags:**

build, C, C++, cmake, color-theme, IntelliSense, Microsoft

**Works With:**

All Platforms

---

### 2. [Chinese (Simplified) (简体中文) Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-zh-hans)
*Extension #143 of 329*

Language pack extension for Chinese (Simplified)

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-zh-hans`
- **Version:** 1.107.2025120309
- **Installs:** 46.55M
- **Rating:** ⭐ 4.8/5 (79 ratings)
- **Published:** 2018-04-23
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-zh-hans

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-zh-cn_ms-vscode.js-debug, __lp-zh-cn_vscode, __lp-zh-cn_vscode.bat, __lp-zh-cn_vscode.builtin-notebook-renderers, __lp-zh-cn_vscode.clojure, __lp-zh-cn_vscode.coffeescript, __lp-zh-cn_vscode.configuration-editing, __lp-zh-cn_vscode.cpp, __lp-zh-cn_vscode.csharp, __lp-zh-cn_vscode.css, __lp-zh-cn_vscode.css-language-features, __lp-zh-cn_vscode.dart, __lp-zh-cn_vscode.debug-auto-launch, __lp-zh-cn_vscode.debug-server-ready, __lp-zh-cn_vscode.diff, __lp-zh-cn_vscode.docker, __lp-zh-cn_vscode.dotenv, __lp-zh-cn_vscode.emmet, __lp-zh-cn_vscode.extension-editing, __lp-zh-cn_vscode.fsharp, __lp-zh-cn_vscode.git, __lp-zh-cn_vscode.git-base, __lp-zh-cn_vscode.github, __lp-zh-cn_vscode.github-authentication, __lp-zh-cn_vscode.go, __lp-zh-cn_vscode.groovy, __lp-zh-cn_vscode.grunt, __lp-zh-cn_vscode.gulp, __lp-zh-cn_vscode.handlebars, __lp-zh-cn_vscode.hlsl, __lp-zh-cn_vscode.html, __lp-zh-cn_vscode.html-language-features, __lp-zh-cn_vscode.ini, __lp-zh-cn_vscode.ipynb, __lp-zh-cn_vscode.jake, __lp-zh-cn_vscode.java, __lp-zh-cn_vscode.javascript, __lp-zh-cn_vscode.json, __lp-zh-cn_vscode.json-language-features, __lp-zh-cn_vscode.julia, __lp-zh-cn_vscode.latex, __lp-zh-cn_vscode.less, __lp-zh-cn_vscode.log, __lp-zh-cn_vscode.lua, __lp-zh-cn_vscode.make, __lp-zh-cn_vscode.markdown, __lp-zh-cn_vscode.markdown-language-features, __lp-zh-cn_vscode.markdown-math, __lp-zh-cn_vscode.media-preview, __lp-zh-cn_vscode.merge-conflict, __lp-zh-cn_vscode.mermaid-chat-features, __lp-zh-cn_vscode.microsoft-authentication, __lp-zh-cn_vscode.npm, __lp-zh-cn_vscode.objective-c, __lp-zh-cn_vscode.perl, __lp-zh-cn_vscode.php, __lp-zh-cn_vscode.php-language-features, __lp-zh-cn_vscode.powershell, __lp-zh-cn_vscode.prompt, __lp-zh-cn_vscode.pug, __lp-zh-cn_vscode.python, __lp-zh-cn_vscode.r, __lp-zh-cn_vscode.razor, __lp-zh-cn_vscode.references-view, __lp-zh-cn_vscode.restructuredtext, __lp-zh-cn_vscode.ruby, __lp-zh-cn_vscode.rust, __lp-zh-cn_vscode.scss, __lp-zh-cn_vscode.search-result, __lp-zh-cn_vscode.shaderlab, __lp-zh-cn_vscode.shellscript, __lp-zh-cn_vscode.simple-browser, __lp-zh-cn_vscode.sql, __lp-zh-cn_vscode.swift, __lp-zh-cn_vscode.terminal-suggest, __lp-zh-cn_vscode.theme-abyss, __lp-zh-cn_vscode.theme-defaults, __lp-zh-cn_vscode.theme-kimbie-dark, __lp-zh-cn_vscode.theme-monokai, __lp-zh-cn_vscode.theme-monokai-dimmed, __lp-zh-cn_vscode.theme-quietlight, __lp-zh-cn_vscode.theme-red, __lp-zh-cn_vscode.theme-solarized-dark, __lp-zh-cn_vscode.theme-solarized-light, __lp-zh-cn_vscode.theme-tomorrow-night-blue, __lp-zh-cn_vscode.tunnel-forwarding, __lp-zh-cn_vscode.typescript, __lp-zh-cn_vscode.typescript-language-features, __lp-zh-cn_vscode.vb, __lp-zh-cn_vscode.vscode-theme-seti, __lp-zh-cn_vscode.xml, __lp-zh-cn_vscode.yaml, lp-zh-cn, 中文(简体)

**Works With:**

All Platforms

---

### 3. [Extension Pack for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-pack)
*Extension #144 of 329*

Popular extensions for Java development that provides Java IntelliSense, debugging, testing, Maven/Gradle support, project management and more

**Details:**

- **Extension ID:** `vscjava.vscode-java-pack`
- **Version:** 0.30.2025111702
- **Installs:** 40.12M
- **Rating:** ⭐ 3.7/5 (87 ratings)
- **Published:** 2017-09-27
- **Last Updated:** 2025-11-17
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-pack

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-java-pack.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-java-pack/issues)
- 💬 [Support](https://github.com/Microsoft/vscode-java-pack)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-java-pack)

**Marketplace Categories:**

Programming Languages, Snippets, Linters, Debuggers, Formatters, Extension Packs

**Tags:**

debugger, java, junit, keybindings, maven, spring

**Works With:**

All Platforms

---

### 4. [Japanese Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-ja)
*Extension #145 of 329*

Language pack extension for Japanese

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-ja`
- **Version:** 1.107.2025120309
- **Installs:** 12.97M
- **Rating:** ⭐ 4.6/5 (10 ratings)
- **Published:** 2018-05-10
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-ja

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-ja_ms-vscode.js-debug, __lp-ja_vscode, __lp-ja_vscode.bat, __lp-ja_vscode.builtin-notebook-renderers, __lp-ja_vscode.clojure, __lp-ja_vscode.coffeescript, __lp-ja_vscode.configuration-editing, __lp-ja_vscode.cpp, __lp-ja_vscode.csharp, __lp-ja_vscode.css, __lp-ja_vscode.css-language-features, __lp-ja_vscode.dart, __lp-ja_vscode.debug-auto-launch, __lp-ja_vscode.debug-server-ready, __lp-ja_vscode.diff, __lp-ja_vscode.docker, __lp-ja_vscode.dotenv, __lp-ja_vscode.emmet, __lp-ja_vscode.extension-editing, __lp-ja_vscode.fsharp, __lp-ja_vscode.git, __lp-ja_vscode.git-base, __lp-ja_vscode.github, __lp-ja_vscode.github-authentication, __lp-ja_vscode.go, __lp-ja_vscode.groovy, __lp-ja_vscode.grunt, __lp-ja_vscode.gulp, __lp-ja_vscode.handlebars, __lp-ja_vscode.hlsl, __lp-ja_vscode.html, __lp-ja_vscode.html-language-features, __lp-ja_vscode.ini, __lp-ja_vscode.ipynb, __lp-ja_vscode.jake, __lp-ja_vscode.java, __lp-ja_vscode.javascript, __lp-ja_vscode.json, __lp-ja_vscode.json-language-features, __lp-ja_vscode.julia, __lp-ja_vscode.latex, __lp-ja_vscode.less, __lp-ja_vscode.log, __lp-ja_vscode.lua, __lp-ja_vscode.make, __lp-ja_vscode.markdown, __lp-ja_vscode.markdown-language-features, __lp-ja_vscode.markdown-math, __lp-ja_vscode.media-preview, __lp-ja_vscode.merge-conflict, __lp-ja_vscode.mermaid-chat-features, __lp-ja_vscode.microsoft-authentication, __lp-ja_vscode.npm, __lp-ja_vscode.objective-c, __lp-ja_vscode.perl, __lp-ja_vscode.php, __lp-ja_vscode.php-language-features, __lp-ja_vscode.powershell, __lp-ja_vscode.prompt, __lp-ja_vscode.pug, __lp-ja_vscode.python, __lp-ja_vscode.r, __lp-ja_vscode.razor, __lp-ja_vscode.references-view, __lp-ja_vscode.restructuredtext, __lp-ja_vscode.ruby, __lp-ja_vscode.rust, __lp-ja_vscode.scss, __lp-ja_vscode.search-result, __lp-ja_vscode.shaderlab, __lp-ja_vscode.shellscript, __lp-ja_vscode.simple-browser, __lp-ja_vscode.sql, __lp-ja_vscode.swift, __lp-ja_vscode.terminal-suggest, __lp-ja_vscode.theme-abyss, __lp-ja_vscode.theme-defaults, __lp-ja_vscode.theme-kimbie-dark, __lp-ja_vscode.theme-monokai, __lp-ja_vscode.theme-monokai-dimmed, __lp-ja_vscode.theme-quietlight, __lp-ja_vscode.theme-red, __lp-ja_vscode.theme-solarized-dark, __lp-ja_vscode.theme-solarized-light, __lp-ja_vscode.theme-tomorrow-night-blue, __lp-ja_vscode.tunnel-forwarding, __lp-ja_vscode.typescript, __lp-ja_vscode.typescript-language-features, __lp-ja_vscode.vb, __lp-ja_vscode.vscode-theme-seti, __lp-ja_vscode.xml, __lp-ja_vscode.yaml, lp-ja, 日本語

**Works With:**

All Platforms

---

### 5. [Spanish Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-es)
*Extension #146 of 329*

Language pack extension for Spanish

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-es`
- **Version:** 1.107.2025120309
- **Installs:** 10.55M
- **Rating:** ⭐ 4.4/5 (15 ratings)
- **Published:** 2018-05-10
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-es

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-es_ms-vscode.js-debug, __lp-es_vscode, __lp-es_vscode.bat, __lp-es_vscode.builtin-notebook-renderers, __lp-es_vscode.clojure, __lp-es_vscode.coffeescript, __lp-es_vscode.configuration-editing, __lp-es_vscode.cpp, __lp-es_vscode.csharp, __lp-es_vscode.css, __lp-es_vscode.css-language-features, __lp-es_vscode.dart, __lp-es_vscode.debug-auto-launch, __lp-es_vscode.debug-server-ready, __lp-es_vscode.diff, __lp-es_vscode.docker, __lp-es_vscode.dotenv, __lp-es_vscode.emmet, __lp-es_vscode.extension-editing, __lp-es_vscode.fsharp, __lp-es_vscode.git, __lp-es_vscode.git-base, __lp-es_vscode.github, __lp-es_vscode.github-authentication, __lp-es_vscode.go, __lp-es_vscode.groovy, __lp-es_vscode.grunt, __lp-es_vscode.gulp, __lp-es_vscode.handlebars, __lp-es_vscode.hlsl, __lp-es_vscode.html, __lp-es_vscode.html-language-features, __lp-es_vscode.ini, __lp-es_vscode.ipynb, __lp-es_vscode.jake, __lp-es_vscode.java, __lp-es_vscode.javascript, __lp-es_vscode.json, __lp-es_vscode.json-language-features, __lp-es_vscode.julia, __lp-es_vscode.latex, __lp-es_vscode.less, __lp-es_vscode.log, __lp-es_vscode.lua, __lp-es_vscode.make, __lp-es_vscode.markdown, __lp-es_vscode.markdown-language-features, __lp-es_vscode.markdown-math, __lp-es_vscode.media-preview, __lp-es_vscode.merge-conflict, __lp-es_vscode.mermaid-chat-features, __lp-es_vscode.microsoft-authentication, __lp-es_vscode.npm, __lp-es_vscode.objective-c, __lp-es_vscode.perl, __lp-es_vscode.php, __lp-es_vscode.php-language-features, __lp-es_vscode.powershell, __lp-es_vscode.prompt, __lp-es_vscode.pug, __lp-es_vscode.python, __lp-es_vscode.r, __lp-es_vscode.razor, __lp-es_vscode.references-view, __lp-es_vscode.restructuredtext, __lp-es_vscode.ruby, __lp-es_vscode.rust, __lp-es_vscode.scss, __lp-es_vscode.search-result, __lp-es_vscode.shaderlab, __lp-es_vscode.shellscript, __lp-es_vscode.simple-browser, __lp-es_vscode.sql, __lp-es_vscode.swift, __lp-es_vscode.terminal-suggest, __lp-es_vscode.theme-abyss, __lp-es_vscode.theme-defaults, __lp-es_vscode.theme-kimbie-dark, __lp-es_vscode.theme-monokai, __lp-es_vscode.theme-monokai-dimmed, __lp-es_vscode.theme-quietlight, __lp-es_vscode.theme-red, __lp-es_vscode.theme-solarized-dark, __lp-es_vscode.theme-solarized-light, __lp-es_vscode.theme-tomorrow-night-blue, __lp-es_vscode.tunnel-forwarding, __lp-es_vscode.typescript, __lp-es_vscode.typescript-language-features, __lp-es_vscode.vb, __lp-es_vscode.vscode-theme-seti, __lp-es_vscode.xml, __lp-es_vscode.yaml, español, lp-es

**Works With:**

All Platforms

---

### 6. [Portuguese (Brazil) Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-pt-BR)
*Extension #147 of 329*

Language pack extension for Portuguese (Brazil)

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-pt-BR`
- **Version:** 1.107.2025120309
- **Installs:** 8.36M
- **Rating:** ⭐ 4.7/5 (32 ratings)
- **Published:** 2018-02-21
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-pt-BR

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-pt-br_ms-vscode.js-debug, __lp-pt-br_vscode, __lp-pt-br_vscode.bat, __lp-pt-br_vscode.builtin-notebook-renderers, __lp-pt-br_vscode.clojure, __lp-pt-br_vscode.coffeescript, __lp-pt-br_vscode.configuration-editing, __lp-pt-br_vscode.cpp, __lp-pt-br_vscode.csharp, __lp-pt-br_vscode.css, __lp-pt-br_vscode.css-language-features, __lp-pt-br_vscode.dart, __lp-pt-br_vscode.debug-auto-launch, __lp-pt-br_vscode.debug-server-ready, __lp-pt-br_vscode.diff, __lp-pt-br_vscode.docker, __lp-pt-br_vscode.dotenv, __lp-pt-br_vscode.emmet, __lp-pt-br_vscode.extension-editing, __lp-pt-br_vscode.fsharp, __lp-pt-br_vscode.git, __lp-pt-br_vscode.git-base, __lp-pt-br_vscode.github, __lp-pt-br_vscode.github-authentication, __lp-pt-br_vscode.go, __lp-pt-br_vscode.groovy, __lp-pt-br_vscode.grunt, __lp-pt-br_vscode.gulp, __lp-pt-br_vscode.handlebars, __lp-pt-br_vscode.hlsl, __lp-pt-br_vscode.html, __lp-pt-br_vscode.html-language-features, __lp-pt-br_vscode.ini, __lp-pt-br_vscode.ipynb, __lp-pt-br_vscode.jake, __lp-pt-br_vscode.java, __lp-pt-br_vscode.javascript, __lp-pt-br_vscode.json, __lp-pt-br_vscode.json-language-features, __lp-pt-br_vscode.julia, __lp-pt-br_vscode.latex, __lp-pt-br_vscode.less, __lp-pt-br_vscode.log, __lp-pt-br_vscode.lua, __lp-pt-br_vscode.make, __lp-pt-br_vscode.markdown, __lp-pt-br_vscode.markdown-language-features, __lp-pt-br_vscode.markdown-math, __lp-pt-br_vscode.media-preview, __lp-pt-br_vscode.merge-conflict, __lp-pt-br_vscode.mermaid-chat-features, __lp-pt-br_vscode.microsoft-authentication, __lp-pt-br_vscode.npm, __lp-pt-br_vscode.objective-c, __lp-pt-br_vscode.perl, __lp-pt-br_vscode.php, __lp-pt-br_vscode.php-language-features, __lp-pt-br_vscode.powershell, __lp-pt-br_vscode.prompt, __lp-pt-br_vscode.pug, __lp-pt-br_vscode.python, __lp-pt-br_vscode.r, __lp-pt-br_vscode.razor, __lp-pt-br_vscode.references-view, __lp-pt-br_vscode.restructuredtext, __lp-pt-br_vscode.ruby, __lp-pt-br_vscode.rust, __lp-pt-br_vscode.scss, __lp-pt-br_vscode.search-result, __lp-pt-br_vscode.shaderlab, __lp-pt-br_vscode.shellscript, __lp-pt-br_vscode.simple-browser, __lp-pt-br_vscode.sql, __lp-pt-br_vscode.swift, __lp-pt-br_vscode.terminal-suggest, __lp-pt-br_vscode.theme-abyss, __lp-pt-br_vscode.theme-defaults, __lp-pt-br_vscode.theme-kimbie-dark, __lp-pt-br_vscode.theme-monokai, __lp-pt-br_vscode.theme-monokai-dimmed, __lp-pt-br_vscode.theme-quietlight, __lp-pt-br_vscode.theme-red, __lp-pt-br_vscode.theme-solarized-dark, __lp-pt-br_vscode.theme-solarized-light, __lp-pt-br_vscode.theme-tomorrow-night-blue, __lp-pt-br_vscode.tunnel-forwarding, __lp-pt-br_vscode.typescript, __lp-pt-br_vscode.typescript-language-features, __lp-pt-br_vscode.vb, __lp-pt-br_vscode.vscode-theme-seti, __lp-pt-br_vscode.xml, __lp-pt-br_vscode.yaml, lp-pt-br, português

**Works With:**

All Platforms

---

### 7. [Russian Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-ru)
*Extension #148 of 329*

Language pack extension for Russian

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-ru`
- **Version:** 1.107.2025120309
- **Installs:** 8.06M
- **Rating:** ⭐ 4.7/5 (29 ratings)
- **Published:** 2018-05-10
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-ru

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-ru_ms-vscode.js-debug, __lp-ru_vscode, __lp-ru_vscode.bat, __lp-ru_vscode.builtin-notebook-renderers, __lp-ru_vscode.clojure, __lp-ru_vscode.coffeescript, __lp-ru_vscode.configuration-editing, __lp-ru_vscode.cpp, __lp-ru_vscode.csharp, __lp-ru_vscode.css, __lp-ru_vscode.css-language-features, __lp-ru_vscode.dart, __lp-ru_vscode.debug-auto-launch, __lp-ru_vscode.debug-server-ready, __lp-ru_vscode.diff, __lp-ru_vscode.docker, __lp-ru_vscode.dotenv, __lp-ru_vscode.emmet, __lp-ru_vscode.extension-editing, __lp-ru_vscode.fsharp, __lp-ru_vscode.git, __lp-ru_vscode.git-base, __lp-ru_vscode.github, __lp-ru_vscode.github-authentication, __lp-ru_vscode.go, __lp-ru_vscode.groovy, __lp-ru_vscode.grunt, __lp-ru_vscode.gulp, __lp-ru_vscode.handlebars, __lp-ru_vscode.hlsl, __lp-ru_vscode.html, __lp-ru_vscode.html-language-features, __lp-ru_vscode.ini, __lp-ru_vscode.ipynb, __lp-ru_vscode.jake, __lp-ru_vscode.java, __lp-ru_vscode.javascript, __lp-ru_vscode.json, __lp-ru_vscode.json-language-features, __lp-ru_vscode.julia, __lp-ru_vscode.latex, __lp-ru_vscode.less, __lp-ru_vscode.log, __lp-ru_vscode.lua, __lp-ru_vscode.make, __lp-ru_vscode.markdown, __lp-ru_vscode.markdown-language-features, __lp-ru_vscode.markdown-math, __lp-ru_vscode.media-preview, __lp-ru_vscode.merge-conflict, __lp-ru_vscode.mermaid-chat-features, __lp-ru_vscode.microsoft-authentication, __lp-ru_vscode.npm, __lp-ru_vscode.objective-c, __lp-ru_vscode.perl, __lp-ru_vscode.php, __lp-ru_vscode.php-language-features, __lp-ru_vscode.powershell, __lp-ru_vscode.prompt, __lp-ru_vscode.pug, __lp-ru_vscode.python, __lp-ru_vscode.r, __lp-ru_vscode.razor, __lp-ru_vscode.references-view, __lp-ru_vscode.restructuredtext, __lp-ru_vscode.ruby, __lp-ru_vscode.rust, __lp-ru_vscode.scss, __lp-ru_vscode.search-result, __lp-ru_vscode.shaderlab, __lp-ru_vscode.shellscript, __lp-ru_vscode.simple-browser, __lp-ru_vscode.sql, __lp-ru_vscode.swift, __lp-ru_vscode.terminal-suggest, __lp-ru_vscode.theme-abyss, __lp-ru_vscode.theme-defaults, __lp-ru_vscode.theme-kimbie-dark, __lp-ru_vscode.theme-monokai, __lp-ru_vscode.theme-monokai-dimmed, __lp-ru_vscode.theme-quietlight, __lp-ru_vscode.theme-red, __lp-ru_vscode.theme-solarized-dark, __lp-ru_vscode.theme-solarized-light, __lp-ru_vscode.theme-tomorrow-night-blue, __lp-ru_vscode.tunnel-forwarding, __lp-ru_vscode.typescript, __lp-ru_vscode.typescript-language-features, __lp-ru_vscode.vb, __lp-ru_vscode.vscode-theme-seti, __lp-ru_vscode.xml, __lp-ru_vscode.yaml, lp-ru, русский

**Works With:**

All Platforms

---

### 8. [Korean Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-ko)
*Extension #149 of 329*

Language pack extension for Korean

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-ko`
- **Version:** 1.107.2025120309
- **Installs:** 6.11M
- **Rating:** ⭐ 4.3/5 (12 ratings)
- **Published:** 2018-05-08
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-ko

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-ko_ms-vscode.js-debug, __lp-ko_vscode, __lp-ko_vscode.bat, __lp-ko_vscode.builtin-notebook-renderers, __lp-ko_vscode.clojure, __lp-ko_vscode.coffeescript, __lp-ko_vscode.configuration-editing, __lp-ko_vscode.cpp, __lp-ko_vscode.csharp, __lp-ko_vscode.css, __lp-ko_vscode.css-language-features, __lp-ko_vscode.dart, __lp-ko_vscode.debug-auto-launch, __lp-ko_vscode.debug-server-ready, __lp-ko_vscode.diff, __lp-ko_vscode.docker, __lp-ko_vscode.dotenv, __lp-ko_vscode.emmet, __lp-ko_vscode.extension-editing, __lp-ko_vscode.fsharp, __lp-ko_vscode.git, __lp-ko_vscode.git-base, __lp-ko_vscode.github, __lp-ko_vscode.github-authentication, __lp-ko_vscode.go, __lp-ko_vscode.groovy, __lp-ko_vscode.grunt, __lp-ko_vscode.gulp, __lp-ko_vscode.handlebars, __lp-ko_vscode.hlsl, __lp-ko_vscode.html, __lp-ko_vscode.html-language-features, __lp-ko_vscode.ini, __lp-ko_vscode.ipynb, __lp-ko_vscode.jake, __lp-ko_vscode.java, __lp-ko_vscode.javascript, __lp-ko_vscode.json, __lp-ko_vscode.json-language-features, __lp-ko_vscode.julia, __lp-ko_vscode.latex, __lp-ko_vscode.less, __lp-ko_vscode.log, __lp-ko_vscode.lua, __lp-ko_vscode.make, __lp-ko_vscode.markdown, __lp-ko_vscode.markdown-language-features, __lp-ko_vscode.markdown-math, __lp-ko_vscode.media-preview, __lp-ko_vscode.merge-conflict, __lp-ko_vscode.mermaid-chat-features, __lp-ko_vscode.microsoft-authentication, __lp-ko_vscode.npm, __lp-ko_vscode.objective-c, __lp-ko_vscode.perl, __lp-ko_vscode.php, __lp-ko_vscode.php-language-features, __lp-ko_vscode.powershell, __lp-ko_vscode.prompt, __lp-ko_vscode.pug, __lp-ko_vscode.python, __lp-ko_vscode.r, __lp-ko_vscode.razor, __lp-ko_vscode.references-view, __lp-ko_vscode.restructuredtext, __lp-ko_vscode.ruby, __lp-ko_vscode.rust, __lp-ko_vscode.scss, __lp-ko_vscode.search-result, __lp-ko_vscode.shaderlab, __lp-ko_vscode.shellscript, __lp-ko_vscode.simple-browser, __lp-ko_vscode.sql, __lp-ko_vscode.swift, __lp-ko_vscode.terminal-suggest, __lp-ko_vscode.theme-abyss, __lp-ko_vscode.theme-defaults, __lp-ko_vscode.theme-kimbie-dark, __lp-ko_vscode.theme-monokai, __lp-ko_vscode.theme-monokai-dimmed, __lp-ko_vscode.theme-quietlight, __lp-ko_vscode.theme-red, __lp-ko_vscode.theme-solarized-dark, __lp-ko_vscode.theme-solarized-light, __lp-ko_vscode.theme-tomorrow-night-blue, __lp-ko_vscode.tunnel-forwarding, __lp-ko_vscode.typescript, __lp-ko_vscode.typescript-language-features, __lp-ko_vscode.vb, __lp-ko_vscode.vscode-theme-seti, __lp-ko_vscode.xml, __lp-ko_vscode.yaml, lp-ko, 한국어

**Works With:**

All Platforms

---

### 9. [French Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-fr)
*Extension #150 of 329*

Language pack extension for French

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-fr`
- **Version:** 1.107.2025120309
- **Installs:** 5.81M
- **Rating:** ⭐ 4.2/5 (10 ratings)
- **Published:** 2018-05-10
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-fr

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-fr_ms-vscode.js-debug, __lp-fr_vscode, __lp-fr_vscode.bat, __lp-fr_vscode.builtin-notebook-renderers, __lp-fr_vscode.clojure, __lp-fr_vscode.coffeescript, __lp-fr_vscode.configuration-editing, __lp-fr_vscode.cpp, __lp-fr_vscode.csharp, __lp-fr_vscode.css, __lp-fr_vscode.css-language-features, __lp-fr_vscode.dart, __lp-fr_vscode.debug-auto-launch, __lp-fr_vscode.debug-server-ready, __lp-fr_vscode.diff, __lp-fr_vscode.docker, __lp-fr_vscode.dotenv, __lp-fr_vscode.emmet, __lp-fr_vscode.extension-editing, __lp-fr_vscode.fsharp, __lp-fr_vscode.git, __lp-fr_vscode.git-base, __lp-fr_vscode.github, __lp-fr_vscode.github-authentication, __lp-fr_vscode.go, __lp-fr_vscode.groovy, __lp-fr_vscode.grunt, __lp-fr_vscode.gulp, __lp-fr_vscode.handlebars, __lp-fr_vscode.hlsl, __lp-fr_vscode.html, __lp-fr_vscode.html-language-features, __lp-fr_vscode.ini, __lp-fr_vscode.ipynb, __lp-fr_vscode.jake, __lp-fr_vscode.java, __lp-fr_vscode.javascript, __lp-fr_vscode.json, __lp-fr_vscode.json-language-features, __lp-fr_vscode.julia, __lp-fr_vscode.latex, __lp-fr_vscode.less, __lp-fr_vscode.log, __lp-fr_vscode.lua, __lp-fr_vscode.make, __lp-fr_vscode.markdown, __lp-fr_vscode.markdown-language-features, __lp-fr_vscode.markdown-math, __lp-fr_vscode.media-preview, __lp-fr_vscode.merge-conflict, __lp-fr_vscode.mermaid-chat-features, __lp-fr_vscode.microsoft-authentication, __lp-fr_vscode.npm, __lp-fr_vscode.objective-c, __lp-fr_vscode.perl, __lp-fr_vscode.php, __lp-fr_vscode.php-language-features, __lp-fr_vscode.powershell, __lp-fr_vscode.prompt, __lp-fr_vscode.pug, __lp-fr_vscode.python, __lp-fr_vscode.r, __lp-fr_vscode.razor, __lp-fr_vscode.references-view, __lp-fr_vscode.restructuredtext, __lp-fr_vscode.ruby, __lp-fr_vscode.rust, __lp-fr_vscode.scss, __lp-fr_vscode.search-result, __lp-fr_vscode.shaderlab, __lp-fr_vscode.shellscript, __lp-fr_vscode.simple-browser, __lp-fr_vscode.sql, __lp-fr_vscode.swift, __lp-fr_vscode.terminal-suggest, __lp-fr_vscode.theme-abyss, __lp-fr_vscode.theme-defaults, __lp-fr_vscode.theme-kimbie-dark, __lp-fr_vscode.theme-monokai, __lp-fr_vscode.theme-monokai-dimmed, __lp-fr_vscode.theme-quietlight, __lp-fr_vscode.theme-red, __lp-fr_vscode.theme-solarized-dark, __lp-fr_vscode.theme-solarized-light, __lp-fr_vscode.theme-tomorrow-night-blue, __lp-fr_vscode.tunnel-forwarding, __lp-fr_vscode.typescript, __lp-fr_vscode.typescript-language-features, __lp-fr_vscode.vb, __lp-fr_vscode.vscode-theme-seti, __lp-fr_vscode.xml, __lp-fr_vscode.yaml, français, lp-fr

**Works With:**

All Platforms

---

### 10. [German Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-de)
*Extension #151 of 329*

Language pack extension for German

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-de`
- **Version:** 1.107.2025120309
- **Installs:** 5.06M
- **Rating:** ⭐ 4/5 (6 ratings)
- **Published:** 2018-04-23
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-de

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-de_ms-vscode.js-debug, __lp-de_vscode, __lp-de_vscode.bat, __lp-de_vscode.builtin-notebook-renderers, __lp-de_vscode.clojure, __lp-de_vscode.coffeescript, __lp-de_vscode.configuration-editing, __lp-de_vscode.cpp, __lp-de_vscode.csharp, __lp-de_vscode.css, __lp-de_vscode.css-language-features, __lp-de_vscode.dart, __lp-de_vscode.debug-auto-launch, __lp-de_vscode.debug-server-ready, __lp-de_vscode.diff, __lp-de_vscode.docker, __lp-de_vscode.dotenv, __lp-de_vscode.emmet, __lp-de_vscode.extension-editing, __lp-de_vscode.fsharp, __lp-de_vscode.git, __lp-de_vscode.git-base, __lp-de_vscode.github, __lp-de_vscode.github-authentication, __lp-de_vscode.go, __lp-de_vscode.groovy, __lp-de_vscode.grunt, __lp-de_vscode.gulp, __lp-de_vscode.handlebars, __lp-de_vscode.hlsl, __lp-de_vscode.html, __lp-de_vscode.html-language-features, __lp-de_vscode.ini, __lp-de_vscode.ipynb, __lp-de_vscode.jake, __lp-de_vscode.java, __lp-de_vscode.javascript, __lp-de_vscode.json, __lp-de_vscode.json-language-features, __lp-de_vscode.julia, __lp-de_vscode.latex, __lp-de_vscode.less, __lp-de_vscode.log, __lp-de_vscode.lua, __lp-de_vscode.make, __lp-de_vscode.markdown, __lp-de_vscode.markdown-language-features, __lp-de_vscode.markdown-math, __lp-de_vscode.media-preview, __lp-de_vscode.merge-conflict, __lp-de_vscode.mermaid-chat-features, __lp-de_vscode.microsoft-authentication, __lp-de_vscode.npm, __lp-de_vscode.objective-c, __lp-de_vscode.perl, __lp-de_vscode.php, __lp-de_vscode.php-language-features, __lp-de_vscode.powershell, __lp-de_vscode.prompt, __lp-de_vscode.pug, __lp-de_vscode.python, __lp-de_vscode.r, __lp-de_vscode.razor, __lp-de_vscode.references-view, __lp-de_vscode.restructuredtext, __lp-de_vscode.ruby, __lp-de_vscode.rust, __lp-de_vscode.scss, __lp-de_vscode.search-result, __lp-de_vscode.shaderlab, __lp-de_vscode.shellscript, __lp-de_vscode.simple-browser, __lp-de_vscode.sql, __lp-de_vscode.swift, __lp-de_vscode.terminal-suggest, __lp-de_vscode.theme-abyss, __lp-de_vscode.theme-defaults, __lp-de_vscode.theme-kimbie-dark, __lp-de_vscode.theme-monokai, __lp-de_vscode.theme-monokai-dimmed, __lp-de_vscode.theme-quietlight, __lp-de_vscode.theme-red, __lp-de_vscode.theme-solarized-dark, __lp-de_vscode.theme-solarized-light, __lp-de_vscode.theme-tomorrow-night-blue, __lp-de_vscode.tunnel-forwarding, __lp-de_vscode.typescript, __lp-de_vscode.typescript-language-features, __lp-de_vscode.vb, __lp-de_vscode.vscode-theme-seti, __lp-de_vscode.xml, __lp-de_vscode.yaml, Deutsch, lp-de

**Works With:**

All Platforms

---

### 11. [Chinese (Traditional) Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-zh-hant)
*Extension #152 of 329*

Language pack extension for Chinese (Traditional)

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-zh-hant`
- **Version:** 1.107.2025120309
- **Installs:** 4.36M
- **Rating:** ⭐ 4.9/5 (28 ratings)
- **Published:** 2018-05-08
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-zh-hant

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-zh-tw_ms-vscode.js-debug, __lp-zh-tw_vscode, __lp-zh-tw_vscode.bat, __lp-zh-tw_vscode.builtin-notebook-renderers, __lp-zh-tw_vscode.clojure, __lp-zh-tw_vscode.coffeescript, __lp-zh-tw_vscode.configuration-editing, __lp-zh-tw_vscode.cpp, __lp-zh-tw_vscode.csharp, __lp-zh-tw_vscode.css, __lp-zh-tw_vscode.css-language-features, __lp-zh-tw_vscode.dart, __lp-zh-tw_vscode.debug-auto-launch, __lp-zh-tw_vscode.debug-server-ready, __lp-zh-tw_vscode.diff, __lp-zh-tw_vscode.docker, __lp-zh-tw_vscode.dotenv, __lp-zh-tw_vscode.emmet, __lp-zh-tw_vscode.extension-editing, __lp-zh-tw_vscode.fsharp, __lp-zh-tw_vscode.git, __lp-zh-tw_vscode.git-base, __lp-zh-tw_vscode.github, __lp-zh-tw_vscode.github-authentication, __lp-zh-tw_vscode.go, __lp-zh-tw_vscode.groovy, __lp-zh-tw_vscode.grunt, __lp-zh-tw_vscode.gulp, __lp-zh-tw_vscode.handlebars, __lp-zh-tw_vscode.hlsl, __lp-zh-tw_vscode.html, __lp-zh-tw_vscode.html-language-features, __lp-zh-tw_vscode.ini, __lp-zh-tw_vscode.ipynb, __lp-zh-tw_vscode.jake, __lp-zh-tw_vscode.java, __lp-zh-tw_vscode.javascript, __lp-zh-tw_vscode.json, __lp-zh-tw_vscode.json-language-features, __lp-zh-tw_vscode.julia, __lp-zh-tw_vscode.latex, __lp-zh-tw_vscode.less, __lp-zh-tw_vscode.log, __lp-zh-tw_vscode.lua, __lp-zh-tw_vscode.make, __lp-zh-tw_vscode.markdown, __lp-zh-tw_vscode.markdown-language-features, __lp-zh-tw_vscode.markdown-math, __lp-zh-tw_vscode.media-preview, __lp-zh-tw_vscode.merge-conflict, __lp-zh-tw_vscode.mermaid-chat-features, __lp-zh-tw_vscode.microsoft-authentication, __lp-zh-tw_vscode.npm, __lp-zh-tw_vscode.objective-c, __lp-zh-tw_vscode.perl, __lp-zh-tw_vscode.php, __lp-zh-tw_vscode.php-language-features, __lp-zh-tw_vscode.powershell, __lp-zh-tw_vscode.prompt, __lp-zh-tw_vscode.pug, __lp-zh-tw_vscode.python, __lp-zh-tw_vscode.r, __lp-zh-tw_vscode.razor, __lp-zh-tw_vscode.references-view, __lp-zh-tw_vscode.restructuredtext, __lp-zh-tw_vscode.ruby, __lp-zh-tw_vscode.rust, __lp-zh-tw_vscode.scss, __lp-zh-tw_vscode.search-result, __lp-zh-tw_vscode.shaderlab, __lp-zh-tw_vscode.shellscript, __lp-zh-tw_vscode.simple-browser, __lp-zh-tw_vscode.sql, __lp-zh-tw_vscode.swift, __lp-zh-tw_vscode.terminal-suggest, __lp-zh-tw_vscode.theme-abyss, __lp-zh-tw_vscode.theme-defaults, __lp-zh-tw_vscode.theme-kimbie-dark, __lp-zh-tw_vscode.theme-monokai, __lp-zh-tw_vscode.theme-monokai-dimmed, __lp-zh-tw_vscode.theme-quietlight, __lp-zh-tw_vscode.theme-red, __lp-zh-tw_vscode.theme-solarized-dark, __lp-zh-tw_vscode.theme-solarized-light, __lp-zh-tw_vscode.theme-tomorrow-night-blue, __lp-zh-tw_vscode.tunnel-forwarding, __lp-zh-tw_vscode.typescript, __lp-zh-tw_vscode.typescript-language-features, __lp-zh-tw_vscode.vb, __lp-zh-tw_vscode.vscode-theme-seti, __lp-zh-tw_vscode.xml, __lp-zh-tw_vscode.yaml, lp-zh-tw, 中文(繁體)

**Works With:**

All Platforms

---

### 12. [Italian Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-it)
*Extension #153 of 329*

Language pack extension for Italian

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-it`
- **Version:** 1.107.2025120309
- **Installs:** 3.17M
- **Rating:** ⭐ 4.5/5 (2 ratings)
- **Published:** 2018-05-10
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-it

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-it_ms-vscode.js-debug, __lp-it_vscode, __lp-it_vscode.bat, __lp-it_vscode.builtin-notebook-renderers, __lp-it_vscode.clojure, __lp-it_vscode.coffeescript, __lp-it_vscode.configuration-editing, __lp-it_vscode.cpp, __lp-it_vscode.csharp, __lp-it_vscode.css, __lp-it_vscode.css-language-features, __lp-it_vscode.dart, __lp-it_vscode.debug-auto-launch, __lp-it_vscode.debug-server-ready, __lp-it_vscode.diff, __lp-it_vscode.docker, __lp-it_vscode.dotenv, __lp-it_vscode.emmet, __lp-it_vscode.extension-editing, __lp-it_vscode.fsharp, __lp-it_vscode.git, __lp-it_vscode.git-base, __lp-it_vscode.github, __lp-it_vscode.github-authentication, __lp-it_vscode.go, __lp-it_vscode.groovy, __lp-it_vscode.grunt, __lp-it_vscode.gulp, __lp-it_vscode.handlebars, __lp-it_vscode.hlsl, __lp-it_vscode.html, __lp-it_vscode.html-language-features, __lp-it_vscode.ini, __lp-it_vscode.ipynb, __lp-it_vscode.jake, __lp-it_vscode.java, __lp-it_vscode.javascript, __lp-it_vscode.json, __lp-it_vscode.json-language-features, __lp-it_vscode.julia, __lp-it_vscode.latex, __lp-it_vscode.less, __lp-it_vscode.log, __lp-it_vscode.lua, __lp-it_vscode.make, __lp-it_vscode.markdown, __lp-it_vscode.markdown-language-features, __lp-it_vscode.markdown-math, __lp-it_vscode.media-preview, __lp-it_vscode.merge-conflict, __lp-it_vscode.mermaid-chat-features, __lp-it_vscode.microsoft-authentication, __lp-it_vscode.npm, __lp-it_vscode.objective-c, __lp-it_vscode.perl, __lp-it_vscode.php, __lp-it_vscode.php-language-features, __lp-it_vscode.powershell, __lp-it_vscode.prompt, __lp-it_vscode.pug, __lp-it_vscode.python, __lp-it_vscode.r, __lp-it_vscode.razor, __lp-it_vscode.references-view, __lp-it_vscode.restructuredtext, __lp-it_vscode.ruby, __lp-it_vscode.rust, __lp-it_vscode.scss, __lp-it_vscode.search-result, __lp-it_vscode.shaderlab, __lp-it_vscode.shellscript, __lp-it_vscode.simple-browser, __lp-it_vscode.sql, __lp-it_vscode.swift, __lp-it_vscode.terminal-suggest, __lp-it_vscode.theme-abyss, __lp-it_vscode.theme-defaults, __lp-it_vscode.theme-kimbie-dark, __lp-it_vscode.theme-monokai, __lp-it_vscode.theme-monokai-dimmed, __lp-it_vscode.theme-quietlight, __lp-it_vscode.theme-red, __lp-it_vscode.theme-solarized-dark, __lp-it_vscode.theme-solarized-light, __lp-it_vscode.theme-tomorrow-night-blue, __lp-it_vscode.tunnel-forwarding, __lp-it_vscode.typescript, __lp-it_vscode.typescript-language-features, __lp-it_vscode.vb, __lp-it_vscode.vscode-theme-seti, __lp-it_vscode.xml, __lp-it_vscode.yaml, italiano, lp-it

**Works With:**

All Platforms

---

### 13. [Polish Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-pl)
*Extension #154 of 329*

Language pack extension for Polish

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-pl`
- **Version:** 1.107.2025120309
- **Installs:** 2.72M
- **Rating:** ⭐ 5/5 (5 ratings)
- **Published:** 2020-05-20
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-pl

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-pl_ms-vscode.js-debug, __lp-pl_vscode, __lp-pl_vscode.bat, __lp-pl_vscode.builtin-notebook-renderers, __lp-pl_vscode.clojure, __lp-pl_vscode.coffeescript, __lp-pl_vscode.configuration-editing, __lp-pl_vscode.cpp, __lp-pl_vscode.csharp, __lp-pl_vscode.css, __lp-pl_vscode.css-language-features, __lp-pl_vscode.dart, __lp-pl_vscode.debug-auto-launch, __lp-pl_vscode.debug-server-ready, __lp-pl_vscode.diff, __lp-pl_vscode.docker, __lp-pl_vscode.dotenv, __lp-pl_vscode.emmet, __lp-pl_vscode.extension-editing, __lp-pl_vscode.fsharp, __lp-pl_vscode.git, __lp-pl_vscode.git-base, __lp-pl_vscode.github, __lp-pl_vscode.github-authentication, __lp-pl_vscode.go, __lp-pl_vscode.groovy, __lp-pl_vscode.grunt, __lp-pl_vscode.gulp, __lp-pl_vscode.handlebars, __lp-pl_vscode.hlsl, __lp-pl_vscode.html, __lp-pl_vscode.html-language-features, __lp-pl_vscode.ini, __lp-pl_vscode.ipynb, __lp-pl_vscode.jake, __lp-pl_vscode.java, __lp-pl_vscode.javascript, __lp-pl_vscode.json, __lp-pl_vscode.json-language-features, __lp-pl_vscode.julia, __lp-pl_vscode.latex, __lp-pl_vscode.less, __lp-pl_vscode.log, __lp-pl_vscode.lua, __lp-pl_vscode.make, __lp-pl_vscode.markdown, __lp-pl_vscode.markdown-language-features, __lp-pl_vscode.markdown-math, __lp-pl_vscode.media-preview, __lp-pl_vscode.merge-conflict, __lp-pl_vscode.mermaid-chat-features, __lp-pl_vscode.microsoft-authentication, __lp-pl_vscode.npm, __lp-pl_vscode.objective-c, __lp-pl_vscode.perl, __lp-pl_vscode.php, __lp-pl_vscode.php-language-features, __lp-pl_vscode.powershell, __lp-pl_vscode.prompt, __lp-pl_vscode.pug, __lp-pl_vscode.python, __lp-pl_vscode.r, __lp-pl_vscode.razor, __lp-pl_vscode.references-view, __lp-pl_vscode.restructuredtext, __lp-pl_vscode.ruby, __lp-pl_vscode.rust, __lp-pl_vscode.scss, __lp-pl_vscode.search-result, __lp-pl_vscode.shaderlab, __lp-pl_vscode.shellscript, __lp-pl_vscode.simple-browser, __lp-pl_vscode.sql, __lp-pl_vscode.swift, __lp-pl_vscode.terminal-suggest, __lp-pl_vscode.theme-abyss, __lp-pl_vscode.theme-defaults, __lp-pl_vscode.theme-kimbie-dark, __lp-pl_vscode.theme-monokai, __lp-pl_vscode.theme-monokai-dimmed, __lp-pl_vscode.theme-quietlight, __lp-pl_vscode.theme-red, __lp-pl_vscode.theme-solarized-dark, __lp-pl_vscode.theme-solarized-light, __lp-pl_vscode.theme-tomorrow-night-blue, __lp-pl_vscode.tunnel-forwarding, __lp-pl_vscode.typescript, __lp-pl_vscode.typescript-language-features, __lp-pl_vscode.vb, __lp-pl_vscode.vscode-theme-seti, __lp-pl_vscode.xml, __lp-pl_vscode.yaml, lp-pl, polski

**Works With:**

All Platforms

---

### 14. [Czech Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-cs)
*Extension #155 of 329*

Language pack extension for Czech

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-cs`
- **Version:** 1.107.2025120309
- **Installs:** 2.17M
- **Rating:** ⭐ 5/5 (4 ratings)
- **Published:** 2020-05-20
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-cs

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-cs_ms-vscode.js-debug, __lp-cs_vscode, __lp-cs_vscode.bat, __lp-cs_vscode.builtin-notebook-renderers, __lp-cs_vscode.clojure, __lp-cs_vscode.coffeescript, __lp-cs_vscode.configuration-editing, __lp-cs_vscode.cpp, __lp-cs_vscode.csharp, __lp-cs_vscode.css, __lp-cs_vscode.css-language-features, __lp-cs_vscode.dart, __lp-cs_vscode.debug-auto-launch, __lp-cs_vscode.debug-server-ready, __lp-cs_vscode.diff, __lp-cs_vscode.docker, __lp-cs_vscode.dotenv, __lp-cs_vscode.emmet, __lp-cs_vscode.extension-editing, __lp-cs_vscode.fsharp, __lp-cs_vscode.git, __lp-cs_vscode.git-base, __lp-cs_vscode.github, __lp-cs_vscode.github-authentication, __lp-cs_vscode.go, __lp-cs_vscode.groovy, __lp-cs_vscode.grunt, __lp-cs_vscode.gulp, __lp-cs_vscode.handlebars, __lp-cs_vscode.hlsl, __lp-cs_vscode.html, __lp-cs_vscode.html-language-features, __lp-cs_vscode.ini, __lp-cs_vscode.ipynb, __lp-cs_vscode.jake, __lp-cs_vscode.java, __lp-cs_vscode.javascript, __lp-cs_vscode.json, __lp-cs_vscode.json-language-features, __lp-cs_vscode.julia, __lp-cs_vscode.latex, __lp-cs_vscode.less, __lp-cs_vscode.log, __lp-cs_vscode.lua, __lp-cs_vscode.make, __lp-cs_vscode.markdown, __lp-cs_vscode.markdown-language-features, __lp-cs_vscode.markdown-math, __lp-cs_vscode.media-preview, __lp-cs_vscode.merge-conflict, __lp-cs_vscode.mermaid-chat-features, __lp-cs_vscode.microsoft-authentication, __lp-cs_vscode.npm, __lp-cs_vscode.objective-c, __lp-cs_vscode.perl, __lp-cs_vscode.php, __lp-cs_vscode.php-language-features, __lp-cs_vscode.powershell, __lp-cs_vscode.prompt, __lp-cs_vscode.pug, __lp-cs_vscode.python, __lp-cs_vscode.r, __lp-cs_vscode.razor, __lp-cs_vscode.references-view, __lp-cs_vscode.restructuredtext, __lp-cs_vscode.ruby, __lp-cs_vscode.rust, __lp-cs_vscode.scss, __lp-cs_vscode.search-result, __lp-cs_vscode.shaderlab, __lp-cs_vscode.shellscript, __lp-cs_vscode.simple-browser, __lp-cs_vscode.sql, __lp-cs_vscode.swift, __lp-cs_vscode.terminal-suggest, __lp-cs_vscode.theme-abyss, __lp-cs_vscode.theme-defaults, __lp-cs_vscode.theme-kimbie-dark, __lp-cs_vscode.theme-monokai, __lp-cs_vscode.theme-monokai-dimmed, __lp-cs_vscode.theme-quietlight, __lp-cs_vscode.theme-red, __lp-cs_vscode.theme-solarized-dark, __lp-cs_vscode.theme-solarized-light, __lp-cs_vscode.theme-tomorrow-night-blue, __lp-cs_vscode.tunnel-forwarding, __lp-cs_vscode.typescript, __lp-cs_vscode.typescript-language-features, __lp-cs_vscode.vb, __lp-cs_vscode.vscode-theme-seti, __lp-cs_vscode.xml, __lp-cs_vscode.yaml, čeština, lp-cs

**Works With:**

All Platforms

---

### 15. [Hungarian Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-hu)
*Extension #156 of 329*

Language pack extension for Hungarian

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-hu`
- **Version:** 1.48.3
- **Installs:** 2M
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2018-02-21
- **Last Updated:** 2020-08-12
- **VS Code Engine:** ^1.48.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-hu

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.node-debug, __lp_ms-vscode.node-debug2, __lp_vscode, __lp_vscode.bat, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.docker, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.github-authentication, __lp_vscode.git-ui, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.image-preview, __lp_vscode.ini, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown-basics, __lp_vscode.markdown-language-features, __lp_vscode.merge-conflict, __lp_vscode.ms-vscode.js-debug-nightly, __lp_vscode.ms-vscode.node-debug, __lp_vscode.ms-vscode.node-debug2, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-seti, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.typescript-basics, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-account, __lp_vscode.vscode-chrome-debug-core, __lp_vscode.xml, __lp_vscode.yaml, __lp-hu_ms-vscode.node-debug, __lp-hu_ms-vscode.node-debug2, __lp-hu_vscode, __lp-hu_vscode.bat, __lp-hu_vscode.clojure, __lp-hu_vscode.coffeescript, __lp-hu_vscode.configuration-editing, __lp-hu_vscode.cpp, __lp-hu_vscode.csharp, __lp-hu_vscode.css, __lp-hu_vscode.css-language-features, __lp-hu_vscode.debug-auto-launch, __lp-hu_vscode.debug-server-ready, __lp-hu_vscode.docker, __lp-hu_vscode.emmet, __lp-hu_vscode.extension-editing, __lp-hu_vscode.fsharp, __lp-hu_vscode.git, __lp-hu_vscode.github-authentication, __lp-hu_vscode.git-ui, __lp-hu_vscode.go, __lp-hu_vscode.groovy, __lp-hu_vscode.grunt, __lp-hu_vscode.gulp, __lp-hu_vscode.handlebars, __lp-hu_vscode.hlsl, __lp-hu_vscode.html, __lp-hu_vscode.html-language-features, __lp-hu_vscode.image-preview, __lp-hu_vscode.ini, __lp-hu_vscode.jake, __lp-hu_vscode.java, __lp-hu_vscode.javascript, __lp-hu_vscode.json, __lp-hu_vscode.json-language-features, __lp-hu_vscode.less, __lp-hu_vscode.log, __lp-hu_vscode.lua, __lp-hu_vscode.make, __lp-hu_vscode.markdown-basics, __lp-hu_vscode.markdown-language-features, __lp-hu_vscode.merge-conflict, __lp-hu_vscode.ms-vscode.js-debug-nightly, __lp-hu_vscode.ms-vscode.node-debug, __lp-hu_vscode.ms-vscode.node-debug2, __lp-hu_vscode.npm, __lp-hu_vscode.objective-c, __lp-hu_vscode.perl, __lp-hu_vscode.php, __lp-hu_vscode.php-language-features, __lp-hu_vscode.powershell, __lp-hu_vscode.pug, __lp-hu_vscode.python, __lp-hu_vscode.r, __lp-hu_vscode.razor, __lp-hu_vscode.ruby, __lp-hu_vscode.rust, __lp-hu_vscode.scss, __lp-hu_vscode.search-result, __lp-hu_vscode.shaderlab, __lp-hu_vscode.shellscript, __lp-hu_vscode.sql, __lp-hu_vscode.swift, __lp-hu_vscode.theme-abyss, __lp-hu_vscode.theme-defaults, __lp-hu_vscode.theme-kimbie-dark, __lp-hu_vscode.theme-monokai, __lp-hu_vscode.theme-monokai-dimmed, __lp-hu_vscode.theme-quietlight, __lp-hu_vscode.theme-red, __lp-hu_vscode.theme-seti, __lp-hu_vscode.theme-solarized-dark, __lp-hu_vscode.theme-solarized-light, __lp-hu_vscode.theme-tomorrow-night-blue, __lp-hu_vscode.typescript-basics, __lp-hu_vscode.typescript-language-features, __lp-hu_vscode.vb, __lp-hu_vscode.vscode-account, __lp-hu_vscode.vscode-chrome-debug-core, __lp-hu_vscode.xml, __lp-hu_vscode.yaml, lp-hu

**Works With:**

All Platforms

---

### 16. [Bulgarian Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-bg)
*Extension #157 of 329*

Language pack extension for Bulgarian.

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-bg`
- **Version:** 1.48.3
- **Installs:** 1.74M
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2018-05-14
- **Last Updated:** 2020-08-12
- **VS Code Engine:** ^1.48.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-bg

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.node-debug, __lp_ms-vscode.node-debug2, __lp_vscode, __lp_vscode.bat, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.docker, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.github-authentication, __lp_vscode.git-ui, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.image-preview, __lp_vscode.ini, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown-basics, __lp_vscode.markdown-language-features, __lp_vscode.merge-conflict, __lp_vscode.ms-vscode.js-debug-nightly, __lp_vscode.ms-vscode.node-debug, __lp_vscode.ms-vscode.node-debug2, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-seti, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.typescript-basics, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-account, __lp_vscode.vscode-chrome-debug-core, __lp_vscode.xml, __lp_vscode.yaml, __lp-bg_ms-vscode.node-debug, __lp-bg_ms-vscode.node-debug2, __lp-bg_vscode, __lp-bg_vscode.bat, __lp-bg_vscode.clojure, __lp-bg_vscode.coffeescript, __lp-bg_vscode.configuration-editing, __lp-bg_vscode.cpp, __lp-bg_vscode.csharp, __lp-bg_vscode.css, __lp-bg_vscode.css-language-features, __lp-bg_vscode.debug-auto-launch, __lp-bg_vscode.debug-server-ready, __lp-bg_vscode.docker, __lp-bg_vscode.emmet, __lp-bg_vscode.extension-editing, __lp-bg_vscode.fsharp, __lp-bg_vscode.git, __lp-bg_vscode.github-authentication, __lp-bg_vscode.git-ui, __lp-bg_vscode.go, __lp-bg_vscode.groovy, __lp-bg_vscode.grunt, __lp-bg_vscode.gulp, __lp-bg_vscode.handlebars, __lp-bg_vscode.hlsl, __lp-bg_vscode.html, __lp-bg_vscode.html-language-features, __lp-bg_vscode.image-preview, __lp-bg_vscode.ini, __lp-bg_vscode.jake, __lp-bg_vscode.java, __lp-bg_vscode.javascript, __lp-bg_vscode.json, __lp-bg_vscode.json-language-features, __lp-bg_vscode.less, __lp-bg_vscode.log, __lp-bg_vscode.lua, __lp-bg_vscode.make, __lp-bg_vscode.markdown-basics, __lp-bg_vscode.markdown-language-features, __lp-bg_vscode.merge-conflict, __lp-bg_vscode.ms-vscode.js-debug-nightly, __lp-bg_vscode.ms-vscode.node-debug, __lp-bg_vscode.ms-vscode.node-debug2, __lp-bg_vscode.npm, __lp-bg_vscode.objective-c, __lp-bg_vscode.perl, __lp-bg_vscode.php, __lp-bg_vscode.php-language-features, __lp-bg_vscode.powershell, __lp-bg_vscode.pug, __lp-bg_vscode.python, __lp-bg_vscode.r, __lp-bg_vscode.razor, __lp-bg_vscode.ruby, __lp-bg_vscode.rust, __lp-bg_vscode.scss, __lp-bg_vscode.search-result, __lp-bg_vscode.shaderlab, __lp-bg_vscode.shellscript, __lp-bg_vscode.sql, __lp-bg_vscode.swift, __lp-bg_vscode.theme-abyss, __lp-bg_vscode.theme-defaults, __lp-bg_vscode.theme-kimbie-dark, __lp-bg_vscode.theme-monokai, __lp-bg_vscode.theme-monokai-dimmed, __lp-bg_vscode.theme-quietlight, __lp-bg_vscode.theme-red, __lp-bg_vscode.theme-seti, __lp-bg_vscode.theme-solarized-dark, __lp-bg_vscode.theme-solarized-light, __lp-bg_vscode.theme-tomorrow-night-blue, __lp-bg_vscode.typescript-basics, __lp-bg_vscode.typescript-language-features, __lp-bg_vscode.vb, __lp-bg_vscode.vscode-account, __lp-bg_vscode.vscode-chrome-debug-core, __lp-bg_vscode.xml, __lp-bg_vscode.yaml, lp-bg

**Works With:**

All Platforms

---

### 17. [Turkish Language Pack for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-tr)
*Extension #158 of 329*

Language pack extension for Turkish

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-tr`
- **Version:** 1.107.2025120309
- **Installs:** 1.01M
- **Rating:** ⭐ 4.7/5 (12 ratings)
- **Published:** 2018-02-21
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-tr

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-tr_ms-vscode.js-debug, __lp-tr_vscode, __lp-tr_vscode.bat, __lp-tr_vscode.builtin-notebook-renderers, __lp-tr_vscode.clojure, __lp-tr_vscode.coffeescript, __lp-tr_vscode.configuration-editing, __lp-tr_vscode.cpp, __lp-tr_vscode.csharp, __lp-tr_vscode.css, __lp-tr_vscode.css-language-features, __lp-tr_vscode.dart, __lp-tr_vscode.debug-auto-launch, __lp-tr_vscode.debug-server-ready, __lp-tr_vscode.diff, __lp-tr_vscode.docker, __lp-tr_vscode.dotenv, __lp-tr_vscode.emmet, __lp-tr_vscode.extension-editing, __lp-tr_vscode.fsharp, __lp-tr_vscode.git, __lp-tr_vscode.git-base, __lp-tr_vscode.github, __lp-tr_vscode.github-authentication, __lp-tr_vscode.go, __lp-tr_vscode.groovy, __lp-tr_vscode.grunt, __lp-tr_vscode.gulp, __lp-tr_vscode.handlebars, __lp-tr_vscode.hlsl, __lp-tr_vscode.html, __lp-tr_vscode.html-language-features, __lp-tr_vscode.ini, __lp-tr_vscode.ipynb, __lp-tr_vscode.jake, __lp-tr_vscode.java, __lp-tr_vscode.javascript, __lp-tr_vscode.json, __lp-tr_vscode.json-language-features, __lp-tr_vscode.julia, __lp-tr_vscode.latex, __lp-tr_vscode.less, __lp-tr_vscode.log, __lp-tr_vscode.lua, __lp-tr_vscode.make, __lp-tr_vscode.markdown, __lp-tr_vscode.markdown-language-features, __lp-tr_vscode.markdown-math, __lp-tr_vscode.media-preview, __lp-tr_vscode.merge-conflict, __lp-tr_vscode.mermaid-chat-features, __lp-tr_vscode.microsoft-authentication, __lp-tr_vscode.npm, __lp-tr_vscode.objective-c, __lp-tr_vscode.perl, __lp-tr_vscode.php, __lp-tr_vscode.php-language-features, __lp-tr_vscode.powershell, __lp-tr_vscode.prompt, __lp-tr_vscode.pug, __lp-tr_vscode.python, __lp-tr_vscode.r, __lp-tr_vscode.razor, __lp-tr_vscode.references-view, __lp-tr_vscode.restructuredtext, __lp-tr_vscode.ruby, __lp-tr_vscode.rust, __lp-tr_vscode.scss, __lp-tr_vscode.search-result, __lp-tr_vscode.shaderlab, __lp-tr_vscode.shellscript, __lp-tr_vscode.simple-browser, __lp-tr_vscode.sql, __lp-tr_vscode.swift, __lp-tr_vscode.terminal-suggest, __lp-tr_vscode.theme-abyss, __lp-tr_vscode.theme-defaults, __lp-tr_vscode.theme-kimbie-dark, __lp-tr_vscode.theme-monokai, __lp-tr_vscode.theme-monokai-dimmed, __lp-tr_vscode.theme-quietlight, __lp-tr_vscode.theme-red, __lp-tr_vscode.theme-solarized-dark, __lp-tr_vscode.theme-solarized-light, __lp-tr_vscode.theme-tomorrow-night-blue, __lp-tr_vscode.tunnel-forwarding, __lp-tr_vscode.typescript, __lp-tr_vscode.typescript-language-features, __lp-tr_vscode.vb, __lp-tr_vscode.vscode-theme-seti, __lp-tr_vscode.xml, __lp-tr_vscode.yaml, lp-tr, Türkçe

**Works With:**

All Platforms

---

### 18. [Learn Authoring Pack](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-authoring-pack)
*Extension #159 of 329*

Collection of extensions to assist with content development for learn.microsoft.com

**Details:**

- **Extension ID:** `docsmsft.docs-authoring-pack`
- **Version:** 1.0.3
- **Installs:** 118K
- **Rating:** ⭐ 2.9/5 (7 ratings)
- **Published:** 2018-04-05
- **Last Updated:** 2025-04-15
- **VS Code Engine:** ^1.71.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-authoring-pack

**Resources:**

- 📦 [Repository](https://ceapex@dev.azure.com/ceapex/Engineering/_git/LearnAuthoringPack)

**Marketplace Categories:**

Extension Packs

**Tags:**

__web_extension, markdown, Microsoft Learn

**Works With:**

All Platforms

---

### 19. [Chinese (Simplified, China) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-zh-cn)
*Extension #160 of 329*

Chinese (Simplified, China) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-zh-cn`
- **Version:** 0.5.1
- **Installs:** 108K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2024-02-23
- **Last Updated:** 2025-04-16
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-zh-cn

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 20. [Brackets Extension Pack](https://marketplace.visualstudio.com/items?itemName=ms-vscode.brackets-pack)
*Extension #161 of 329*

Popular Brackets features as extensions for VS Code.

**Details:**

- **Extension ID:** `ms-vscode.brackets-pack`
- **Version:** 0.1.1
- **Installs:** 101K
- **Rating:** ⭐ 2.6/5 (5 ratings)
- **Published:** 2020-12-03
- **Last Updated:** 2021-02-26
- **VS Code Engine:** ^1.51.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.brackets-pack

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-brackets-pack.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-brackets-pack/issues)

**Marketplace Categories:**

Extension Packs

**Tags:**

__web_extension, brackets

**Works With:**

All Platforms

---

### 21. [Pseudo Language Language Pack](https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-qps-ploc)
*Extension #162 of 329*

Language pack extension for Pseudo Language

**Details:**

- **Extension ID:** `MS-CEINTL.vscode-language-pack-qps-ploc`
- **Version:** 1.107.2025120309
- **Installs:** 45K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2021-06-08
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.107.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-CEINTL.vscode-language-pack-qps-ploc

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-loc.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-loc/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-loc#readme)

**Marketplace Categories:**

Language Packs

**Tags:**

__lp_ms-vscode.js-debug, __lp_vscode, __lp_vscode.bat, __lp_vscode.builtin-notebook-renderers, __lp_vscode.clojure, __lp_vscode.coffeescript, __lp_vscode.configuration-editing, __lp_vscode.cpp, __lp_vscode.csharp, __lp_vscode.css, __lp_vscode.css-language-features, __lp_vscode.dart, __lp_vscode.debug-auto-launch, __lp_vscode.debug-server-ready, __lp_vscode.diff, __lp_vscode.docker, __lp_vscode.dotenv, __lp_vscode.emmet, __lp_vscode.extension-editing, __lp_vscode.fsharp, __lp_vscode.git, __lp_vscode.git-base, __lp_vscode.github, __lp_vscode.github-authentication, __lp_vscode.go, __lp_vscode.groovy, __lp_vscode.grunt, __lp_vscode.gulp, __lp_vscode.handlebars, __lp_vscode.hlsl, __lp_vscode.html, __lp_vscode.html-language-features, __lp_vscode.ini, __lp_vscode.ipynb, __lp_vscode.jake, __lp_vscode.java, __lp_vscode.javascript, __lp_vscode.json, __lp_vscode.json-language-features, __lp_vscode.julia, __lp_vscode.latex, __lp_vscode.less, __lp_vscode.log, __lp_vscode.lua, __lp_vscode.make, __lp_vscode.markdown, __lp_vscode.markdown-language-features, __lp_vscode.markdown-math, __lp_vscode.media-preview, __lp_vscode.merge-conflict, __lp_vscode.mermaid-chat-features, __lp_vscode.microsoft-authentication, __lp_vscode.npm, __lp_vscode.objective-c, __lp_vscode.perl, __lp_vscode.php, __lp_vscode.php-language-features, __lp_vscode.powershell, __lp_vscode.prompt, __lp_vscode.pug, __lp_vscode.python, __lp_vscode.r, __lp_vscode.razor, __lp_vscode.references-view, __lp_vscode.restructuredtext, __lp_vscode.ruby, __lp_vscode.rust, __lp_vscode.scss, __lp_vscode.search-result, __lp_vscode.shaderlab, __lp_vscode.shellscript, __lp_vscode.simple-browser, __lp_vscode.sql, __lp_vscode.swift, __lp_vscode.terminal-suggest, __lp_vscode.theme-abyss, __lp_vscode.theme-defaults, __lp_vscode.theme-kimbie-dark, __lp_vscode.theme-monokai, __lp_vscode.theme-monokai-dimmed, __lp_vscode.theme-quietlight, __lp_vscode.theme-red, __lp_vscode.theme-solarized-dark, __lp_vscode.theme-solarized-light, __lp_vscode.theme-tomorrow-night-blue, __lp_vscode.tunnel-forwarding, __lp_vscode.typescript, __lp_vscode.typescript-language-features, __lp_vscode.vb, __lp_vscode.vscode-theme-seti, __lp_vscode.xml, __lp_vscode.yaml, __lp-qps-ploc_ms-vscode.js-debug, __lp-qps-ploc_vscode, __lp-qps-ploc_vscode.bat, __lp-qps-ploc_vscode.builtin-notebook-renderers, __lp-qps-ploc_vscode.clojure, __lp-qps-ploc_vscode.coffeescript, __lp-qps-ploc_vscode.configuration-editing, __lp-qps-ploc_vscode.cpp, __lp-qps-ploc_vscode.csharp, __lp-qps-ploc_vscode.css, __lp-qps-ploc_vscode.css-language-features, __lp-qps-ploc_vscode.dart, __lp-qps-ploc_vscode.debug-auto-launch, __lp-qps-ploc_vscode.debug-server-ready, __lp-qps-ploc_vscode.diff, __lp-qps-ploc_vscode.docker, __lp-qps-ploc_vscode.dotenv, __lp-qps-ploc_vscode.emmet, __lp-qps-ploc_vscode.extension-editing, __lp-qps-ploc_vscode.fsharp, __lp-qps-ploc_vscode.git, __lp-qps-ploc_vscode.git-base, __lp-qps-ploc_vscode.github, __lp-qps-ploc_vscode.github-authentication, __lp-qps-ploc_vscode.go, __lp-qps-ploc_vscode.groovy, __lp-qps-ploc_vscode.grunt, __lp-qps-ploc_vscode.gulp, __lp-qps-ploc_vscode.handlebars, __lp-qps-ploc_vscode.hlsl, __lp-qps-ploc_vscode.html, __lp-qps-ploc_vscode.html-language-features, __lp-qps-ploc_vscode.ini, __lp-qps-ploc_vscode.ipynb, __lp-qps-ploc_vscode.jake, __lp-qps-ploc_vscode.java, __lp-qps-ploc_vscode.javascript, __lp-qps-ploc_vscode.json, __lp-qps-ploc_vscode.json-language-features, __lp-qps-ploc_vscode.julia, __lp-qps-ploc_vscode.latex, __lp-qps-ploc_vscode.less, __lp-qps-ploc_vscode.log, __lp-qps-ploc_vscode.lua, __lp-qps-ploc_vscode.make, __lp-qps-ploc_vscode.markdown, __lp-qps-ploc_vscode.markdown-language-features, __lp-qps-ploc_vscode.markdown-math, __lp-qps-ploc_vscode.media-preview, __lp-qps-ploc_vscode.merge-conflict, __lp-qps-ploc_vscode.mermaid-chat-features, __lp-qps-ploc_vscode.microsoft-authentication, __lp-qps-ploc_vscode.npm, __lp-qps-ploc_vscode.objective-c, __lp-qps-ploc_vscode.perl, __lp-qps-ploc_vscode.php, __lp-qps-ploc_vscode.php-language-features, __lp-qps-ploc_vscode.powershell, __lp-qps-ploc_vscode.prompt, __lp-qps-ploc_vscode.pug, __lp-qps-ploc_vscode.python, __lp-qps-ploc_vscode.r, __lp-qps-ploc_vscode.razor, __lp-qps-ploc_vscode.references-view, __lp-qps-ploc_vscode.restructuredtext, __lp-qps-ploc_vscode.ruby, __lp-qps-ploc_vscode.rust, __lp-qps-ploc_vscode.scss, __lp-qps-ploc_vscode.search-result, __lp-qps-ploc_vscode.shaderlab, __lp-qps-ploc_vscode.shellscript, __lp-qps-ploc_vscode.simple-browser, __lp-qps-ploc_vscode.sql, __lp-qps-ploc_vscode.swift, __lp-qps-ploc_vscode.terminal-suggest, __lp-qps-ploc_vscode.theme-abyss, __lp-qps-ploc_vscode.theme-defaults, __lp-qps-ploc_vscode.theme-kimbie-dark, __lp-qps-ploc_vscode.theme-monokai, __lp-qps-ploc_vscode.theme-monokai-dimmed, __lp-qps-ploc_vscode.theme-quietlight, __lp-qps-ploc_vscode.theme-red, __lp-qps-ploc_vscode.theme-solarized-dark, __lp-qps-ploc_vscode.theme-solarized-light, __lp-qps-ploc_vscode.theme-tomorrow-night-blue, __lp-qps-ploc_vscode.tunnel-forwarding, __lp-qps-ploc_vscode.typescript, __lp-qps-ploc_vscode.typescript-language-features, __lp-qps-ploc_vscode.vb, __lp-qps-ploc_vscode.vscode-theme-seti, __lp-qps-ploc_vscode.xml, __lp-qps-ploc_vscode.yaml, lp-qps-ploc, Þƨèúδô £áñϱúáϱè

**Works With:**

All Platforms

---

### 22. [Spanish (Spain) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-es-es)
*Extension #163 of 329*

Spanish (Spain) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-es-es`
- **Version:** 0.5.0
- **Installs:** 23K
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-es-es

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 23. [Portuguese (Brazil) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-pt-br)
*Extension #164 of 329*

Portuguese (Brazil) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-pt-br`
- **Version:** 0.5.0
- **Installs:** 19K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-pt-br

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 24. [French (France) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-fr-fr)
*Extension #165 of 329*

French (France) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-fr-fr`
- **Version:** 0.5.0
- **Installs:** 15K
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-fr-fr

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 25. [Russian (Russia) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-ru-ru)
*Extension #166 of 329*

Russian (Russia) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-ru-ru`
- **Version:** 0.5.0
- **Installs:** 14K
- **Published:** 2024-02-23
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-ru-ru

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 26. [Japanese (Japan) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-ja-jp)
*Extension #167 of 329*

Japanese (Japan) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-ja-jp`
- **Version:** 0.5.0
- **Installs:** 11K
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-ja-jp

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 27. [Microsoft AI Tools Extension Pack](https://marketplace.visualstudio.com/items?itemName=ms-windows-ai-studio.microsoft-ai-tools-pack)
*Extension #168 of 329*

A curated set of essential extensions for building generative AI applications and agents in Visual Studio Code.

**Details:**

- **Extension ID:** `ms-windows-ai-studio.microsoft-ai-tools-pack`
- **Version:** 0.1.0
- **Installs:** 10K
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2025-05-13
- **Last Updated:** 2025-05-14
- **VS Code Engine:** ^1.98.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-windows-ai-studio.microsoft-ai-tools-pack

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-ai-pack.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-ai-pack/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-ai-pack/)

**Marketplace Categories:**

Extension Packs, Azure, Data Science, Machine Learning, AI

**Tags:**

__web_extension, AI, Azure, copilot, data cleaning, data wrangling, generative AI

**Works With:**

All Platforms

---

### 28. [German language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-de-de)
*Extension #169 of 329*

German language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-de-de`
- **Version:** 0.5.0
- **Installs:** 10K
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-de-de

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 29. [Chinese (Traditional, Taiwan) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-zh-tw)
*Extension #170 of 329*

Chinese (Traditional, Taiwan) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-zh-tw`
- **Version:** 0.5.0
- **Installs:** 8K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2024-02-23
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-zh-tw

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 30. [Spanish (Mexico) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-es-mx)
*Extension #171 of 329*

Spanish (Mexico) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-es-mx`
- **Version:** 0.5.0
- **Installs:** 6K
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-es-mx

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 31. [English (United Kingdom) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-gb)
*Extension #172 of 329*

English (United Kingdom) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-en-gb`
- **Version:** 0.5.0
- **Installs:** 5K
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-gb

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 32. [Korean (South Korea) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-ko-kr)
*Extension #173 of 329*

Korean (South Korea) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-ko-kr`
- **Version:** 0.5.0
- **Installs:** 4K
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-ko-kr

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 33. [Italian (Italy) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-it-it)
*Extension #174 of 329*

Italian (Italy) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-it-it`
- **Version:** 0.5.0
- **Installs:** 3K
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-it-it

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 34. [Turkish (Turkey) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-tr-tr)
*Extension #175 of 329*

Turkish (Turkey) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-tr-tr`
- **Version:** 0.5.0
- **Installs:** 3K
- **Published:** 2024-02-23
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-tr-tr

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 35. [Portuguese (Portugal) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-pt-pt)
*Extension #176 of 329*

Portuguese (Portugal) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-pt-pt`
- **Version:** 0.5.0
- **Installs:** 2K
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-pt-pt

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 36. [English (India) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-in)
*Extension #177 of 329*

English (India) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-en-in`
- **Version:** 0.5.0
- **Installs:** 2K
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-in

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 37. [Hindi (India) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-hi-in)
*Extension #178 of 329*

Hindi (India) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-hi-in`
- **Version:** 0.5.0
- **Installs:** 2K
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-hi-in

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 38. [English (Canada) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-ca)
*Extension #179 of 329*

English (Canada) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-en-ca`
- **Version:** 0.5.0
- **Installs:** 900
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-ca

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 39. [French (Canada) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-fr-ca)
*Extension #180 of 329*

French (Canada) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-fr-ca`
- **Version:** 0.5.0
- **Installs:** 791
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-fr-ca

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 40. [English (Australia) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-au)
*Extension #181 of 329*

English (Australia) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-en-au`
- **Version:** 0.5.0
- **Installs:** 738
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-au

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 41. [Dutch (Netherlands) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-nl-nl)
*Extension #182 of 329*

Dutch (Netherlands) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-nl-nl`
- **Version:** 0.5.0
- **Installs:** 727
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-nl-nl

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 42. [Swedish (Sweden) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-sv-se)
*Extension #183 of 329*

Swedish (Sweden) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-sv-se`
- **Version:** 0.5.0
- **Installs:** 571
- **Published:** 2024-02-23
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-sv-se

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 43. [Chinese (Traditional, Hong Kong) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-zh-hk)
*Extension #184 of 329*

Chinese (Traditional, Hong Kong) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-zh-hk`
- **Version:** 0.5.0
- **Installs:** 545
- **Published:** 2024-02-23
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-zh-hk

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 44. [English (Ireland) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-ie)
*Extension #185 of 329*

English (Ireland) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-en-ie`
- **Version:** 0.5.0
- **Installs:** 384
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-ie

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 45. [English (New Zealand) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-nz)
*Extension #186 of 329*

English (New Zealand) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-en-nz`
- **Version:** 0.5.0
- **Installs:** 332
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-en-nz

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

### 46. [Danish (Denmark) language support for VS Code Speech](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-da-dk)
*Extension #187 of 329*

Danish (Denmark) language support for speech-to-text and other voice capabilities in VS Code.

**Details:**

- **Extension ID:** `ms-vscode.vscode-speech-language-pack-da-dk`
- **Version:** 0.5.0
- **Installs:** 286
- **Published:** 2024-02-22
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-speech-language-pack-da-dk

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-speech-assets.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode/issues?q=is%3Aopen+is%3Aissue+label%3Aworkbench-voice)
- 🏠 [Homepage](https://github.com/microsoft/vscode/wiki/VS-Code-Speech)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, a11y, Accessibility, ai, Chat, co-pilot, Microsoft, multi-root ready, STT, Transcription, Voice

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## AI & Machine Learning

**Category Stats:** 31 extensions | Total Installs: 489.16M | Avg Rating: 4.1⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [Jupyter](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter)
*Extension #188 of 329*

Jupyter notebook support, interactive programming and computing that supports Intellisense, debugging and more.

**Details:**

- **Extension ID:** `ms-toolsai.jupyter`
- **Version:** 2025.10.2025101002
- **Installs:** 98.83M
- **Rating:** ⭐ 2.7/5 (350 ratings)
- **Published:** 2020-11-11
- **Last Updated:** 2025-10-10
- **VS Code Engine:** ^1.106.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-jupyter.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-jupyter/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-jupyter)

**Marketplace Categories:**

Extension Packs, Data Science, Machine Learning, Visualization, Notebooks

**Tags:**

__ext_jl, __web_extension, debuggers, interactive, julia, jupyter, keybindings, language-model-tools, multi-root ready, notebook, notebookKernelJupyterNotebook, python, raw, tools

**Works With:**

macOS Intel, macOS Apple Silicon, Alpine Linux ARM64, Alpine Linux x64, Windows ARM64, Windows x64, Linux ARM64, Linux x64, Web, Linux ARM, Windows x86

---

### 2. [Jupyter Keymap](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-keymap)
*Extension #189 of 329*

Jupyter keymaps for notebooks

**Details:**

- **Extension ID:** `ms-toolsai.jupyter-keymap`
- **Version:** 1.1.2
- **Installs:** 78.64M
- **Rating:** ⭐ 4/5 (3 ratings)
- **Published:** 2021-08-20
- **Last Updated:** 2023-06-05
- **VS Code Engine:** ^1.79.0-insider
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-keymap

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-jupyter-keymap.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-jupyter-keymap/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-jupyter-keymap#readme)

**Marketplace Categories:**

Notebooks

**Tags:**

__web_extension, keybindings, notebook-keymap

**Works With:**

All Platforms

---

### 3. [Jupyter Notebook Renderers](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-renderers)
*Extension #190 of 329*

Renderers for Jupyter Notebooks (with plotly, vega, gif, png, svg, jpeg and other such outputs)

**Details:**

- **Extension ID:** `ms-toolsai.jupyter-renderers`
- **Version:** 1.3.2025062701
- **Installs:** 77.74M
- **Rating:** ⭐ 2.8/5 (14 ratings)
- **Published:** 2021-09-21
- **Last Updated:** 2025-06-27
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-renderers

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-notebook-renderers.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-notebook-renderers/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-notebook-renderers)

**Marketplace Categories:**

Data Science, Machine Learning, Visualization, Notebooks

**Tags:**

__web_extension, jupyter, multi-root ready, notebook, notebookRenderer

**Works With:**

All Platforms

---

### 4. [IntelliCode](https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.vscodeintellicode)
*Extension #191 of 329*

AI-assisted development

**Details:**

- **Extension ID:** `VisualStudioExptTeam.vscodeintellicode`
- **Version:** 1.3.2
- **Installs:** 60.28M
- **Rating:** ⭐ 3.9/5 (103 ratings)
- **Published:** 2018-07-24
- **Last Updated:** 2024-10-23
- **VS Code Engine:** ^1.94.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.vscodeintellicode

**Resources:**

- 📦 [Repository](https://github.com/MicrosoftDocs/intellicode.git)
- 🐛 [Issues](https://github.com/MicrosoftDocs/intellicode/issues)
- 💬 [Support](https://go.microsoft.com/fwlink/?linkid=2005855)
- 🏠 [Homepage](https://github.com/MicrosoftDocs/intellicode#readme)

**Marketplace Categories:**

Other

**Tags:**

java, javascript, javascriptreact, python, sql, typescript, typescriptreact

**Works With:**

All Platforms

---

### 5. [Jupyter Cell Tags](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-jupyter-cell-tags)
*Extension #192 of 329*

Jupyter Cell Tags support for VS Code

**Details:**

- **Extension ID:** `ms-toolsai.vscode-jupyter-cell-tags`
- **Version:** 0.1.9
- **Installs:** 59.03M
- **Rating:** ⭐ 4/5 (2 ratings)
- **Published:** 2022-09-21
- **Last Updated:** 2024-04-03
- **VS Code Engine:** ^1.88.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-jupyter-cell-tags

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-jupyter-cell-tags.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-jupyter-cell-tags/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-jupyter-cell-tags#readme)

**Marketplace Categories:**

Notebooks

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 6. [Jupyter Slide Show](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-jupyter-slideshow)
*Extension #193 of 329*

Jupyter Slide Show support for VS Code

**Details:**

- **Extension ID:** `ms-toolsai.vscode-jupyter-slideshow`
- **Version:** 0.1.6
- **Installs:** 58.96M
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2022-09-21
- **Last Updated:** 2024-04-03
- **VS Code Engine:** ^1.88.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-jupyter-slideshow

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-jupyter-slideshow.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-jupyter-slideshow/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-jupyter-slideshow#readme)

**Marketplace Categories:**

Notebooks

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 7. [IntelliCode API Usage Examples](https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.intellicode-api-usage-examples)
*Extension #194 of 329*

See relevant code examples from GitHub for over 100K different APIs right in your editor.

**Details:**

- **Extension ID:** `VisualStudioExptTeam.intellicode-api-usage-examples`
- **Version:** 0.2.9
- **Installs:** 44.6M
- **Rating:** ⭐ 3.8/5 (21 ratings)
- **Published:** 2021-09-10
- **Last Updated:** 2024-10-23
- **VS Code Engine:** ^1.94.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.intellicode-api-usage-examples

**Resources:**

- 📦 [Repository](https://github.com/MicrosoftDocs/intellicode.git)
- 🐛 [Issues](https://github.com/MicrosoftDocs/intellicode/issues)
- 💬 [Support](https://go.microsoft.com/fwlink/?linkid=2005855)
- 🏠 [Homepage](https://github.com/MicrosoftDocs/intellicode#readme)

**Marketplace Categories:**

Other

**Tags:**

javascript, javascriptreact, python, typescript, typescriptreact

**Works With:**

All Platforms

---

### 8. [Azure Machine Learning](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai)
*Extension #195 of 329*

Visual Studio Code extension for Azure Machine Learning

**Details:**

- **Extension ID:** `ms-toolsai.vscode-ai`
- **Version:** 1.5.2025102109
- **Installs:** 2.68M
- **Rating:** ⭐ 4.6/5 (15 ratings)
- **Published:** 2017-09-25
- **Last Updated:** 2025-10-22
- **VS Code Engine:** ^1.105.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-tools-for-ai.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-tools-for-ai/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-tools-for-ai#readme)

**Marketplace Categories:**

Azure

**Tags:**

__ext_eyaml, __ext_eyml, __ext_yaml, __ext_yml, AML, Azure Machine Learning, Azure ML, Deep Learning, keybindings, yaml

**Works With:**

All Platforms

---

### 9. [Azure Machine Learning - Remote](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai-remote)
*Extension #196 of 329*

This extension is used by the Azure Machine Learning Extension

**Details:**

- **Extension ID:** `ms-toolsai.vscode-ai-remote`
- **Version:** 1.5.2025102909
- **Installs:** 2.51M
- **Rating:** ⭐ 3.2/5 (5 ratings)
- **Published:** 2021-02-11
- **Last Updated:** 2025-10-29
- **VS Code Engine:** ^1.105.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai-remote

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-tools-for-ai.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-tools-for-ai/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-tools-for-ai#readme)

**Marketplace Categories:**

Azure

**Tags:**

__web_extension, AML, Azure Machine Learning, Azure ML, Deep Learning

**Works With:**

All Platforms

---

### 10. [Data Wrangler](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.datawrangler)
*Extension #197 of 329*

Data viewing, cleaning and preparation for tabular datasets

**Details:**

- **Extension ID:** `ms-toolsai.datawrangler`
- **Version:** 1.24.0
- **Installs:** 1.61M
- **Rating:** ⭐ 4.5/5 (71 ratings)
- **Published:** 2023-03-16
- **Last Updated:** 2025-11-26
- **VS Code Engine:** ^1.82.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.datawrangler

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-data-wrangler.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-data-wrangler/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-data-wrangler)

**Marketplace Categories:**

Data Science, Machine Learning

**Tags:**

__ext_DataWrangler, __ext_DW, __web_extension, Business Intelligence, charts, code generation, code generator, csv, data, data analysis, Data cleaning, data cleansing, data editor, data exploration, data export, data frame, data import, data prep, data preparation, data quality, Data Science, data summary, data table, data transformation, data viewer, data visualization, data wrangling, dataframe, dataset, data-wrangler, EDA, ETL, excel, Exploratory Data Analysis, graphs, grid, insights, jsonl, Jupyter, Jupyter Notebook, jupyterVariableViewers, keybindings, language-model-tools, numpy, pandas, parquet, plots, power query, python, spreadsheet, Stata, table, tools, tsv, visualization, xls, xlsx

**Works With:**

All Platforms

---

### 11. [Jupyter PowerToys](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-jupyter-powertoys)
*Extension #198 of 329*

Experimental features for Jupyter notebook support in VS Code.

**Details:**

- **Extension ID:** `ms-toolsai.vscode-jupyter-powertoys`
- **Version:** 0.1.1
- **Installs:** 792K
- **Rating:** ⭐ 4.7/5 (6 ratings)
- **Published:** 2022-05-06
- **Last Updated:** 2024-04-03
- **VS Code Engine:** ^1.88.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-jupyter-powertoys

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-jupyter-powertoys.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-jupyter-powertoys/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-jupyter-powertoys)

**Marketplace Categories:**

Data Science, Machine Learning, Visualization, Notebooks

**Tags:**

__web_extension, execution, ipynb, json, jupyter, notebook, notebookKernelJupyterNotebook

**Works With:**

All Platforms

---

### 12. [AI Toolkit for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=ms-windows-ai-studio.windows-ai-studio)
*Extension #199 of 329*

AI Toolkit for VS Code streamlines generative AI app development by integrating tools and models. Browse and download public and custom models; author, test and evaluate prompts; fine-tune; and use them in your applications.

**Details:**

- **Extension ID:** `ms-windows-ai-studio.windows-ai-studio`
- **Version:** 0.26.3
- **Installs:** 621K
- **Rating:** ⭐ 4.4/5 (15 ratings)
- **Published:** 2023-12-13
- **Last Updated:** 2025-11-25
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-windows-ai-studio.windows-ai-studio

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/windows-ai-studio.git)
- 🐛 [Issues](https://github.com/Microsoft/windows-ai-studio/issues)
- 🏠 [Homepage](https://github.com/Microsoft/windows-ai-studio#readme)

**Marketplace Categories:**

Azure, Data Science, Machine Learning, AI, Chat

**Tags:**

ai, Azure, Chat, Data Science, language-models, language-model-tools, LLM, Machine Learning, models, studio, tools, windows

**Works With:**

Windows x64, macOS Apple Silicon, macOS Intel, Windows ARM64, Linux x64

---

### 13. [Tensorboard](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.tensorboard)
*Extension #200 of 329*

Launch and view Tensorboards within VS Code

**Details:**

- **Extension ID:** `ms-toolsai.tensorboard`
- **Version:** 2024.3.1002291100
- **Installs:** 550K
- **Rating:** ⭐ 5/5 (4 ratings)
- **Published:** 2023-10-17
- **Last Updated:** 2025-08-15
- **VS Code Engine:** ^1.85.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.tensorboard

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-tensorboard.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-tensorboard/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-tensorboard)

**Marketplace Categories:**

Data Science, Machine Learning, Notebooks

**Tags:**

jupyter, multi-root ready, python

**Works With:**

All Platforms

---

### 14. [IntelliCode Completions](https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.vscodeintellicode-completions)
*Extension #201 of 329*

IntelliCode Completions: AI-driven code auto-completion

**Details:**

- **Extension ID:** `VisualStudioExptTeam.vscodeintellicode-completions`
- **Version:** 2.0.1
- **Installs:** 528K
- **Rating:** ⭐ 4.8/5 (6 ratings)
- **Published:** 2022-01-26
- **Last Updated:** 2024-10-23
- **VS Code Engine:** ^1.82.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.vscodeintellicode-completions

**Resources:**

- 📦 [Repository](https://github.com/MicrosoftDocs/intellicode.git)
- 🐛 [Issues](https://github.com/MicrosoftDocs/intellicode/issues)
- 💬 [Support](https://go.microsoft.com/fwlink/?linkid=2005855)
- 🏠 [Homepage](https://github.com/MicrosoftDocs/intellicode#readme)

**Marketplace Categories:**

Other

**Tags:**

javascript, javascriptreact, python, typescript, typescriptreact

**Works With:**

All Platforms

---

### 15. [Microsoft Foundry](https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.vscode-ai-foundry)
*Extension #202 of 329*

Visual Studio Code extension for Microsoft Foundry

**Details:**

- **Extension ID:** `TeamsDevApp.vscode-ai-foundry`
- **Version:** 0.12.4
- **Installs:** 467K
- **Rating:** ⭐ 3.7/5 (3 ratings)
- **Published:** 2025-03-27
- **Last Updated:** 2025-11-28
- **VS Code Engine:** ^1.105.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.vscode-ai-foundry

**Resources:**

- 📦 [Repository](https://github.com/microsoft/ai-foundry-for-vscode.git)
- 🐛 [Issues](https://github.com/microsoft/ai-foundry-for-vscode/issues)
- 🏠 [Homepage](https://github.com/microsoft/ai-foundry-for-vscode#readme)

**Marketplace Categories:**

Azure, AI

**Works With:**

Linux x64

---

### 16. [GitHub Copilot app modernization - upgrade for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-upgrade)
*Extension #203 of 329*

Upgrade Java project with GitHub Copilot

**Details:**

- **Extension ID:** `vscjava.vscode-java-upgrade`
- **Version:** 1.9.1
- **Installs:** 463K
- **Rating:** ⭐ 2.1/5 (14 ratings)
- **Published:** 2025-05-16
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-upgrade

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-app-mod-pack.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-app-mod-pack/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-app-mod-pack#readme)

**Marketplace Categories:**

AI, Chat

**Tags:**

java, language-model-tools, tools

**Works With:**

All Platforms

---

### 17. [JupyterHub](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-hub)
*Extension #204 of 329*

Support for connecting to Jupyter Hub in VS Code along with the Jupyter Extension

**Details:**

- **Extension ID:** `ms-toolsai.jupyter-hub`
- **Version:** 2024.10.1002831100
- **Installs:** 253K
- **Published:** 2023-09-19
- **Last Updated:** 2024-10-08
- **VS Code Engine:** ^1.83.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter-hub

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-jupyter-hub.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-jupyter-hub/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-jupyter-hub)

**Marketplace Categories:**

Data Science, Machine Learning, Notebooks

**Tags:**

__web_extension, interactive, jupyter, jupyterhub, multi-root ready, notebook, notebookKernelJupyterNotebook, python

**Works With:**

All Platforms

---

### 18. [IntelliCode Insiders](https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.vscodeintellicode-insiders)
*Extension #205 of 329*

AI-assisted development

**Details:**

- **Extension ID:** `VisualStudioExptTeam.vscodeintellicode-insiders`
- **Version:** 1.1.10
- **Installs:** 177K
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2020-12-17
- **Last Updated:** 2022-06-30
- **VS Code Engine:** ^1.58.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=VisualStudioExptTeam.vscodeintellicode-insiders

**Resources:**

- 📦 [Repository](https://github.com/MicrosoftDocs/intellicode.git)
- 🐛 [Issues](https://github.com/MicrosoftDocs/intellicode/issues)
- 💬 [Support](https://go.microsoft.com/fwlink/?linkid=2005855)
- 🏠 [Homepage](https://github.com/MicrosoftDocs/intellicode#readme)

**Marketplace Categories:**

Other

**Tags:**

java, javascript, javascriptreact, python, typescript, typescriptreact

**Works With:**

All Platforms

---

### 19. [Web Search for Copilot](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-websearchforcopilot)
*Extension #206 of 329*

Gives access to search engines from within Copilot

**Details:**

- **Extension ID:** `ms-vscode.vscode-websearchforcopilot`
- **Version:** 0.1.2025120401
- **Installs:** 128K
- **Rating:** ⭐ 3.2/5 (6 ratings)
- **Published:** 2024-10-22
- **Last Updated:** 2025-12-04
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-websearchforcopilot

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-websearchforcopilot.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-websearchforcopilot/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-websearchforcopilot#readme)

**Marketplace Categories:**

AI, Chat

**Tags:**

chat-participant, language-model-tools, tools

**Works With:**

All Platforms

---

### 20. [vscode-mermAId](https://marketplace.visualstudio.com/items?itemName=ms-vscode.copilot-mermaid-diagram)
*Extension #207 of 329*

Exploration into Copilot Chat-powered Diagram Generation

**Details:**

- **Extension ID:** `ms-vscode.copilot-mermaid-diagram`
- **Version:** 0.0.2025120401
- **Installs:** 89K
- **Rating:** ⭐ 5/5 (7 ratings)
- **Published:** 2024-10-22
- **Last Updated:** 2025-12-04
- **VS Code Engine:** ^1.96.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.copilot-mermaid-diagram

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-mermAId.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-mermAId/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-mermAId#readme)

**Marketplace Categories:**

Visualization, AI, Chat

**Tags:**

chat, chat-participant, copilot, diagram, groq, language-model-tools, mermaid, sequence, tools, uml

**Works With:**

All Platforms

---

### 21. [Vision for Copilot Preview](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-copilot-vision)
*Extension #208 of 329*

A chat extension providing vision capabilities in VS Code, with a focus on accessibility.

**Details:**

- **Extension ID:** `ms-vscode.vscode-copilot-vision`
- **Version:** 0.2.2025032409
- **Installs:** 75K
- **Rating:** ⭐ 4/5 (4 ratings)
- **Published:** 2024-10-18
- **Last Updated:** 2025-03-24
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-copilot-vision

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-copilot-vision.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-copilot-vision/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-copilot-vision#readme)

**Marketplace Categories:**

AI, Chat

**Tags:**

accessibility, ai, anthropic, chat-participant, claude, co-pilot, gemini, html, images, javascriptreact, markdown, openai, pilot, typescriptreact, vision

**Works With:**

All Platforms

---

### 22. [Prompty](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.prompty)
*Extension #209 of 329*

Prompty Extension

**Details:**

- **Extension ID:** `ms-toolsai.prompty`
- **Version:** 1.20250714.152700
- **Installs:** 46K
- **Rating:** ⭐ 3.5/5 (2 ratings)
- **Published:** 2024-05-07
- **Last Updated:** 2025-07-14
- **VS Code Engine:** ^1.98.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.prompty

**Resources:**

- 📦 [Repository](https://github.com/microsoft/prompty.git)
- 🐛 [Issues](https://github.com/microsoft/prompty/issues)
- 🏠 [Homepage](https://github.com/microsoft/prompty#readme)

**Marketplace Categories:**

Azure, Data Science, Machine Learning

**Tags:**

__ext_prompty, __ext_tracy, Azure OpenAI, keybindings, LLM, LLMOps, plaintext, Prompt, Prompt Engineering, prompty, tracy

**Works With:**

All Platforms

---

### 23. [Data Analysis for Copilot](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-copilot-data-analysis)
*Extension #210 of 329*

This tool extends the LLM's capabilities by allowing it to run Python code in a sandboxed Python environment (Pyodide) for a wide range of computational tasks and data manipulations that it cannot perform directly.

**Details:**

- **Extension ID:** `ms-vscode.vscode-copilot-data-analysis`
- **Version:** 0.2.2
- **Installs:** 31K
- **Rating:** ⭐ 3.8/5 (6 ratings)
- **Published:** 2024-10-18
- **Last Updated:** 2024-10-31
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-copilot-data-analysis

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-data-analysis-for-copilot.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-data-analysis-for-copilot/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-data-analysis-for-copilot)

**Marketplace Categories:**

Programming Languages, Data Science, Machine Learning, Visualization, AI, Chat

**Tags:**

ai, analysis, chat, chat-participant, co-pilot, data, github-copilot, python

**Works With:**

All Platforms

---

### 24. [Azure Machine Learning - Inference](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai-inference)
*Extension #211 of 329*

This extension is used by the Azure Machine Learning extension to enable debugging of local endpoints.

**Details:**

- **Extension ID:** `ms-toolsai.vscode-ai-inference`
- **Version:** 0.1.0
- **Installs:** 15K
- **Published:** 2021-11-12
- **Last Updated:** 2021-11-19
- **VS Code Engine:** ^1.53.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai-inference

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-tools-for-ai)
- 🐛 [Issues](https://github.com/Microsoft/vscode-tools-for-ai/issues)

**Marketplace Categories:**

Azure

**Tags:**

AML, Azure Machine Learning, Azure ML, Deep Learning, Inference

**Works With:**

All Platforms

---

### 25. [Azure Machine Learning - Remote (Web)](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai-remote-web)
*Extension #212 of 329*

This extension enables remote connection to Azure Machine Learning compute instances in vscode.dev

**Details:**

- **Extension ID:** `ms-toolsai.vscode-ai-remote-web`
- **Version:** 1.0.0
- **Installs:** 13K
- **Published:** 2022-04-18
- **Last Updated:** 2024-08-22
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-toolsai.vscode-ai-remote-web

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-tools-for-ai.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-tools-for-ai/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-tools-for-ai#readme)

**Marketplace Categories:**

Azure

**Tags:**

__web_extension, AML, Azure Machine Learning, Azure ML, Deep Learning

**Works With:**

All Platforms

---

### 26. [GenAIScript](https://marketplace.visualstudio.com/items?itemName=genaiscript.genaiscript-vscode)
*Extension #213 of 329*

Generative AI Scripting.

**Details:**

- **Extension ID:** `genaiscript.genaiscript-vscode`
- **Version:** 2.5.1
- **Installs:** 13K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2024-03-18
- **Last Updated:** 2025-09-26
- **VS Code Engine:** ^1.100.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=genaiscript.genaiscript-vscode

**Resources:**

- 📦 [Repository](https://github.com/microsoft/genaiscript.git)
- 🐛 [Issues](https://github.com/microsoft/genaiscript/issues)
- 🏠 [Homepage](https://microsoft.github.io/genaiscript/)

**Marketplace Categories:**

Programming Languages, Machine Learning, Notebooks, Education, AI

**Tags:**

__ext_genai.md, ai, chatgpt, chat-participant, copilot, genai, GenAIScriptMarkdown, genaiscript-markdown, gpt, gpt4, javascript, llm, mcp, openai, python, scripting, typescript

**Works With:**

All Platforms

---

### 27. [Copilot Studio (Preview)](https://marketplace.visualstudio.com/items?itemName=ms-CopilotStudio.vscode-copilotstudio)
*Extension #214 of 329*

This technology preview enables developers to edit their Copilot Studio agents from within Visual Studio code. This includes the ability to clone, pull, push from Copilot Studio.

**Details:**

- **Extension ID:** `ms-CopilotStudio.vscode-copilotstudio`
- **Version:** 0.2.487
- **Installs:** 11K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2025-04-01
- **Last Updated:** 2025-12-05
- **VS Code Engine:** ^1.96.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-CopilotStudio.vscode-copilotstudio

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-copilotstudio.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-copilotstudio/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-copilotstudio/blob/main/README.md)

**Marketplace Categories:**

Programming Languages, Azure, AI, Chat

**Tags:**

__ext_fx1, __ext_mcsyaml, __ext_mcsyml, __ext_yaml, __ext_yml, Agents, Copilot, CopilotStudio, Fx, Microsoft Copilot Studio, Power Fx, PowerFx, Yaml

**Works With:**

macOS Apple Silicon, macOS Intel, Linux x64, Windows x64, Linux ARM64, Windows ARM64

---

### 28. [Vision AI DevKit](https://marketplace.visualstudio.com/items?itemName=VisionAIDevKit.vision-ai-devkit)
*Extension #215 of 329*

Visual Studio Code extension for Vision AI DevKit

**Details:**

- **Extension ID:** `VisionAIDevKit.vision-ai-devkit`
- **Version:** 0.1.0
- **Installs:** 10K
- **Published:** 2019-08-06
- **Last Updated:** 2019-08-06
- **VS Code Engine:** ^1.30.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=VisionAIDevKit.vision-ai-devkit

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vision-ai-developer-kit)
- 🐛 [Issues](https://github.com/Microsoft/vision-ai-developer-kit/issues)
- 🏠 [Homepage](https://azure.github.io/Vision-AI-DevKit-Pages)

**Marketplace Categories:**

Extension Packs

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 29. [Git DevOps Assistant (MCP Powered)](https://marketplace.visualstudio.com/items?itemName=MS-DAW-TCA.ado-productivity-copilot)
*Extension #216 of 329*

AI-powered Azure DevOps GitHub Copilot chat extension

**Details:**

- **Extension ID:** `MS-DAW-TCA.ado-productivity-copilot`
- **Version:** 1.8.1
- **Installs:** 3K
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2025-08-21
- **Last Updated:** 2025-11-26
- **VS Code Engine:** ^1.106.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-DAW-TCA.ado-productivity-copilot

**Marketplace Categories:**

AI, Chat

**Tags:**

ado, agile, ai-assistant, automation, azure-devops, builds, chat, chat-participant, ci-cd, code-search, devops, github-copilot, mcp, model-context-protocol, productivity, pull-requests, work-items

**Works With:**

All Platforms

---

### 30. [AI-Powered Content Creator](https://marketplace.visualstudio.com/items?itemName=PDETs-prod.new-module-creation)
*Extension #217 of 329*

**Details:**

- **Extension ID:** `PDETs-prod.new-module-creation`
- **Version:** 1.0.132
- **Installs:** 808
- **Rating:** ⭐ 3/5 (2 ratings)
- **Published:** 2025-01-08
- **Last Updated:** 2025-11-05
- **VS Code Engine:** ^1.82.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=PDETs-prod.new-module-creation

**Marketplace Categories:**

Other

**Tags:**

keybindings

**Works With:**

All Platforms

---

### 31. [AI Gallery](https://marketplace.visualstudio.com/items?itemName=ai-gallery.ai-gallery)
*Extension #218 of 329*

**Details:**

- **Extension ID:** `ai-gallery.ai-gallery`
- **Version:** 0.0.11
- **Installs:** 340
- **Published:** 2025-04-02
- **Last Updated:** 2025-05-14
- **VS Code Engine:** ^1.98.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ai-gallery.ai-gallery

**Marketplace Categories:**

Other

**Tags:**

__web_extension

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## JavaScript & TypeScript

**Category Stats:** 14 extensions | Total Installs: 27.39M | Avg Rating: 4.1⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [[Deprecated] Debugger for Chrome](https://marketplace.visualstudio.com/items?itemName=msjsdiag.debugger-for-chrome)
*Extension #219 of 329*

Debug your JavaScript code in the Chrome browser, or any other target that supports the Chrome Debugger protocol.

**Details:**

- **Extension ID:** `msjsdiag.debugger-for-chrome`
- **Version:** 4.13.0
- **Installs:** 10.84M
- **Rating:** ⭐ 4.2/5 (141 ratings)
- **Published:** 2015-11-11
- **Last Updated:** 2021-08-03
- **VS Code Engine:** ^1.60.0-insider
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=msjsdiag.debugger-for-chrome

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-chrome-debug)
- 🐛 [Issues](https://github.com/microsoft/vscode-chrome-debug/issues)

**Marketplace Categories:**

Debuggers

**Tags:**

debuggers, javascript, multi-root ready

**Works With:**

All Platforms

---

### 2. [JavaScript and TypeScript Nightly](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-typescript-next)
*Extension #220 of 329*

Enables typescript@next to power VS Code's built-in JavaScript and TypeScript support

**Details:**

- **Extension ID:** `ms-vscode.vscode-typescript-next`
- **Version:** 6.0.20251204
- **Installs:** 6.13M
- **Rating:** ⭐ 2.8/5 (57 ratings)
- **Published:** 2019-06-10
- **Last Updated:** 2025-12-05
- **VS Code Engine:** ^1.36.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-typescript-next

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-typescript-next.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-typescript-next/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-typescript-next#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, JavaScript, javascriptreact, js, nightly, ts, TypeScript, typescriptreact

**Works With:**

All Platforms

---

### 3. [React Native Tools](https://marketplace.visualstudio.com/items?itemName=msjsdiag.vscode-react-native)
*Extension #221 of 329*

Debugging and integrated commands for React Native

**Details:**

- **Extension ID:** `msjsdiag.vscode-react-native`
- **Version:** 1.13.0
- **Installs:** 5.79M
- **Rating:** ⭐ 4.1/5 (48 ratings)
- **Published:** 2016-02-19
- **Last Updated:** 2024-03-25
- **VS Code Engine:** ^1.46.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=msjsdiag.vscode-react-native

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-react-native.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-react-native/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-react-native#readme)

**Marketplace Categories:**

Debuggers

**Tags:**

debuggers, expo, javascript, multi-root ready, react-native

**Works With:**

All Platforms

---

### 4. [JavaScript Debugger (Nightly)](https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-debug-nightly)
*Extension #222 of 329*

An extension for debugging Node.js programs and Chrome.

**Details:**

- **Extension ID:** `ms-vscode.js-debug-nightly`
- **Version:** 2025.11.2517
- **Installs:** 3.2M
- **Rating:** ⭐ 4.1/5 (7 ratings)
- **Published:** 2019-11-12
- **Last Updated:** 2025-11-26
- **VS Code Engine:** ^1.80.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-debug-nightly

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-pwa.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-pwa/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-pwa#readme)

**Marketplace Categories:**

Debuggers

**Tags:**

__ext_wasm, __ext_wat, chrome, debugger, debuggers, javascript, keybindings, node, pwa, wat, WebAssemblyTextFormat

**Works With:**

All Platforms

---

### 5. [Cordova Tools](https://marketplace.visualstudio.com/items?itemName=msjsdiag.cordova-tools)
*Extension #223 of 329*

Code-hinting, debugging and integrated commands for Apache Cordova (PhoneGap). With added support for the Ionic framework.

**Details:**

- **Extension ID:** `msjsdiag.cordova-tools`
- **Version:** 2.7.0
- **Installs:** 515K
- **Rating:** ⭐ 4.4/5 (22 ratings)
- **Published:** 2016-01-27
- **Last Updated:** 2024-05-16
- **VS Code Engine:** ^1.40.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=msjsdiag.cordova-tools

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-cordova.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-cordova/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-cordova#readme)

**Marketplace Categories:**

Snippets, Debuggers

**Tags:**

cordova, debuggers, ionic, keybindings, multi-root ready

**Works With:**

All Platforms

---

### 6. [JavaScript Atom Grammar](https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-atom-grammar)
*Extension #224 of 329*

The Atom editor's JavaScript text mate grammar.

**Details:**

- **Extension ID:** `ms-vscode.js-atom-grammar`
- **Version:** 0.1.14
- **Installs:** 261K
- **Rating:** ⭐ 4.6/5 (11 ratings)
- **Published:** 2016-06-27
- **Last Updated:** 2017-10-09
- **VS Code Engine:** *
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-atom-grammar

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-js-atom-grammar.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-js-atom-grammar/issues)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension, javascript

**Works With:**

All Platforms

---

### 7. [JavaScript Debugger Companion Extension](https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-debug-companion)
*Extension #225 of 329*

Companion extension to js-debug that provides capability for remote debugging

**Details:**

- **Extension ID:** `ms-vscode.js-debug-companion`
- **Version:** 1.1.3
- **Installs:** 161K
- **Published:** 2020-02-20
- **Last Updated:** 2024-07-24
- **VS Code Engine:** ^1.90.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-debug-companion

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-js-debug-companion.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-js-debug-companion/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-js-debug-companion#readme)

**Marketplace Categories:**

Other

**Tags:**

javascript

**Works With:**

All Platforms

---

### 8. [JavaScript Debugger](https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-debug)
*Extension #226 of 329*

An extension for debugging Node.js programs and Chrome.

**Details:**

- **Extension ID:** `ms-vscode.js-debug`
- **Version:** 2022.8.1217
- **Installs:** 114K
- **Rating:** ⭐ 3.5/5 (2 ratings)
- **Published:** 2020-06-05
- **Last Updated:** 2022-08-13
- **VS Code Engine:** ^1.71.0-insider
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.js-debug

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-pwa.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-pwa/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-pwa#readme)

**Marketplace Categories:**

Debuggers

**Tags:**

__ext_wasm, __ext_wat, chrome, debugger, debuggers, javascript, keybindings, node, pwa, wat, WebAssemblyTextFormat

**Works With:**

All Platforms

---

### 9. [Flame Chart Visualizer for JavaScript Profiles](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-js-profile-flame)
*Extension #227 of 329*

Flame graph visualizer for Heap and CPU profiles taken from the JavaScript debugger

**Details:**

- **Extension ID:** `ms-vscode.vscode-js-profile-flame`
- **Version:** 1.0.9
- **Installs:** 113K
- **Rating:** ⭐ 3.7/5 (11 ratings)
- **Published:** 2020-04-19
- **Last Updated:** 2024-04-05
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-js-profile-flame

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-js-profile-visualizer.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-js-profile-visualizer/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-js-profile-visualizer#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, javascript

**Works With:**

All Platforms

---

### 10. [Debugger for Chrome (Nightly)](https://marketplace.visualstudio.com/items?itemName=msjsdiag.debugger-for-chrome-nightly)
*Extension #228 of 329*

Debug your JavaScript code in the Chrome browser, or any other target that supports the Chrome Debugger protocol.

**Details:**

- **Extension ID:** `msjsdiag.debugger-for-chrome-nightly`
- **Version:** 2020.2.15300
- **Installs:** 90K
- **Published:** 2019-07-03
- **Last Updated:** 2020-02-11
- **VS Code Engine:** ^1.17.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=msjsdiag.debugger-for-chrome-nightly

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-chrome-debug)
- 🐛 [Issues](https://github.com/microsoft/vscode-chrome-debug/issues)

**Marketplace Categories:**

Debuggers

**Tags:**

debuggers, javascript, keybindings, multi-root ready

**Works With:**

All Platforms

---

### 11. [React Native Tools (Preview)](https://marketplace.visualstudio.com/items?itemName=msjsdiag.vscode-react-native-preview)
*Extension #229 of 329*

Debugging and integrated commands for React Native

**Details:**

- **Extension ID:** `msjsdiag.vscode-react-native-preview`
- **Version:** 2024.6.2621
- **Installs:** 69K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2020-06-01
- **Last Updated:** 2024-06-27
- **VS Code Engine:** ^1.46.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=msjsdiag.vscode-react-native-preview

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-react-native.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-react-native/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-react-native#readme)

**Marketplace Categories:**

Debuggers

**Tags:**

debuggers, expo, javascript, multi-root ready, react-native

**Works With:**

All Platforms

---

### 12. [anycode-typescript](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-typescript)
*Extension #230 of 329*

TypeScript for Anycode

**Details:**

- **Extension ID:** `ms-vscode.anycode-typescript`
- **Version:** 0.0.7
- **Installs:** 45K
- **Published:** 2022-03-15
- **Last Updated:** 2025-03-28
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-typescript

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-anycode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-anycode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-anycode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 13. [TypeScript (Native Preview)](https://marketplace.visualstudio.com/items?itemName=TypeScriptTeam.native-preview)
*Extension #231 of 329*

Preview of the native TypeScript language server for Visual Studio Code.

**Details:**

- **Extension ID:** `TypeScriptTeam.native-preview`
- **Version:** 0.20251205.1
- **Installs:** 42K
- **Rating:** ⭐ 5/5 (8 ratings)
- **Published:** 2025-05-21
- **Last Updated:** 2025-12-05
- **VS Code Engine:** ^1.100.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=TypeScriptTeam.native-preview

**Resources:**

- 📦 [Repository](https://github.com/microsoft/typescript-go.git)
- 🐛 [Issues](https://github.com/microsoft/typescript-go/issues)
- 🏠 [Homepage](https://github.com/microsoft/typescript-go#readme)

**Marketplace Categories:**

Other

**Tags:**

javascript, javascriptreact, typescript, typescriptreact

**Works With:**

Windows x64, Windows ARM64, Linux x64, Linux ARM, Linux ARM64, macOS Intel, macOS Apple Silicon, Alpine Linux x64, Alpine Linux ARM64

---

### 14. [Table Visualizer for JavaScript Profiles](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-js-profile-table)
*Extension #232 of 329*

Text visualizer for profiles taken from the JavaScript debugger

**Details:**

- **Extension ID:** `ms-vscode.vscode-js-profile-table`
- **Version:** 1.0.10
- **Installs:** 19K
- **Published:** 2020-04-19
- **Last Updated:** 2024-09-25
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-js-profile-table

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-js-profile-visualizer.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-js-profile-visualizer/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-js-profile-visualizer#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, javascript

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Python Development

**Category Stats:** 13 extensions | Total Installs: 515.94M | Avg Rating: 3.5⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [Python](https://marketplace.visualstudio.com/items?itemName=ms-python.python)
*Extension #233 of 329*

Python language support with extension access points for IntelliSense (Pylance), Debugging (Python Debugger), linting, formatting, refactoring, unit tests, and more.

**Details:**

- **Extension ID:** `ms-python.python`
- **Version:** 2025.19.2025120401
- **Installs:** 194.96M
- **Rating:** ⭐ 4.2/5 (619 ratings)
- **Published:** 2016-01-19
- **Last Updated:** 2025-12-04
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-python.python

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-python.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-python/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-python)

**Marketplace Categories:**

Programming Languages, Debuggers, Data Science, Machine Learning

**Tags:**

__ext_j2, __ext_jinja2, __web_extension, debuggers, django, ini, jinja, json, keybindings, language-model-tools, linters, multi-root ready, piprequirements, pip-requirements, python, requirements.txt, toml, tools, unittest, yaml

**Works With:**

macOS Apple Silicon, Windows ARM64, Windows x64, macOS Intel, Linux ARM, Linux ARM64, Linux x64, Alpine Linux x64, Alpine Linux ARM64, Web

---

### 2. [Pylance](https://marketplace.visualstudio.com/items?itemName=ms-python.vscode-pylance)
*Extension #234 of 329*

A performant, feature-rich language server for Python in VS Code

**Details:**

- **Extension ID:** `ms-python.vscode-pylance`
- **Version:** 2025.10.4
- **Installs:** 162.84M
- **Rating:** ⭐ 3/5 (265 ratings)
- **Published:** 2020-06-30
- **Last Updated:** 2025-12-05
- **VS Code Engine:** ^1.105.1
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-python.vscode-pylance

**Resources:**

- 📦 [Repository](https://github.com/microsoft/pylance-release.git)
- 🐛 [Issues](https://github.com/microsoft/pylance-release/issues)
- 🏠 [Homepage](https://github.com/microsoft/pylance-release#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension, json, python

**Works With:**

All Platforms

---

### 3. [Python Debugger](https://marketplace.visualstudio.com/items?itemName=ms-python.debugpy)
*Extension #235 of 329*

Python Debugger extension using debugpy.

**Details:**

- **Extension ID:** `ms-python.debugpy`
- **Version:** 2025.17.2025120401
- **Installs:** 100.29M
- **Rating:** ⭐ 4.6/5 (14 ratings)
- **Published:** 2023-06-14
- **Last Updated:** 2025-12-04
- **VS Code Engine:** ^1.92.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-python.debugpy

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-python-debugger.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-python-debugger/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-python-debugger)

**Marketplace Categories:**

Debuggers

**Tags:**

debugger, debuggers, debugpy, python

**Works With:**

macOS Intel, Linux ARM64, Windows x64, macOS Apple Silicon, Linux ARM, Linux x64, Windows ARM64, Windows x86

---

### 4. [isort](https://marketplace.visualstudio.com/items?itemName=ms-python.isort)
*Extension #236 of 329*

Import organization support for Python files using isort.

**Details:**

- **Extension ID:** `ms-python.isort`
- **Version:** 2025.1.13251007
- **Installs:** 24.92M
- **Rating:** ⭐ 2.5/5 (36 ratings)
- **Published:** 2022-04-25
- **Last Updated:** 2025-11-21
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-python.isort

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-isort.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-isort/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-isort)

**Marketplace Categories:**

Programming Languages, Formatters

**Tags:**

formatting, isort, python, toml

**Works With:**

All Platforms

---

### 5. [Python Environments](https://marketplace.visualstudio.com/items?itemName=ms-python.vscode-python-envs)
*Extension #237 of 329*

Provides a unified python environment experience

**Details:**

- **Extension ID:** `ms-python.vscode-python-envs`
- **Version:** 1.13.13381830
- **Installs:** 16.5M
- **Rating:** ⭐ 1.4/5 (60 ratings)
- **Published:** 2024-12-10
- **Last Updated:** 2025-12-04
- **VS Code Engine:** ^1.106.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-python.vscode-python-envs

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-python-environments.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-python-environments/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-python-environments)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, python

**Works With:**

Windows x64, Windows ARM64, Alpine Linux x64, Alpine Linux ARM64, Linux ARM, Linux x64, Linux ARM64, macOS Apple Silicon, macOS Intel, Web

---

### 6. [Black Formatter](https://marketplace.visualstudio.com/items?itemName=ms-python.black-formatter)
*Extension #238 of 329*

Formatting support for Python files using the Black formatter.

**Details:**

- **Extension ID:** `ms-python.black-formatter`
- **Version:** 2025.3.11831009
- **Installs:** 5.97M
- **Rating:** ⭐ 3.4/5 (53 ratings)
- **Published:** 2022-04-14
- **Last Updated:** 2025-07-02
- **VS Code Engine:** ^1.82.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-python.black-formatter

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-black-formatter.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-black-formatter/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-black-formatter)

**Marketplace Categories:**

Programming Languages, Formatters

**Tags:**

black, formatting, python, toml

**Works With:**

All Platforms

---

### 7. [autopep8](https://marketplace.visualstudio.com/items?itemName=ms-python.autopep8)
*Extension #239 of 329*

Formatting support for Python files using the autopep8 formatter.

**Details:**

- **Extension ID:** `ms-python.autopep8`
- **Version:** 2025.3.13231724
- **Installs:** 5.03M
- **Rating:** ⭐ 2.9/5 (15 ratings)
- **Published:** 2022-10-04
- **Last Updated:** 2025-11-19
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-python.autopep8

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-autopep8.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-autopep8/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-autopep8)

**Marketplace Categories:**

Programming Languages, Formatters

**Tags:**

autopep8, formatting, ini, python, toml

**Works With:**

All Platforms

---

### 8. [Pylint](https://marketplace.visualstudio.com/items?itemName=ms-python.pylint)
*Extension #240 of 329*

Linting support for Python files using Pylint.

**Details:**

- **Extension ID:** `ms-python.pylint`
- **Version:** 2025.3.12271016
- **Installs:** 2.91M
- **Rating:** ⭐ 2.9/5 (17 ratings)
- **Published:** 2022-03-10
- **Last Updated:** 2025-08-15
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-python.pylint

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-pylint.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-pylint/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-pylint)

**Marketplace Categories:**

Programming Languages, Linters

**Tags:**

ini, linters, linting, pylint, python

**Works With:**

All Platforms

---

### 9. [Flake8](https://marketplace.visualstudio.com/items?itemName=ms-python.flake8)
*Extension #241 of 329*

Linting support for Python files using Flake8.

**Details:**

- **Extension ID:** `ms-python.flake8`
- **Version:** 2025.3.13371015
- **Installs:** 1.64M
- **Rating:** ⭐ 5/5 (6 ratings)
- **Published:** 2022-08-24
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-python.flake8

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-flake8.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-flake8/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-flake8)

**Marketplace Categories:**

Programming Languages, Linters

**Tags:**

flake8, ini, linters, linting, python

**Works With:**

All Platforms

---

### 10. [Mypy Type Checker](https://marketplace.visualstudio.com/items?itemName=ms-python.mypy-type-checker)
*Extension #242 of 329*

Type checking support for Python files using Mypy.

**Details:**

- **Extension ID:** `ms-python.mypy-type-checker`
- **Version:** 2025.3.13371016
- **Installs:** 690K
- **Rating:** ⭐ 3.3/5 (16 ratings)
- **Published:** 2023-04-04
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-python.mypy-type-checker

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-mypy.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-mypy/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-mypy)

**Marketplace Categories:**

Programming Languages, Linters, Formatters

**Tags:**

mypy, python

**Works With:**

All Platforms

---

### 11. [Gather](https://marketplace.visualstudio.com/items?itemName=ms-python.gather)
*Extension #243 of 329*

Adds gather functionality in the Python language to the Jupyter Extension

**Details:**

- **Extension ID:** `ms-python.gather`
- **Version:** 2025.4.0
- **Installs:** 119K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2020-07-10
- **Last Updated:** 2025-05-08
- **VS Code Engine:** ^1.68.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-python.gather

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-gather.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-jupyter/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-jupyter)

**Marketplace Categories:**

Programming Languages, Extension Packs, Data Science, Machine Learning, Notebooks

**Tags:**

jupyter, python

**Works With:**

All Platforms

---

### 12. [anycode-python](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-python)
*Extension #244 of 329*

Python for Anycode

**Details:**

- **Extension ID:** `ms-vscode.anycode-python`
- **Version:** 0.0.7
- **Installs:** 65K
- **Published:** 2022-03-15
- **Last Updated:** 2025-03-28
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-python

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-anycode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-anycode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-anycode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension, python

**Works With:**

All Platforms

---

### 13. [Experimental - Python for the Web](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-python-web-wasm)
*Extension #245 of 329*

Experimental support for Python in the Web using WebAssemblies

**Details:**

- **Extension ID:** `ms-vscode.vscode-python-web-wasm`
- **Version:** 0.13.0
- **Installs:** 8K
- **Published:** 2022-09-26
- **Last Updated:** 2023-03-16
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-python-web-wasm

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-python-web-wasm.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-python-web-wasm/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-python-web-wasm#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, debuggers, python

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Remote Development

**Category Stats:** 10 extensions | Total Installs: 175.3M | Avg Rating: 4.1⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [WSL](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-wsl)
*Extension #246 of 329*

Open any folder in the Windows Subsystem for Linux (WSL) and take advantage of Visual Studio Code's full feature set.

**Details:**

- **Extension ID:** `ms-vscode-remote.remote-wsl`
- **Version:** 0.104.3
- **Installs:** 37.25M
- **Rating:** ⭐ 4.8/5 (84 ratings)
- **Published:** 2019-05-02
- **Last Updated:** 2025-10-01
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-wsl

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-remote-release.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-remote-release/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-remote-release#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, keybindings, remote, remote development, remote-menu, wsl

**Works With:**

All Platforms

---

### 2. [Dev Containers](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers)
*Extension #247 of 329*

Open any folder or repository inside a Docker container and take advantage of Visual Studio Code's full feature set.

**Details:**

- **Extension ID:** `ms-vscode-remote.remote-containers`
- **Version:** 0.435.0
- **Installs:** 35.18M
- **Rating:** ⭐ 4.3/5 (56 ratings)
- **Published:** 2019-05-02
- **Last Updated:** 2025-12-05
- **VS Code Engine:** ^1.99.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-containers

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-remote-release.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-remote-release/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-remote-release#readme)

**Marketplace Categories:**

Other

**Tags:**

containers, continueOn, dev container, devcontainer, development container, docker, json, jsonc, remote, remote development, remote-menu

**Works With:**

All Platforms

---

### 3. [Remote - SSH](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh)
*Extension #248 of 329*

Open any folder on a remote machine using SSH and take advantage of VS Code's full feature set.

**Details:**

- **Extension ID:** `ms-vscode-remote.remote-ssh`
- **Version:** 0.122.2025111815
- **Installs:** 30.74M
- **Rating:** ⭐ 3.7/5 (207 ratings)
- **Published:** 2019-05-02
- **Last Updated:** 2025-11-18
- **VS Code Engine:** ^1.96.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-remote-release.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-remote-release/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-remote-release#readme)

**Marketplace Categories:**

Other

**Tags:**

chat-participant, remote, remote development, remote-menu, ssh

**Works With:**

All Platforms

---

### 4. [Remote - SSH: Editing Configuration Files](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh-edit)
*Extension #249 of 329*

Edit SSH configuration files

**Details:**

- **Extension ID:** `ms-vscode-remote.remote-ssh-edit`
- **Version:** 0.87.0
- **Installs:** 28.62M
- **Rating:** ⭐ 4.2/5 (21 ratings)
- **Published:** 2019-05-02
- **Last Updated:** 2024-10-01
- **VS Code Engine:** ^1.74.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh-edit

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-remote-release.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-remote-release/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-remote-release#readme)

**Marketplace Categories:**

Programming Languages, Snippets

**Tags:**

remote, remote development, ssh, SSH Config, ssh_config

**Works With:**

All Platforms

---

### 5. [Remote Explorer](https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-explorer)
*Extension #250 of 329*

View remote machines for SSH and Tunnels.

**Details:**

- **Extension ID:** `ms-vscode.remote-explorer`
- **Version:** 0.6.2025081809
- **Installs:** 22.76M
- **Rating:** ⭐ 4.5/5 (12 ratings)
- **Published:** 2022-08-10
- **Last Updated:** 2025-08-18
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-explorer

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-remote-release.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-remote-release/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-remote-release#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, remote, remote development

**Works With:**

All Platforms

---

### 6. [Remote Development](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.vscode-remote-extensionpack)
*Extension #251 of 329*

An extension pack that lets you open any folder in a container, on a remote machine, or in WSL and take advantage of VS Code's full feature set.

**Details:**

- **Extension ID:** `ms-vscode-remote.vscode-remote-extensionpack`
- **Version:** 0.26.0
- **Installs:** 8.06M
- **Rating:** ⭐ 4.6/5 (118 ratings)
- **Published:** 2019-05-02
- **Last Updated:** 2024-10-24
- **VS Code Engine:** ^1.34.20
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.vscode-remote-extensionpack

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-remote-release.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-remote-release/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-remote-release#readme)

**Marketplace Categories:**

Extension Packs

**Tags:**

__web_extension, containers, dev container, development container, docker, remote, remote development, ssh, wsl

**Works With:**

All Platforms

---

### 7. [Remote - Tunnels](https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-server)
*Extension #252 of 329*

Connect to a remote machine through a Tunnel

**Details:**

- **Extension ID:** `ms-vscode.remote-server`
- **Version:** 1.6.2025102109
- **Installs:** 7.45M
- **Rating:** ⭐ 4.4/5 (9 ratings)
- **Published:** 2022-03-14
- **Last Updated:** 2025-10-21
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-server

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-remote-release.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-remote-release/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-remote-release#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, remote-menu

**Works With:**

All Platforms

---

### 8. [Remote Repositories](https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-repositories)
*Extension #253 of 329*

Remotely browse and edit git repositories

**Details:**

- **Extension ID:** `ms-vscode.remote-repositories`
- **Version:** 0.43.2025081801
- **Installs:** 5.18M
- **Rating:** ⭐ 4.2/5 (5 ratings)
- **Published:** 2022-03-23
- **Last Updated:** 2025-08-18
- **VS Code Engine:** ^1.78.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-repositories

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-remote-repositories-github.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-remote-repositories-github/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-remote-repositories-github#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, continueOn, git, json, remote-menu

**Works With:**

All Platforms

---

### 9. [WSL: Recommender](https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-wsl-recommender)
*Extension #254 of 329*

Recommends using the Windows Subsystem for Linux (WSL) and the WSL extension.

**Details:**

- **Extension ID:** `ms-vscode-remote.remote-wsl-recommender`
- **Version:** 0.0.20
- **Installs:** 69K
- **Rating:** ⭐ 2.3/5 (3 ratings)
- **Published:** 2021-01-12
- **Last Updated:** 2024-10-23
- **VS Code Engine:** ^1.94.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-wsl-recommender

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-remote-wsl-recommender.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-remote-release/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-remote-wsl-recommender#readme)

**Marketplace Categories:**

Other

**Tags:**

remote, remote development, remote-menu, wsl

**Works With:**

All Platforms

---

### 10. [Fabric Data Engineering VS Code - Remote](https://marketplace.visualstudio.com/items?itemName=SynapseVSCode.vscode-synapse-remote)
*Extension #255 of 329*

Create and manage Microsoft Fabric notebooks inside Visual Studio Code for the Web (Previously Synapse VS Code - Remote)

**Details:**

- **Extension ID:** `SynapseVSCode.vscode-synapse-remote`
- **Version:** 0.9.180244826
- **Installs:** 10K
- **Rating:** ⭐ 4/5 (2 ratings)
- **Published:** 2023-05-18
- **Last Updated:** 2025-07-10
- **VS Code Engine:** ^1.78.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=SynapseVSCode.vscode-synapse-remote

**Resources:**

- 📦 [Repository](msdata@vs-ssh.visualstudio.com:v3/msdata/A365/vscode-trident)

**Marketplace Categories:**

Data Science, Machine Learning

**Tags:**

__web_extension, Data Engineering and Data Science, Fabric, Machine Learning, Microsoft, Spark, Synapse

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Containers & Kubernetes

**Category Stats:** 9 extensions | Total Installs: 7.36M | Avg Rating: 4.5⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [Kubernetes](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.vscode-kubernetes-tools)
*Extension #256 of 329*

Develop, deploy and debug Kubernetes applications

**Details:**

- **Extension ID:** `ms-kubernetes-tools.vscode-kubernetes-tools`
- **Version:** 1.3.27
- **Installs:** 6.34M
- **Rating:** ⭐ 4.4/5 (39 ratings)
- **Published:** 2018-01-10
- **Last Updated:** 2025-11-25
- **VS Code Engine:** ^1.106.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.vscode-kubernetes-tools

**Resources:**

- 📦 [Repository](https://github.com/vscode-kubernetes-tools/vscode-kubernetes-tools)
- 🐛 [Issues](https://github.com/vscode-kubernetes-tools/vscode-kubernetes-tools/issues)

**Marketplace Categories:**

Snippets, Linters, Debuggers, Azure

**Tags:**

aks, aws, gke, helm, helm-template, ignore, keybindings, kubernetes, snippet, yaml

**Works With:**

All Platforms

---

### 2. [Bridge to Kubernetes](https://marketplace.visualstudio.com/items?itemName=mindaro.mindaro)
*Extension #257 of 329*

Rapid Kubernetes development for teams

**Details:**

- **Extension ID:** `mindaro.mindaro`
- **Version:** 2.0.120250115
- **Installs:** 488K
- **Rating:** ⭐ 4.8/5 (6 ratings)
- **Published:** 2020-07-23
- **Last Updated:** 2025-01-15
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=mindaro.mindaro

**Resources:**

- 📦 [Repository](https://github.com/Azure/vscode-bridge-to-kubernetes.git)
- 🐛 [Issues](https://github.com/Azure/vscode-bridge-to-kubernetes/issues)
- 🏠 [Homepage](https://aka.ms/bridge-to-k8s-vscode-quickstart)

**Marketplace Categories:**

Debuggers, Azure

**Tags:**

azds, Azure Dev Spaces, Bridge, Bridge to Kubernetes, cloud native, cluster, connect, container, debug, debuggers, docker, kubernetes, kubernetes-extension-local-tunnel-debug-provider, Local Process with Kubernetes, lpk, microservice

**Works With:**

All Platforms

---

### 3. [Azure Kubernetes Service](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.vscode-aks-tools)
*Extension #258 of 329*

Display Azure Kubernetes Services within VS Code

**Details:**

- **Extension ID:** `ms-kubernetes-tools.vscode-aks-tools`
- **Version:** 1.6.14
- **Installs:** 402K
- **Rating:** ⭐ 4.8/5 (13 ratings)
- **Published:** 2019-05-10
- **Last Updated:** 2025-11-04
- **VS Code Engine:** ^1.105.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.vscode-aks-tools

**Resources:**

- 📦 [Repository](https://github.com/Azure/vscode-aks-tools.git)
- 🐛 [Issues](https://github.com/Azure/vscode-aks-tools/issues)
- 🏠 [Homepage](https://github.com/Azure/vscode-aks-tools#readme)

**Marketplace Categories:**

Azure

**Tags:**

acs, aks, azure, kubernetes, kubernetes-extension-cloud-provider

**Works With:**

All Platforms

---

### 4. [Kubernetes Kind](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.kind-vscode)
*Extension #259 of 329*

Work with the Kind local Kubernetes provider in Visual Studio Code

**Details:**

- **Extension ID:** `ms-kubernetes-tools.kind-vscode`
- **Version:** 0.0.3
- **Installs:** 102K
- **Rating:** ⭐ 4/5 (1 ratings)
- **Published:** 2019-07-22
- **Last Updated:** 2020-03-26
- **VS Code Engine:** ^1.32.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.kind-vscode

**Resources:**

- 📦 [Repository](https://github.com/deislabs/kind-vscode)
- 🐛 [Issues](https://github.com/deislabs/kind-vscode/issues)

**Marketplace Categories:**

Other

**Tags:**

kind, kubernetes, kubernetes-extension-cloud-provider

**Works With:**

All Platforms

---

### 5. [Developer Tools for Azure Kubernetes Service (AKS)](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.aks-devx-tools)
*Extension #260 of 329*

Create deployment files and configure GitHub Actions workflows to deploy applications to Azure Kubernetes Service (AKS).

**Details:**

- **Extension ID:** `ms-kubernetes-tools.aks-devx-tools`
- **Version:** 0.1.2
- **Installs:** 22K
- **Published:** 2022-07-25
- **Last Updated:** 2023-05-23
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.aks-devx-tools

**Resources:**

- 📦 [Repository](https://github.com/Azure/aks-devx-tools)
- 🐛 [Issues](https://github.com/Azure/aks-devx-tools/issues)

**Marketplace Categories:**

Azure

**Works With:**

All Platforms

---

### 6. [Open Policy Agent for Kubernetes](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.kubernetes-opa-vscode)
*Extension #261 of 329*

Work with Open Policy Agent as a Kubernetes admission controller

**Details:**

- **Extension ID:** `ms-kubernetes-tools.kubernetes-opa-vscode`
- **Version:** 0.0.2
- **Installs:** 5K
- **Published:** 2019-12-02
- **Last Updated:** 2019-12-06
- **VS Code Engine:** ^1.40.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.kubernetes-opa-vscode

**Resources:**

- 📦 [Repository](https://github.com/deislabs/kubernetes-opa-vscode)
- 🐛 [Issues](https://github.com/deislabs/kubernetes-opa-vscode/issues)

**Marketplace Categories:**

Other

**Tags:**

kubernetes, opa, open policy agent

**Works With:**

All Platforms

---

### 7. [Duffle](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.duffle-vscode)
*Extension #262 of 329*

Build and deploy Cloud Native Application Bundles with Duffle

**Details:**

- **Extension ID:** `ms-kubernetes-tools.duffle-vscode`
- **Version:** 0.3.0
- **Installs:** 4K
- **Published:** 2018-12-04
- **Last Updated:** 2019-08-14
- **VS Code Engine:** ^1.26.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.duffle-vscode

**Resources:**

- 📦 [Repository](https://github.com/deislabs/duffle-vscode)
- 🐛 [Issues](https://github.com/deislabs/duffle-vscode/issues)

**Marketplace Categories:**

Other

**Tags:**

cloud-native, cnab, duffle, json, snippet

**Works With:**

All Platforms

---

### 8. [Porter](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.porter-vscode)
*Extension #263 of 329*

Build CNAB bundles with Porter

**Details:**

- **Extension ID:** `ms-kubernetes-tools.porter-vscode`
- **Version:** 0.0.2
- **Installs:** 3K
- **Published:** 2019-11-11
- **Last Updated:** 2019-11-13
- **VS Code Engine:** ^1.30.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.porter-vscode

**Resources:**

- 📦 [Repository](https://github.com/deislabs/porter-vscode)
- 🐛 [Issues](https://github.com/deislabs/porter-vscode/issues)

**Marketplace Categories:**

Other

**Tags:**

cnab, debuggers, porter, yaml

**Works With:**

All Platforms

---

### 9. [Duffle Coat](https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.duffle-coat)
*Extension #264 of 329*

Generates CNAB self-installers

**Details:**

- **Extension ID:** `ms-kubernetes-tools.duffle-coat`
- **Version:** 0.3.4
- **Installs:** 2K
- **Published:** 2018-12-11
- **Last Updated:** 2019-08-21
- **VS Code Engine:** ^1.27.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-kubernetes-tools.duffle-coat

**Resources:**

- 📦 [Repository](https://github.com/deislabs/duffle-coat)
- 🐛 [Issues](https://github.com/deislabs/duffle-coat/issues)

**Marketplace Categories:**

Other

**Tags:**

cnab, duffle

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## .NET Development

**Category Stats:** 9 extensions | Total Installs: 81.09M | Avg Rating: 3.3⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [C#](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csharp)
*Extension #265 of 329*

Base language support for C#

**Details:**

- **Extension ID:** `ms-dotnettools.csharp`
- **Version:** 2.103.33
- **Installs:** 37.45M
- **Rating:** ⭐ 2.7/5 (662 ratings)
- **Published:** 2016-02-26
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.106.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csharp

**Resources:**

- 📦 [Repository](https://github.com/dotnet/vscode-csharp.git)
- 🐛 [Issues](https://github.com/dotnet/vscode-csharp/issues)
- 💬 [Support](https://github.com/dotnet/vscode-csharp)
- 🏠 [Homepage](https://github.com/dotnet/vscode-csharp#readme)

**Marketplace Categories:**

Programming Languages, Snippets, Linters, Debuggers

**Tags:**

.NET, .NET Core, __ext_cshtml, __ext_razor, __ext_xaml, ASP.NET, ASP.NETRazor, aspnetcorerazor, C#, color-theme, coreclr, csharp, debuggers, dotnet, json, keybindings, multi-root ready, snippet, theme, xaml

**Works With:**

Alpine Linux ARM64, Alpine Linux x64, macOS Apple Silicon, Linux ARM64, Windows ARM64, macOS Intel, Linux x64, Windows x64, Windows x86

---

### 2. [.NET Install Tool](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscode-dotnet-runtime)
*Extension #266 of 329*

This extension installs and manages different versions of the .NET SDK and Runtime.

**Details:**

- **Extension ID:** `ms-dotnettools.vscode-dotnet-runtime`
- **Version:** 2.4.1
- **Installs:** 23.23M
- **Rating:** ⭐ 1.9/5 (91 ratings)
- **Published:** 2020-03-19
- **Last Updated:** 2025-12-01
- **VS Code Engine:** ^1.101.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscode-dotnet-runtime

**Resources:**

- 📦 [Repository](https://github.com/dotnet/vscode-dotnet-runtime.git)
- 🐛 [Issues](https://github.com/dotnet/vscode-dotnet-runtime/issues)
- 🏠 [Homepage](https://github.com/dotnet/vscode-dotnet-runtime#readme)

**Marketplace Categories:**

Other

**Tags:**

.NET, .NET Core, dotnet, Extension Authoring, runtime

**Works With:**

All Platforms

---

### 3. [C# Dev Kit](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csdevkit)
*Extension #267 of 329*

Official C# extension from Microsoft

**Details:**

- **Extension ID:** `ms-dotnettools.csdevkit`
- **Version:** 1.83.5
- **Installs:** 12.56M
- **Rating:** ⭐ 2.9/5 (182 ratings)
- **Published:** 2023-06-06
- **Last Updated:** 2025-12-04
- **VS Code Engine:** ^1.95.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csdevkit

**Resources:**

- 🐛 [Issues](https://github.com/microsoft/vscode-dotnettools/issues)

**Marketplace Categories:**

Programming Languages, Linters, Debuggers, Testing

**Tags:**

asp.net, c#, csharp, devkit, dotnet, json, keybindings

**Works With:**

Alpine Linux x64, Linux ARM64, macOS Apple Silicon, Alpine Linux ARM64, macOS Intel, Linux x64, Windows x64, Windows ARM64, Windows x86

---

### 4. [IntelliCode for C# Dev Kit](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscodeintellicode-csharp)
*Extension #268 of 329*

AI-assisted development for C# Dev Kit

**Details:**

- **Extension ID:** `ms-dotnettools.vscodeintellicode-csharp`
- **Version:** 2.2.3
- **Installs:** 4.21M
- **Rating:** ⭐ 2.1/5 (43 ratings)
- **Published:** 2023-06-06
- **Last Updated:** 2024-11-13
- **VS Code Engine:** ^1.82.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscodeintellicode-csharp

**Resources:**

- 📦 [Repository](https://github.com/MicrosoftDocs/intellicode.git)
- 🐛 [Issues](https://github.com/MicrosoftDocs/intellicode/issues)
- 🏠 [Homepage](https://github.com/MicrosoftDocs/intellicode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

c#, csharp

**Works With:**

macOS Intel, Windows x64, Windows ARM64, Linux x64, Linux ARM64, macOS Apple Silicon

---

### 5. [Polyglot Notebooks](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.dotnet-interactive-vscode)
*Extension #269 of 329*

Polyglot Notebooks for VS Code. Use multiple languages in one notebook with full language server support for each language and share variables between them.

**Details:**

- **Extension ID:** `ms-dotnettools.dotnet-interactive-vscode`
- **Version:** 1.0.6323011
- **Installs:** 1.75M
- **Rating:** ⭐ 4.1/5 (47 ratings)
- **Published:** 2020-05-15
- **Last Updated:** 2025-06-24
- **VS Code Engine:** ^1.87.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.dotnet-interactive-vscode

**Resources:**

- 📦 [Repository](https://github.com/dotnet/interactive.git)
- 🐛 [Issues](https://github.com/dotnet/interactive/issues)
- 🏠 [Homepage](https://github.com/dotnet/interactive#readme)

**Marketplace Categories:**

Data Science, Machine Learning, Visualization, Notebooks

**Tags:**

.NET, C#, Code, data science, dotnet, F#, interactive, ipynb, Jupyter, keybindings, notebooks, polyglot-notebook, PowerShell

**Works With:**

All Platforms

---

### 6. [.NET Extension Pack](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscode-dotnet-pack)
*Extension #270 of 329*

The ultimate collection of extensions for working with .NET in VS Code!

**Details:**

- **Extension ID:** `ms-dotnettools.vscode-dotnet-pack`
- **Version:** 1.0.13
- **Installs:** 1.35M
- **Rating:** ⭐ 2.6/5 (5 ratings)
- **Published:** 2021-02-03
- **Last Updated:** 2023-12-04
- **VS Code Engine:** ^1.83.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscode-dotnet-pack

**Resources:**

- 📦 [Repository](https://github.com/dotnet/vscode-dotnet-pack.git)
- 🐛 [Issues](https://github.com/dotnet/vscode-dotnet-pack/issues)
- 🏠 [Homepage](https://github.com/dotnet/vscode-dotnet-pack)

**Marketplace Categories:**

Programming Languages, Snippets, Linters, Debuggers, Extension Packs, Notebooks

**Tags:**

.NET, .NET Core, .NET Interactive, aspnetcorerazor, C#, csharp, dotnet, dotnet Interactive, F#, fsharp, interactive programming, Jupyter, notebooks, Polyglot Notebooks

**Works With:**

All Platforms

---

### 7. [.NET MAUI](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.dotnet-maui)
*Extension #271 of 329*

Extend C# Dev Kit with tools for building .NET Multi-platform App UI (MAUI) apps

**Details:**

- **Extension ID:** `ms-dotnettools.dotnet-maui`
- **Version:** 1.12.13
- **Installs:** 347K
- **Rating:** ⭐ 3.5/5 (28 ratings)
- **Published:** 2023-07-11
- **Last Updated:** 2025-12-04
- **VS Code Engine:** ^1.72.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.dotnet-maui

**Resources:**

- 🐛 [Issues](https://github.com/microsoft/vscode-dotnettools/issues)

**Marketplace Categories:**

Programming Languages, Linters, Debuggers

**Tags:**

__ext_cs, __ext_xaml, C, C#, csharp, debuggers, dotnet, mobile, xaml

**Works With:**

macOS Intel, Linux x64, Alpine Linux x64, Windows ARM64, Alpine Linux ARM64, macOS Apple Silicon, Linux ARM64, Windows x64, Windows x86

---

### 8. [Microsoft.AspNetCore.Razor.VSCode.BlazorWasmDebuggingExtension](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.blazorwasm-companion)
*Extension #272 of 329*

A companion extension for debugging Blazor WebAssembly applications in VS Code. Must be installed alongside the C# extension.

**Details:**

- **Extension ID:** `ms-dotnettools.blazorwasm-companion`
- **Version:** 1.1.4
- **Installs:** 176K
- **Rating:** ⭐ 5/5 (5 ratings)
- **Published:** 2021-02-01
- **Last Updated:** 2023-02-06
- **VS Code Engine:** ^1.69.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.blazorwasm-companion

**Resources:**

- 📦 [Repository](https://github.com/dotnet/razor.git)
- 🐛 [Issues](https://github.com/dotnet/razor/issues)
- 🏠 [Homepage](https://github.com/dotnet/razor#readme)

**Marketplace Categories:**

Other

**Tags:**

c#

**Works With:**

All Platforms

---

### 9. [GitHub Copilot app modernization for .NET](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscode-dotnet-modernize)
*Extension #273 of 329*

GitHub Copilot app modernization for .NET is a public preview AI-powered experience that helps you bring your .NET applications to the latest version quicker and more confidently than ever before. Powered by GitHub Copilot and Agent Mode, it serves as an intelligent upgrade companion that understand

**Details:**

- **Extension ID:** `ms-dotnettools.vscode-dotnet-modernize`
- **Version:** 0.1.542
- **Installs:** 6K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2025-11-18
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.106.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.vscode-dotnet-modernize

**Marketplace Categories:**

AI, Chat

**Tags:**

.NET, agent, chat, copilot, csharp, Upgrade

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Education & Learning

**Category Stats:** 8 extensions | Total Installs: 1.7M | Avg Rating: 4.4⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [learn-markdown](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-markdown)
*Extension #274 of 329*

Learn Markdown Extension

**Details:**

- **Extension ID:** `docsmsft.docs-markdown`
- **Version:** 1.0.17
- **Installs:** 471K
- **Rating:** ⭐ 4.1/5 (7 ratings)
- **Published:** 2018-04-05
- **Last Updated:** 2025-10-21
- **VS Code Engine:** ^1.71.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-markdown

**Resources:**

- 📦 [Repository](https://ceapex@dev.azure.com/ceapex/Engineering/_git/learn-markdown)
- 🐛 [Issues](https://github.com/Microsoft/vscode-docs-authoring/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-docs-authoring)

**Marketplace Categories:**

Other

**Tags:**

json, keybindings, markdown, snippet, yaml

**Works With:**

All Platforms

---

### 2. [learn-yaml](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-yaml)
*Extension #275 of 329*

YAML schema validation and auto-completion for learn.microsoft.com authoring

**Details:**

- **Extension ID:** `docsmsft.docs-yaml`
- **Version:** 1.0.5
- **Installs:** 466K
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2019-03-13
- **Last Updated:** 2024-09-20
- **VS Code Engine:** ^1.71.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-yaml

**Resources:**

- 📦 [Repository](https://ceapex@dev.azure.com/ceapex/Engineering/_git/learn-yaml)
- 🐛 [Issues](https://github.com/Microsoft/vscode-docs-authoring/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-docs-authoring)

**Marketplace Categories:**

Programming Languages, Snippets, Azure

**Tags:**

azure, docs, yaml, YamlMime

**Works With:**

All Platforms

---

### 3. [learn-images](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-images)
*Extension #276 of 329*

Learn Images Extension

**Details:**

- **Extension ID:** `docsmsft.docs-images`
- **Version:** 1.0.4
- **Installs:** 295K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2019-12-20
- **Last Updated:** 2024-09-13
- **VS Code Engine:** ^1.76.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-images

**Resources:**

- 📦 [Repository](https://ceapex@dev.azure.com/ceapex/Engineering/_git/learn-images)
- 🐛 [Issues](https://github.com/Microsoft/vscode-docs-authoring/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-docs-authoring)

**Marketplace Categories:**

Other

**Tags:**

__ext_gif, __ext_jpeg, __ext_jpg, __ext_png, __ext_svg, __ext_webp, compress, compression, compressionTargetFile, image, images, optimization, optimize

**Works With:**

All Platforms

---

### 4. [learn-preview](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-preview)
*Extension #277 of 329*

Learn Markdown Preview Extension

**Details:**

- **Extension ID:** `docsmsft.docs-preview`
- **Version:** 2.0.10
- **Installs:** 247K
- **Rating:** ⭐ 3.3/5 (3 ratings)
- **Published:** 2018-06-18
- **Last Updated:** 2025-09-25
- **VS Code Engine:** ^1.86.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-preview

**Resources:**

- 📦 [Repository](https://ceapex@dev.azure.com/ceapex/Engineering/_git/learn-preview)
- 🐛 [Issues](https://github.com/Microsoft/vscode-docs-authoring/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-docs-authoring)

**Marketplace Categories:**

Snippets

**Tags:**

commonmark, dfm, docfx, markdown, yaml

**Works With:**

All Platforms

---

### 5. [learn-article-templates](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-article-templates)
*Extension #278 of 329*

Learn article templates

**Details:**

- **Extension ID:** `docsmsft.docs-article-templates`
- **Version:** 1.0.8
- **Installs:** 125K
- **Rating:** ⭐ 4/5 (2 ratings)
- **Published:** 2018-06-18
- **Last Updated:** 2025-02-12
- **VS Code Engine:** ^1.71.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-article-templates

**Resources:**

- 📦 [Repository](https://ceapex@dev.azure.com/ceapex/Engineering/_git/learn-article-templates)
- 🐛 [Issues](https://github.com/Microsoft/vscode-docs-authoring/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-docs-authoring)

**Marketplace Categories:**

Other

**Tags:**

markdown, yaml

**Works With:**

All Platforms

---

### 6. [learn-scaffolding](https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-scaffolding)
*Extension #279 of 329*

A Visual Studio Code extension developed to aid with scaffolding and updating Learn modules.

**Details:**

- **Extension ID:** `docsmsft.docs-scaffolding`
- **Version:** 1.0.8
- **Installs:** 89K
- **Published:** 2021-02-25
- **Last Updated:** 2024-09-19
- **VS Code Engine:** ^1.71.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=docsmsft.docs-scaffolding

**Resources:**

- 📦 [Repository](https://dev.azure.com/ceapex/Engineering/_git/learn-scaffolding)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 7. [Learn Linter](https://marketplace.visualstudio.com/items?itemName=docsmsft.learnlinter)
*Extension #280 of 329*

Learn markdown/yaml Linter and analysis engine.

**Details:**

- **Extension ID:** `docsmsft.learnlinter`
- **Version:** 5.0.0
- **Installs:** 3K
- **Published:** 2023-08-10
- **Last Updated:** 2025-07-16
- **VS Code Engine:** ^1.80.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=docsmsft.learnlinter

**Resources:**

- 📦 [Repository](https://dev.azure.com/ceapex/Engineering/_git/Linter)
- 💬 [Support](https://aka.ms/LearnLinterFeedback)

**Marketplace Categories:**

Programming Languages, Snippets, Linters

**Tags:**

json, markdown, yaml

**Works With:**

All Platforms

---

### 8. [Microsoft Learn Authoring Assistant](https://marketplace.visualstudio.com/items?itemName=docsmsft.learn-authoring-assistant)
*Extension #281 of 329*

Microsoft Learn Authoring Assistant works with GitHub Copilot Chat to help authors improve their Learn content based on grammatical rules and Microsoft's official writing guidance. This AI-powered extension analyzes Markdown content and automatically fixes grammatical and voice issues to improve cla

**Details:**

- **Extension ID:** `docsmsft.learn-authoring-assistant`
- **Version:** 1.1.2
- **Installs:** 580
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2025-10-17
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.98.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=docsmsft.learn-authoring-assistant

**Resources:**

- 📦 [Repository](https://dev.azure.com/ceapex/Engineering/_git/learn-authoring-ai-assistant)

**Marketplace Categories:**

Education, AI, Chat

**Tags:**

Ask Learn, Authoring, Azure, chat-participant, Developer, Learn, markdown

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Themes & Appearance

**Category Stats:** 8 extensions | Total Installs: 51.91M | Avg Rating: 4.5⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [C/C++ Themes](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools-themes)
*Extension #282 of 329*

UI Themes for C/C++ extension.

**Details:**

- **Extension ID:** `ms-vscode.cpptools-themes`
- **Version:** 2.0.0
- **Installs:** 50.46M
- **Rating:** ⭐ 3.5/5 (19 ratings)
- **Published:** 2019-08-30
- **Last Updated:** 2022-10-24
- **VS Code Engine:** ^1.30.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools-themes

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-cpptools.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-cpptools/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-cpptools)

**Marketplace Categories:**

Themes

**Tags:**

__web_extension, C, C++, color-theme, Microsoft, Theme, Themes

**Works With:**

All Platforms

---

### 2. [Markdown Theme Kit](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-MarkdownKit)
*Extension #283 of 329*

Theme Kit for VS Code optimized for Markdown. Based on the TextMate themes.

**Details:**

- **Extension ID:** `ms-vscode.Theme-MarkdownKit`
- **Version:** 0.1.4
- **Installs:** 565K
- **Rating:** ⭐ 4.3/5 (13 ratings)
- **Published:** 2015-11-14
- **Last Updated:** 2017-04-04
- **VS Code Engine:** 0.10.x
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-MarkdownKit

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-themes.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-themes/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-themes/blob/master/markdown-editing/README.md)

**Marketplace Categories:**

Themes

**Tags:**

__web_extension, color-theme, theme

**Works With:**

All Platforms

---

### 3. [Material Theme Kit](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-MaterialKit)
*Extension #284 of 329*

Material themes for VS Code. Based on the TextMate themes.

**Details:**

- **Extension ID:** `ms-vscode.Theme-MaterialKit`
- **Version:** 0.1.4
- **Installs:** 415K
- **Rating:** ⭐ 3.9/5 (16 ratings)
- **Published:** 2015-11-14
- **Last Updated:** 2017-04-04
- **VS Code Engine:** 0.10.x
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-MaterialKit

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-themes.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-themes/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-themes/blob/master/material/README.md)

**Marketplace Categories:**

Themes

**Tags:**

__web_extension, color-theme, theme

**Works With:**

All Platforms

---

### 4. [Tomorrow and Tomorrow Night Theme Kit](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-TomorrowKit)
*Extension #285 of 329*

Additional Tomorrow and Tomorrow Night themes for VS Code. Based on the TextMate themes.

**Details:**

- **Extension ID:** `ms-vscode.Theme-TomorrowKit`
- **Version:** 0.1.4
- **Installs:** 207K
- **Rating:** ⭐ 4.2/5 (20 ratings)
- **Published:** 2015-11-14
- **Last Updated:** 2017-04-04
- **VS Code Engine:** 0.10.x
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-TomorrowKit

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-themes.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-themes/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-themes/blob/master/tomorrow/README.md)

**Marketplace Categories:**

Themes

**Tags:**

__web_extension, color-theme, theme

**Works With:**

All Platforms

---

### 5. [1337 Theme](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-1337)
*Extension #286 of 329*

1337 Theme for VS Code based on the TextMate theme.

**Details:**

- **Extension ID:** `ms-vscode.Theme-1337`
- **Version:** 0.1.4
- **Installs:** 124K
- **Rating:** ⭐ 5/5 (9 ratings)
- **Published:** 2015-11-14
- **Last Updated:** 2017-04-04
- **VS Code Engine:** 0.10.x
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-1337

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-themes.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-themes/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-themes/blob/master/tomorrow/README.md)

**Marketplace Categories:**

Themes

**Tags:**

__web_extension, color-theme, theme

**Works With:**

All Platforms

---

### 6. [Predawn Theme Kit](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-PredawnKit)
*Extension #287 of 329*

Predawn themes for VS Code. Based on the TextMate themes.

**Details:**

- **Extension ID:** `ms-vscode.Theme-PredawnKit`
- **Version:** 0.1.4
- **Installs:** 107K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2015-11-14
- **Last Updated:** 2017-04-04
- **VS Code Engine:** 0.10.x
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-PredawnKit

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-themes.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-themes/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-themes/blob/master/predawn/README.md)

**Marketplace Categories:**

Themes

**Tags:**

__web_extension, color-theme, theme

**Works With:**

All Platforms

---

### 7. [3024 Theme Kit](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-3024Kit)
*Extension #288 of 329*

3024 Themes for VS Code based on the TextMate theme.

**Details:**

- **Extension ID:** `ms-vscode.Theme-3024Kit`
- **Version:** 0.1.4
- **Installs:** 26K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2015-11-14
- **Last Updated:** 2017-04-04
- **VS Code Engine:** 0.10.x
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.Theme-3024Kit

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-themes.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-themes/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-themes/blob/master/3024/README.md)

**Marketplace Categories:**

Themes

**Tags:**

__web_extension, color-theme, theme

**Works With:**

All Platforms

---

### 8. [Theme Tester](https://marketplace.visualstudio.com/items?itemName=ms-vscode.theme-tester)
*Extension #289 of 329*

Test a color theme without installing it

**Details:**

- **Extension ID:** `ms-vscode.theme-tester`
- **Version:** 1.2.1
- **Installs:** 3K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2021-09-27
- **Last Updated:** 2023-10-11
- **VS Code Engine:** ^1.82.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.theme-tester

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-theme-tester.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-theme-tester/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-theme-tester#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Testing Tools

**Category Stats:** 7 extensions | Total Installs: 48.04M | Avg Rating: 4.6⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [Test Runner for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-test)
*Extension #290 of 329*

Run and debug JUnit or TestNG test cases.

**Details:**

- **Extension ID:** `vscjava.vscode-java-test`
- **Version:** 0.43.2025120408
- **Installs:** 40.58M
- **Rating:** ⭐ 4.2/5 (21 ratings)
- **Published:** 2017-12-01
- **Last Updated:** 2025-12-04
- **VS Code Engine:** ^1.88.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-test

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-java-test.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-java-test/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-java-test)

**Marketplace Categories:**

Testing

**Tags:**

debug, java, junit, test, testng

**Works With:**

All Platforms

---

### 2. [Test Adapter Converter](https://marketplace.visualstudio.com/items?itemName=ms-vscode.test-adapter-converter)
*Extension #291 of 329*

Converter extension from the Test Adapter UI to native VS Code testing

**Details:**

- **Extension ID:** `ms-vscode.test-adapter-converter`
- **Version:** 0.2.1
- **Installs:** 4.65M
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2021-04-21
- **Last Updated:** 2024-11-04
- **VS Code Engine:** ^1.93.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.test-adapter-converter

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-test-adapter-converter.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-test-adapter-converter/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-test-adapter-converter#readme)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 3. [Playwright Test for VSCode](https://marketplace.visualstudio.com/items?itemName=ms-playwright.playwright)
*Extension #292 of 329*

Run Playwright Test tests in Visual Studio Code.

**Details:**

- **Extension ID:** `ms-playwright.playwright`
- **Version:** 1.1.17
- **Installs:** 1.98M
- **Rating:** ⭐ 4/5 (42 ratings)
- **Published:** 2021-08-17
- **Last Updated:** 2025-11-19
- **VS Code Engine:** ^1.93.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-playwright.playwright

**Resources:**

- 📦 [Repository](https://github.com/microsoft/playwright-vscode.git)
- 🐛 [Issues](https://github.com/microsoft/playwright/issues)
- 🏠 [Homepage](https://github.com/microsoft/playwright-vscode#readme)

**Marketplace Categories:**

Testing

**Works With:**

All Platforms

---

### 4. [Azure Load Testing](https://marketplace.visualstudio.com/items?itemName=ms-azure-load-testing.microsoft-testing)
*Extension #293 of 329*

An extension to create and run load tests using Azure Load Testing.

**Details:**

- **Extension ID:** `ms-azure-load-testing.microsoft-testing`
- **Version:** 0.1.17
- **Installs:** 337K
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2025-03-17
- **Last Updated:** 2025-07-04
- **VS Code Engine:** ^1.100.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-azure-load-testing.microsoft-testing

**Resources:**

- 📦 [Repository](https://github.com/microsoft/azureloadtesting-extension.git)
- 🐛 [Issues](https://github.com/microsoft/azureloadtesting-extension/issues)
- 🏠 [Homepage](https://github.com/microsoft/azureloadtesting-extension#readme)

**Marketplace Categories:**

Azure, Testing, AI

**Tags:**

azure, chat-participant, jmeter, language-model-tools, load testing, locust, stress testing, testing, tools

**Works With:**

All Platforms

---

### 5. [Extension Test Runner](https://marketplace.visualstudio.com/items?itemName=ms-vscode.extension-test-runner)
*Extension #294 of 329*

Runs tests in VS Code extensions

**Details:**

- **Extension ID:** `ms-vscode.extension-test-runner`
- **Version:** 0.0.12
- **Installs:** 198K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2023-10-12
- **Last Updated:** 2024-08-24
- **VS Code Engine:** ^1.88.0-insider
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.extension-test-runner

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-test-runner-ext.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-test-runner-ext/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-test-runner-ext#readme)

**Marketplace Categories:**

Testing

**Tags:**

cli, test, vscode

**Works With:**

All Platforms

---

### 6. [vscode-smoketest-check](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-smoketest-check)
*Extension #295 of 329*

A test extension used for smoke testing VS Code Extensions UI 

**Details:**

- **Extension ID:** `ms-vscode.vscode-smoketest-check`
- **Version:** 0.0.1
- **Installs:** 174K
- **Published:** 2021-07-02
- **Last Updated:** 2021-07-02
- **VS Code Engine:** ^1.57.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-smoketest-check

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-smoketest-check)
- 🐛 [Issues](https://github.com/microsoft/vscode-smoketest-check/issues)

**Marketplace Categories:**

Other

**Tags:**

__web_extension

**Works With:**

All Platforms

---

### 7. [Web Template Studio (Preview)](https://marketplace.visualstudio.com/items?itemName=WASTeamAccount.WebTemplateStudio-dev-nightly)
*Extension #296 of 329*

Web Template Studio enables developers to quickly scaffold full-stack web applications with cloud services.

**Details:**

- **Extension ID:** `WASTeamAccount.WebTemplateStudio-dev-nightly`
- **Version:** 0.7.2117301
- **Installs:** 134K
- **Rating:** ⭐ 4.6/5 (20 ratings)
- **Published:** 2019-05-14
- **Last Updated:** 2021-06-23
- **VS Code Engine:** ^1.38.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=WASTeamAccount.WebTemplateStudio-dev-nightly

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/WebTemplateStudio)
- 🐛 [Issues](https://github.com/Microsoft/WebTemplateStudio/issues)
- 🏠 [Homepage](https://github.com/Microsoft/WebTemplateStudio/blob/master/README.md)

**Marketplace Categories:**

Other

**Tags:**

Angular, ASP.NET, Azure App Service, Azure Cosmos DB, Azure Services, Boilerplate, Express, Flask, Full Stack, Moleculer, MoleculerJS, Node, Open Source, React, React Native, React Native Windows, Scaffolding, Templates, Vue.js, Web Application

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Java Development

**Category Stats:** 6 extensions | Total Installs: 154.65M | Avg Rating: 4⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [Debugger for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-debug)
*Extension #297 of 329*

A lightweight Java debugger for Visual Studio Code

**Details:**

- **Extension ID:** `vscjava.vscode-java-debug`
- **Version:** 0.58.2025111205
- **Installs:** 45.46M
- **Rating:** ⭐ 4/5 (27 ratings)
- **Published:** 2017-09-27
- **Last Updated:** 2025-11-25
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-debug

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-java-debug.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-java-debug/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-java-debug/blob/master/README.md)

**Marketplace Categories:**

Programming Languages, Debuggers

**Tags:**

debug, debugger, debuggers, debugging, java

**Works With:**

All Platforms

---

### 2. [Maven for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-maven)
*Extension #298 of 329*

Manage Maven projects, execute goals, generate project from archetype, improve user experience for Java developers.

**Details:**

- **Extension ID:** `vscjava.vscode-maven`
- **Version:** 0.45.0
- **Installs:** 41.9M
- **Rating:** ⭐ 4.1/5 (17 ratings)
- **Published:** 2017-11-13
- **Last Updated:** 2025-11-24
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-maven

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-maven.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-maven/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-maven/)

**Marketplace Categories:**

Programming Languages

**Tags:**

Java, Maven

**Works With:**

All Platforms

---

### 3. [Project Manager for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-dependency)
*Extension #299 of 329*

Manage Java projects in Visual Studio Code

**Details:**

- **Extension ID:** `vscjava.vscode-java-dependency`
- **Version:** 0.26.2025102904
- **Installs:** 40.33M
- **Rating:** ⭐ 4.1/5 (9 ratings)
- **Published:** 2018-10-19
- **Last Updated:** 2025-10-29
- **VS Code Engine:** ^1.83.1
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-dependency

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-java-dependency.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-java-dependency/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-java-dependency/blob/main/README.md)

**Marketplace Categories:**

Other

**Tags:**

explorer, java, keybindings, project

**Works With:**

All Platforms

---

### 4. [Gradle for Java](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-gradle)
*Extension #300 of 329*

Manage Gradle Projects, run Gradle tasks and provide better Gradle file authoring experience in VS Code

**Details:**

- **Extension ID:** `vscjava.vscode-gradle`
- **Version:** 3.17.2025111402
- **Installs:** 21.23M
- **Rating:** ⭐ 3.6/5 (14 ratings)
- **Published:** 2019-10-20
- **Last Updated:** 2025-11-14
- **VS Code Engine:** ^1.76.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-gradle

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-gradle.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-gradle/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-gradle#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__ext_gradle, build, compile, gradle, gradle-build, java, json, tasks

**Works With:**

All Platforms

---

### 5. [Spring Initializr Java Support](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-spring-initializr)
*Extension #301 of 329*

A lightweight extension based on Spring Initializr to generate quick start Spring Boot Java projects.

**Details:**

- **Extension ID:** `vscjava.vscode-spring-initializr`
- **Version:** 0.12.0
- **Installs:** 5.68M
- **Rating:** ⭐ 4/5 (8 ratings)
- **Published:** 2018-01-17
- **Last Updated:** 2025-11-14
- **VS Code Engine:** ^1.75.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-spring-initializr

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-spring-initializr.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-spring-initializr/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-spring-initializr)

**Marketplace Categories:**

Other

**Tags:**

java

**Works With:**

All Platforms

---

### 6. [anycode-java](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-java)
*Extension #302 of 329*

Java for Anycode

**Details:**

- **Extension ID:** `ms-vscode.anycode-java`
- **Version:** 0.0.8
- **Installs:** 54K
- **Published:** 2022-03-15
- **Last Updated:** 2025-03-28
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-java

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-anycode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-anycode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-anycode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension, java

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Web Development

**Category Stats:** 5 extensions | Total Installs: 5.55M | Avg Rating: 4.5⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [Microsoft Edge Tools for VS Code](https://marketplace.visualstudio.com/items?itemName=ms-edgedevtools.vscode-edge-devtools)
*Extension #303 of 329*

Use the Microsoft Edge Tools from within VS Code to see your site's runtime HTML structure, alter its layout, fix styling issues as well as see your site's network requests.

**Details:**

- **Extension ID:** `ms-edgedevtools.vscode-edge-devtools`
- **Version:** 2.1.10
- **Installs:** 5.51M
- **Rating:** ⭐ 4/5 (37 ratings)
- **Published:** 2019-06-21
- **Last Updated:** 2025-11-12
- **VS Code Engine:** ^1.104.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-edgedevtools.vscode-edge-devtools

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-edge-devtools.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-edge-devtools/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-edge-devtools/blob/main/README.md)

**Marketplace Categories:**

Debuggers

**Tags:**

browser, css, debuggers, devtools, dom, elements, html, javascript, javascriptreact, json, jsonc, less, network, sass, scss, styling, typescript, typescriptreact

**Works With:**

All Platforms

---

### 2. [WebAssembly DWARF Debugging](https://marketplace.visualstudio.com/items?itemName=ms-vscode.wasm-dwarf-debugging)
*Extension #304 of 329*

Enables enhanced WebAssembly debugging

**Details:**

- **Extension ID:** `ms-vscode.wasm-dwarf-debugging`
- **Version:** 1.0.1
- **Installs:** 19K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2023-09-20
- **Last Updated:** 2023-09-27
- **VS Code Engine:** ^1.83.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.wasm-dwarf-debugging

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-dwarf-debugging-ext.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-dwarf-debugging-ext/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-dwarf-debugging-ext#readme)

**Marketplace Categories:**

Debuggers

**Tags:**

debug, dwarf, wasm, webassembly

**Works With:**

All Platforms

---

### 3. [WASI based WebAssembly Execution Engine](https://marketplace.visualstudio.com/items?itemName=ms-vscode.wasm-wasi-core)
*Extension #305 of 329*

Executes WebAssemblies compiled to WASI Preview 1 on top of the VS Code API.

**Details:**

- **Extension ID:** `ms-vscode.wasm-wasi-core`
- **Version:** 1.0.2
- **Installs:** 13K
- **Published:** 2023-05-16
- **Last Updated:** 2024-09-10
- **VS Code Engine:** ^1.88.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.wasm-wasi-core

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-wasm.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-wasm/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-wasm/blob/main/wasm-wasi-core/README.md)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, vscode, wasi, wasm, webassembly

**Works With:**

All Platforms

---

### 4. [Experimental - A shell for VS Code Web](https://marketplace.visualstudio.com/items?itemName=ms-vscode.webshell)
*Extension #306 of 329*

A shell for VS Code for the Web - allows executing commands like ls, cat, ...

**Details:**

- **Extension ID:** `ms-vscode.webshell`
- **Version:** 0.13.0
- **Installs:** 4K
- **Published:** 2023-05-17
- **Last Updated:** 2023-06-07
- **VS Code Engine:** ^1.78.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.webshell

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-wasm.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-wasm/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-wasm/blob/main/webshell/README.md)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, shell, vscode, wasi, wasm

**Works With:**

All Platforms

---

### 5. [Custom Web Editors (Experimental)](https://marketplace.visualstudio.com/items?itemName=ms-vscode.web-editors)
*Extension #307 of 329*

This extension allows to use custom web editors.

**Details:**

- **Extension ID:** `ms-vscode.web-editors`
- **Version:** 0.3.0
- **Installs:** 3K
- **Published:** 2024-05-07
- **Last Updated:** 2025-02-14
- **VS Code Engine:** ^1.85.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.web-editors

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-web-editors.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-web-editors/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-web-editors#readme)

**Marketplace Categories:**

Visualization

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## IoT & Hardware

**Category Stats:** 4 extensions | Total Installs: 1.48M | Avg Rating: 4.3⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [ROS](https://marketplace.visualstudio.com/items?itemName=ms-iot.vscode-ros)
*Extension #308 of 329*

Develop Robot Operating System (ROS) with Visual Studio Code.

**Details:**

- **Extension ID:** `ms-iot.vscode-ros`
- **Version:** 0.9.6
- **Installs:** 1.32M
- **Rating:** ⭐ 4.1/5 (13 ratings)
- **Published:** 2019-06-28
- **Last Updated:** 2023-11-13
- **VS Code Engine:** ^1.69.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-iot.vscode-ros

**Resources:**

- 📦 [Repository](https://github.com/ms-iot/vscode-ros.git)
- 🐛 [Issues](https://github.com/ms-iot/vscode-ros/issues)
- 🏠 [Homepage](https://github.com/ms-iot/vscode-ros)

**Marketplace Categories:**

Debuggers

**Tags:**

__ext_action, __ext_launch, __ext_msg, __ext_rviz, __ext_srdf, __ext_srv, __ext_test, __ext_urdf, __ext_xacro, cpp, csharp, debuggers, python, robot, ros, ROS Message, ros.msg, xml, yaml

**Works With:**

All Platforms

---

### 2. [IoT Device Cube](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.vscode-iot-device-cube)
*Extension #309 of 329*

Utility extension to access to physical devices

**Details:**

- **Extension ID:** `vsciot-vscode.vscode-iot-device-cube`
- **Version:** 0.3.0
- **Installs:** 122K
- **Published:** 2019-07-05
- **Last Updated:** 2020-08-18
- **VS Code Engine:** ^1.43.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.vscode-iot-device-cube

**Resources:**

- 📦 [Repository](https://github.com/VSChina/vscode-iot-device-cube.git)
- 🐛 [Issues](https://github.com/VSChina/vscode-iot-device-cube/issues)
- 🏠 [Homepage](https://github.com/VSChina/vscode-iot-device-cube/blob/master/README.md)

**Marketplace Categories:**

Other

**Works With:**

All Platforms

---

### 3. [Microsoft Kiota](https://marketplace.visualstudio.com/items?itemName=ms-graph.kiota)
*Extension #310 of 329*

Client generator for HTTP REST APIs described by OpenAPI which helps eliminate the need to take a dependency on a different API client for every API that you need to call, as well as limiting the generation to the exact API surface area you're interested in, thanks to a filtering capability.

**Details:**

- **Extension ID:** `ms-graph.kiota`
- **Version:** 1.29.100000001
- **Installs:** 24K
- **Rating:** ⭐ 3.7/5 (6 ratings)
- **Published:** 2023-04-05
- **Last Updated:** 2025-10-24
- **VS Code Engine:** ^1.103.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-graph.kiota

**Resources:**

- 📦 [Repository](https://github.com/microsoft/kiota.git)
- 🐛 [Issues](https://github.com/microsoft/kiota/issues)
- 🏠 [Homepage](https://github.com/microsoft/kiota#readme)

**Marketplace Categories:**

Other

**Tags:**

__ext_json, api client, api manifest, api plugins, copilot, json, openapi

**Works With:**

All Platforms

---

### 4. [Microsoft Open Enclave](https://marketplace.visualstudio.com/items?itemName=ms-iot.msiot-vscode-openenclave)
*Extension #311 of 329*

Develop, deploy, and debug your Microsoft Open Enclave

**Details:**

- **Extension ID:** `ms-iot.msiot-vscode-openenclave`
- **Version:** 1.0.10
- **Installs:** 19K
- **Rating:** ⭐ 5/5 (2 ratings)
- **Published:** 2019-05-02
- **Last Updated:** 2019-06-18
- **VS Code Engine:** ^1.32.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-iot.msiot-vscode-openenclave

**Resources:**

- 📦 [Repository](https://github.com/microsoft/openenclave.git)
- 🐛 [Issues](https://github.com/microsoft/openenclave/issues)
- 🏠 [Homepage](https://github.com/microsoft/openenclave/blob/feature.new_platforms/new_platforms/vscode-extension/marketplace-homepage.md)

**Marketplace Categories:**

Debuggers, Azure

**Tags:**

azure, cloud, debug, iot, IoT Edge, Open Enclave, Trusted Agent

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## C/C++ Development

**Category Stats:** 3 extensions | Total Installs: 91.82M | Avg Rating: 3.4⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [C/C++](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools)
*Extension #312 of 329*

C/C++ IntelliSense, debugging, and code browsing.

**Details:**

- **Extension ID:** `ms-vscode.cpptools`
- **Version:** 1.29.2
- **Installs:** 91.67M
- **Rating:** ⭐ 3.4/5 (591 ratings)
- **Published:** 2016-03-29
- **Last Updated:** 2025-12-03
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-cpptools.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-cpptools/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-cpptools)

**Marketplace Categories:**

Programming Languages, Snippets, Linters, Debuggers, Formatters

**Tags:**

C, C++, cpp, cuda-cpp, debuggers, IntelliSense, json, keybindings, Microsoft, multi-root ready

**Works With:**

Alpine Linux x64, Windows x64, macOS Intel, Alpine Linux ARM64, Linux x64, Linux ARM, Windows ARM64, Linux ARM64, macOS Apple Silicon, Windows x86

---

### 2. [anycode-cpp](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-cpp)
*Extension #313 of 329*

C/C++ for Anycode

**Details:**

- **Extension ID:** `ms-vscode.anycode-cpp`
- **Version:** 0.0.7
- **Installs:** 71K
- **Published:** 2022-03-15
- **Last Updated:** 2025-03-28
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-cpp

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-anycode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-anycode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-anycode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension, c++

**Works With:**

All Platforms

---

### 3. [anycode-cpp](https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-cpp)
*Extension #314 of 329*

C/C++ for Anycode

**Details:**

- **Extension ID:** `ms-vscode.anycode-cpp`
- **Version:** 0.0.7
- **Installs:** 71K
- **Published:** 2022-03-15
- **Last Updated:** 2025-03-28
- **VS Code Engine:** ^1.67.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.anycode-cpp

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-anycode.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-anycode/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-anycode#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

__web_extension, c++

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Debugging Tools

**Category Stats:** 3 extensions | Total Installs: 771K | Avg Rating: 4.8⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [[Deprecated] Node Debug](https://marketplace.visualstudio.com/items?itemName=ms-vscode.node-debug2)
*Extension #315 of 329*

Node.js debugging support

**Details:**

- **Extension ID:** `ms-vscode.node-debug2`
- **Version:** 1.43.0
- **Installs:** 374K
- **Rating:** ⭐ 4.7/5 (3 ratings)
- **Published:** 2016-09-22
- **Last Updated:** 2021-08-03
- **VS Code Engine:** ^1.60.0-insider
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.node-debug2

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-node-debug2.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-node-debug2/issues)

**Marketplace Categories:**

Debuggers

**Tags:**

debuggers, javascript, node

**Works With:**

All Platforms

---

### 2. [[Deprecated] Node Debug (legacy)](https://marketplace.visualstudio.com/items?itemName=ms-vscode.node-debug)
*Extension #316 of 329*

Node.js debugging support (versions < 8.0)

**Details:**

- **Extension ID:** `ms-vscode.node-debug`
- **Version:** 1.45.0
- **Installs:** 231K
- **Published:** 2016-09-22
- **Last Updated:** 2021-08-03
- **VS Code Engine:** ^1.60.0-insider
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.node-debug

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-node-debug.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-node-debug/issues)

**Marketplace Categories:**

Debuggers

**Tags:**

debuggers, javascript, multi-root ready, node

**Works With:**

All Platforms

---

### 3. [Microsoft Office Add-ins Development Kit](https://marketplace.visualstudio.com/items?itemName=msoffice.microsoft-office-add-in-debugger)
*Extension #317 of 329*

VSCode Extension for super powering WXP XML Add-ins.

**Details:**

- **Extension ID:** `msoffice.microsoft-office-add-in-debugger`
- **Version:** 1.0.1
- **Installs:** 166K
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2020-05-20
- **Last Updated:** 2024-12-19
- **VS Code Engine:** ^1.88.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=msoffice.microsoft-office-add-in-debugger

**Resources:**

- 📦 [Repository](https://github.com/OfficeDev/OfficeAddinDevKit.git)
- 🐛 [Issues](https://github.com/OfficeDev/OfficeAddinDevKit/issues)
- 💬 [Support](https://aka.ms/officedevkitnewissue)
- 🏠 [Homepage](https://github.com/OfficeDev/OfficeAddinDevKit/blob/main/README.md)

**Marketplace Categories:**

Debuggers

**Tags:**

add-in, dev kit, Microsoft, office, office XML add-in, office-addin-dev-kit-output, toolkit, xml

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Quantum Computing

**Category Stats:** 2 extensions | Total Installs: 29K | Avg Rating: 5⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [Azure Quantum Development Kit (QDK)](https://marketplace.visualstudio.com/items?itemName=quantum.qsharp-lang-vscode)
*Extension #318 of 329*

Q# and OpenQASM Language Support

**Details:**

- **Extension ID:** `quantum.qsharp-lang-vscode`
- **Version:** 1.22.0
- **Installs:** 25K
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2023-09-18
- **Last Updated:** 2025-10-31
- **VS Code Engine:** ^1.77.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=quantum.qsharp-lang-vscode

**Resources:**

- 📦 [Repository](https://github.com/microsoft/qsharp.git)
- 🐛 [Issues](https://github.com/microsoft/qsharp/issues)
- 🏠 [Homepage](https://github.com/microsoft/qsharp#readme)

**Marketplace Categories:**

Programming Languages, Notebooks

**Tags:**

__ext_inc, __ext_qasm, __ext_qs, __ext_qsc, __web_extension, debuggers, json, language-model-tools, openqasm, Q, QASM, QCircuit, qsc, qsharp, qsharpcircuit, tools

**Works With:**

All Platforms

---

### 2. [[DEV BUILD] Azure Quantum Development Kit (QDK)](https://marketplace.visualstudio.com/items?itemName=quantum.qsharp-lang-vscode-dev)
*Extension #319 of 329*

Q# and OpenQASM Language Support

**Details:**

- **Extension ID:** `quantum.qsharp-lang-vscode-dev`
- **Version:** 1.22.4
- **Installs:** 4K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2023-09-07
- **Last Updated:** 2025-11-25
- **VS Code Engine:** ^1.77.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=quantum.qsharp-lang-vscode-dev

**Resources:**

- 📦 [Repository](https://github.com/microsoft/qsharp.git)
- 🐛 [Issues](https://github.com/microsoft/qsharp/issues)
- 🏠 [Homepage](https://github.com/microsoft/qsharp#readme)

**Marketplace Categories:**

Programming Languages, Notebooks

**Tags:**

__ext_inc, __ext_qasm, __ext_qs, __ext_qsc, __web_extension, debuggers, json, language-model-tools, openqasm, Q, QASM, QCircuit, qsc, qsharp, qsharpcircuit, tools

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Collaboration Tools

**Category Stats:** 2 extensions | Total Installs: 27.67M | Avg Rating: 4.4⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [Live Share](https://marketplace.visualstudio.com/items?itemName=MS-vsliveshare.vsliveshare)
*Extension #320 of 329*

Real-time collaborative development from the comfort of your favorite tools.

**Details:**

- **Extension ID:** `MS-vsliveshare.vsliveshare`
- **Version:** 1.0.5959
- **Installs:** 22.23M
- **Rating:** ⭐ 4.2/5 (158 ratings)
- **Published:** 2018-01-29
- **Last Updated:** 2025-08-13
- **VS Code Engine:** ^1.72.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-vsliveshare.vsliveshare

**Resources:**

- 📦 [Repository](https://github.com/MicrosoftDocs/live-share)
- 🐛 [Issues](https://github.com/MicrosoftDocs/live-share/issues)
- 💬 [Support](https://aka.ms/vsls-issues)
- 🏠 [Homepage](https://aka.ms/vsls)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, Co-Debug, Co-Edit, Collaboration, debuggers, keybindings, liveshare, Remote, remote-menu, Sharing

**Works With:**

All Platforms

---

### 2. [[Deprecated] Live Share Audio](https://marketplace.visualstudio.com/items?itemName=MS-vsliveshare.vsliveshare-audio)
*Extension #321 of 329*

Adds audio calling capabilities to Visual Studio Live Share

**Details:**

- **Extension ID:** `MS-vsliveshare.vsliveshare-audio`
- **Version:** 0.1.93
- **Installs:** 5.45M
- **Rating:** ⭐ 4.6/5 (8 ratings)
- **Published:** 2018-07-24
- **Last Updated:** 2023-01-27
- **VS Code Engine:** ^1.21.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=MS-vsliveshare.vsliveshare-audio

**Resources:**

- 📦 [Repository](https://github.com/MicrosoftDocs/live-share)
- 🐛 [Issues](https://github.com/MicrosoftDocs/live-share/issues)
- 💬 [Support](https://aka.ms/vsls-issues)
- 🏠 [Homepage](https://aka.ms/vsls-audio)

**Marketplace Categories:**

Other

**Tags:**

Audio, Call, Collaboration, Sharing, Voice

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Business Central (AL)

**Category Stats:** 2 extensions | Total Installs: 477K | Avg Rating: 4.8⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [AL Language extension for Microsoft Dynamics 365 Business Central](https://marketplace.visualstudio.com/items?itemName=ms-dynamics-smb.al)
*Extension #322 of 329*

Development tools for Dynamics 365 Business Central

**Details:**

- **Extension ID:** `ms-dynamics-smb.al`
- **Version:** 17.0.1869541
- **Installs:** 470K
- **Rating:** ⭐ 4.5/5 (26 ratings)
- **Published:** 2017-08-10
- **Last Updated:** 2025-10-23
- **VS Code Engine:** ^1.100.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-dynamics-smb.al

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/AL.git)
- 🐛 [Issues](https://github.com/microsoft/al/issues)
- 🏠 [Homepage](https://github.com/Microsoft/AL#readme)

**Marketplace Categories:**

Programming Languages, Snippets, Debuggers, Formatters

**Tags:**

__ext_al, __ext_dal, AL, color-theme, debuggers, Dynamics 365 Business Central, ERP, icon-theme, json, keybindings, SMB, snippet, theme

**Works With:**

All Platforms

---

### 2. [Performance Toolkit](https://marketplace.visualstudio.com/items?itemName=ms-dynamics-smb.bcpt)
*Extension #323 of 329*

Performance development tools for Dynamics 365 Business Central

**Details:**

- **Extension ID:** `ms-dynamics-smb.bcpt`
- **Version:** 1.0.1857523
- **Installs:** 7K
- **Rating:** ⭐ 5/5 (1 ratings)
- **Published:** 2022-11-30
- **Last Updated:** 2025-10-15
- **VS Code Engine:** ^1.68.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-dynamics-smb.bcpt

**Resources:**

- 📦 [Repository](https://github.com/microsoft/ALAppExtensions.git)
- 🐛 [Issues](https://github.com/microsoft/ALAppExtensions/issues)
- 🏠 [Homepage](https://github.com/microsoft/ALAppExtensions#readme)

**Marketplace Categories:**

Other

**Tags:**

AL, BCPT, Dynamics 365 Business Central, ERP, Performance Toolkit, SMB

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Linters & Formatters

**Category Stats:** 2 extensions | Total Installs: 51.32M | Avg Rating: 3.6⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [ESLint](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint)
*Extension #324 of 329*

Integrates ESLint JavaScript into VS Code.

**Details:**

- **Extension ID:** `dbaeumer.vscode-eslint`
- **Version:** 3.0.21
- **Installs:** 46.85M
- **Rating:** ⭐ 4.3/5 (245 ratings)
- **Published:** 2015-11-16
- **Last Updated:** 2025-11-13
- **VS Code Engine:** ^1.90.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-eslint.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-eslint/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-eslint#readme)

**Marketplace Categories:**

Programming Languages, Linters

**Tags:**

__ext_eslintignore, ignore, javascript, json, jsonc, linters, multi-root ready

**Works With:**

All Platforms

---

### 2. [TSLint](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-typescript-tslint-plugin)
*Extension #325 of 329*

TSLint support for Visual Studio Code

**Details:**

- **Extension ID:** `ms-vscode.vscode-typescript-tslint-plugin`
- **Version:** 1.3.4
- **Installs:** 4.47M
- **Rating:** ⭐ 3/5 (62 ratings)
- **Published:** 2018-10-23
- **Last Updated:** 2022-03-31
- **VS Code Engine:** ^1.32.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-typescript-tslint-plugin

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-typescript-tslint-plugin.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-typescript-tslint-plugin/issues)
- 🏠 [Homepage](https://github.com/Microsoft/vscode-typescript-tslint-plugin#readme)

**Marketplace Categories:**

Programming Languages

**Tags:**

javascript, javascriptreact, json, lint, linters, ts, tslint, typescript, typescriptreact

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Deprecated Extensions

**Category Stats:** 1 extensions | Total Installs: 42K | Avg Rating: 3⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [[DEPRECATED] LaTeX Language Support](https://marketplace.visualstudio.com/items?itemName=ms-vscode.latex)
*Extension #326 of 329*

[DEPRECATED] LaTeX - colorizer, grammar and snippets.

**Details:**

- **Extension ID:** `ms-vscode.latex`
- **Version:** 1.0.0
- **Installs:** 42K
- **Rating:** ⭐ 3/5 (20 ratings)
- **Published:** 2015-11-17
- **Last Updated:** 2017-08-09
- **VS Code Engine:** ^1.6.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.latex

**Resources:**

- 📦 [Repository](https://github.com/Microsoft/vscode-LaTeX.git)
- 🐛 [Issues](https://github.com/Microsoft/vscode-LaTeX/issues)

**Marketplace Categories:**

Programming Languages, Snippets

**Tags:**

__ext_.bbx, __ext_.cbx, __ext_.cls, __ext_.sty, __ext_.tex, __ext_bbx, __ext_cbx, __ext_cls, __ext_sty, __ext_tex, latex, tex

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Data Science & Notebooks

**Category Stats:** 1 extensions | Total Installs: 46K | Avg Rating: 3⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [Markdown Notebook](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-markdown-notebook)
*Extension #327 of 329*

An extension for editing markdown files in VS Code notebooks

**Details:**

- **Extension ID:** `ms-vscode.vscode-markdown-notebook`
- **Version:** 0.0.26
- **Installs:** 46K
- **Rating:** ⭐ 3/5 (2 ratings)
- **Published:** 2020-06-16
- **Last Updated:** 2021-11-03
- **VS Code Engine:** ^1.57.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-markdown-notebook

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-markdown-notebook.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-markdown-notebook/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-markdown-notebook#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## Version Control

**Category Stats:** 1 extensions | Total Installs: 255K | Avg Rating: 5⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [GitHub Issue Notebooks](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-github-issue-notebooks)
*Extension #328 of 329*

GitHub Issue Notebooks for VS Code

**Details:**

- **Extension ID:** `ms-vscode.vscode-github-issue-notebooks`
- **Version:** 0.0.134
- **Installs:** 255K
- **Rating:** ⭐ 5/5 (3 ratings)
- **Published:** 2020-04-08
- **Last Updated:** 2025-11-07
- **VS Code Engine:** ^1.101.0-insider
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-github-issue-notebooks

**Resources:**

- 📦 [Repository](https://github.com/microsoft/vscode-github-issue-notebooks.git)
- 🐛 [Issues](https://github.com/microsoft/vscode-github-issue-notebooks/issues)
- 🏠 [Homepage](https://github.com/microsoft/vscode-github-issue-notebooks#readme)

**Marketplace Categories:**

Other

**Tags:**

__web_extension, GitHubIssues, github-issues

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---

## PowerShell Development

**Category Stats:** 1 extensions | Total Installs: 17.05M | Avg Rating: 3.6⭐

[⬆ Back to Table of Contents](#-table-of-contents)

---

### 1. [PowerShell](https://marketplace.visualstudio.com/items?itemName=ms-vscode.PowerShell)
*Extension #329 of 329*

Develop PowerShell modules, commands and scripts in Visual Studio Code!

**Details:**

- **Extension ID:** `ms-vscode.PowerShell`
- **Version:** 2025.5.0
- **Installs:** 17.05M
- **Rating:** ⭐ 3.6/5 (171 ratings)
- **Published:** 2015-11-17
- **Last Updated:** 2025-09-24
- **VS Code Engine:** ^1.101.0
- **Marketplace:** https://marketplace.visualstudio.com/items?itemName=ms-vscode.PowerShell

**Resources:**

- 📦 [Repository](https://github.com/PowerShell/vscode-powershell.git)
- 🐛 [Issues](https://github.com/PowerShell/vscode-powershell/issues)
- 🏠 [Homepage](https://github.com/PowerShell/vscode-powershell/blob/main/README.md)

**Marketplace Categories:**

Programming Languages, Snippets, Themes, Linters, Debuggers, Formatters

**Tags:**

color-theme, debuggers, keybindings, powershell, pwsh, snippet, theme

**Works With:**

All Platforms

---

[⬆ Back to Table of Contents](#-table-of-contents)

---


## 📄 Document Information

- **Version:** 2.0
- **Total Extensions:** 329
- **Categories:** 25
- **Generated:** 2025-12-05 14:14:06
- **Script:** generate_markdown.ps1

---

*This document was automatically generated from the Visual Studio Code Marketplace API.*
*For the most up-to-date information, visit the [Microsoft Publisher Page](https://marketplace.visualstudio.com/publishers/Microsoft).*

